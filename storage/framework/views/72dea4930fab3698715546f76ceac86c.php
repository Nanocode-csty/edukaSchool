<?php $__env->startSection('titulo', 'Registro y listado de docentes'); ?>
<?php $__env->startSection('contenidoplantilla'); ?>

    <style>
        .collapse-row.odd {
            background-color: #f5f5f5;
        }

        .collapse-row.even {
            background-color: #e0e0e0;
        }


        .collapse-row {
            transition: all 0.3s ease-in-out;
            display: none;
        }

        .collapse-row.show {
            display: table-row !important;
        }
    </style>


    <?php echo $__env->make('ccomponentes.loader', ['id' => 'loaderPrincipal'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?> 

    <div class="container-fluid margen-movil-2" id="contenido-principal" style="position: relative;">
        <div class="row mt-4 ml-1 mr-1">
            <div class="col-12">
                <div class="card">
                    <button class="estilo-info btn btn-block btn_header header_6" type="button" data-toggle="collapse"
                        aria-expanded="true">
                        <i class="fas fa-file-signature m-1"></i>&nbsp;Registro y listado de docentes
                    </button>

                    <div class="card-body" style="background-color: #fff">
                        <div class="alert alert-interactive" id="alertMatricula">
                            <i class="fas fa-info-circle me-2"></i>
                            <div class="alert-content">
                                <strong>En esta sección podrás registrar estudiantes y consultar sus datos.</strong>
                                <br>

                                <span>
                                    <b>Estimado Usuario:</b> Asegúrate de revisar cuidadosamente los datos antes de
                                    guardarlos. Esta información será utilizada para la gestión académica y administrativa
                                    del estudiante. Cualquier modificación posterior debe realizarse con responsabilidad y
                                    siguiendo los protocolos establecidos por la institución.
                                </span>
                            </div>
                            <button class="btn-close" onclick="cerrarAlert('alertMatricula')">&times;</button>

                        </div>

                        <div class="row d-flex align-items-center justify-content-center gap-4 my-4">
                            <div class="col-md-3 d-flex align-items-center justify-content-center">

                                <a id="btnRegistrar" href="<?php echo e(route('docente.create')); ?>"
                                    class="panel-opcion panel-opcion-btn text-center p-4">

                                    <svg class="icon-panel" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 823.23953 735.54047" xmlns:xlink="http://www.w3.org/1999/xlink"
                                        role="img" artist="Katerina Limpitsouni" source="https://undraw.co/">
                                        <path
                                            d="M958.69312,482.97282,369.50873,735.0358a16.51867,16.51867,0,0,1-21.66-8.68L192.42164,363.05291a12.10184,12.10184,0,0,1,6.3588-15.86858l597.105-255.45154a12.31112,12.31112,0,0,1,16.14288,6.46929L967.37315,461.31279A16.51867,16.51867,0,0,1,958.69312,482.97282Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M931.44937,471.96945,382.3449,706.88557c-8.04931,3.44363-17.22127.11287-20.44591-7.42456L226.55112,383.09225c-2.39149-5.59.51531-12.214,6.48-14.76586L789.51739,130.25223c6.06362-2.59412,12.97442-.08152,15.40529,5.60051L940.19911,452.0545C943.42375,459.59193,939.49857,468.52586,931.44937,471.96945Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#fff" />
                                        <path
                                            d="M812.54362,99.99072,193.01749,365.03441l-3.30415-7.72328a16.57377,16.57377,0,0,1,8.71566-21.72945L787.50463,83.56523a16.57368,16.57368,0,0,1,21.73484,8.7022Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#419EBD" />
                                        <circle cx="27.40974" cy="259.52048" r="4.28342" fill="#fff" />
                                        <circle cx="42.35802" cy="253.12536" r="4.28342" fill="#fff" />
                                        <circle cx="57.3063" cy="246.73023" r="4.28342" fill="#fff" />
                                        <path
                                            d="M492.26816,446.007l-73.22891,31.32856a16.519,16.519,0,0,1-21.66-8.68l-40.42342-94.4877a16.519,16.519,0,0,1,8.68-21.66l73.2289-31.32855a16.519,16.519,0,0,1,21.66,8.68L500.9482,424.347A16.519,16.519,0,0,1,492.26816,446.007Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M616.17675,392.99683l-73.2289,31.32856a16.519,16.519,0,0,1-21.66-8.68l-40.42343-94.48769a16.519,16.519,0,0,1,8.68-21.66l73.22891-31.32855a16.519,16.519,0,0,1,21.66,8.68l40.42342,94.48769A16.519,16.519,0,0,1,616.17675,392.99683Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M740.08535,339.98665l-73.22913,31.32866a16.519,16.519,0,0,1-21.66-8.68l-40.42343-94.4877a16.519,16.519,0,0,1,8.68-21.66l73.22913-31.32865a16.519,16.519,0,0,1,21.66,8.68l40.42343,94.48769A16.519,16.519,0,0,1,740.08535,339.98665Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M554.80812,592.191l-73.22891,31.32856a16.519,16.519,0,0,1-21.66-8.68l-40.42342-94.48769a16.519,16.519,0,0,1,8.68-21.66l73.2289-31.32855a16.519,16.519,0,0,1,21.66,8.68L563.48816,570.531A16.519,16.519,0,0,1,554.80812,592.191Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M678.71671,539.1808l-73.2289,31.32856a16.519,16.519,0,0,1-21.66-8.68l-40.42343-94.48769a16.519,16.519,0,0,1,8.68-21.66L625.3133,414.353a16.519,16.519,0,0,1,21.66,8.68l40.42342,94.48769A16.519,16.519,0,0,1,678.71671,539.1808Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M802.62531,486.17063l-73.22913,31.32865a16.519,16.519,0,0,1-21.66-8.68l-40.42343-94.4877a16.519,16.519,0,0,1,8.68-21.66l73.22913-31.32865a16.519,16.519,0,0,1,21.66,8.68l40.42343,94.48769A16.519,16.519,0,0,1,802.62531,486.17063Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#f2f2f2" />
                                        <path
                                            d="M995.11977,817.77023H354.28126a16.51867,16.51867,0,0,1-16.5-16.5V406.11643a12.10185,12.10185,0,0,1,12.0879-12.08838H999.32265a12.31112,12.31112,0,0,1,12.29712,12.29736V801.27023A16.51867,16.51867,0,0,1,995.11977,817.77023Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#e6e6e6" />
                                        <path
                                            d="M974.4,796.93791H377.15521c-8.755,0-15.87757-6.66992-15.87757-14.86816v-344.105c0-6.08008,5.278-11.02686,11.76563-11.02686H978.317c6.59522,0,11.96069,5.02832,11.96069,11.2085V782.06975C990.27764,790.268,983.15484,796.93791,974.4,796.93791Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#fff" />
                                        <path
                                            d="M1011.39,408.17258H337.5497v-8.40039a16.57377,16.57377,0,0,1,16.56006-16.54981H994.83A16.57369,16.57369,0,0,1,1011.39,399.77219Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#419EBD" />
                                        <circle cx="179.26482" cy="313.49262" r="4.28342" fill="#fff" />
                                        <circle cx="195.52363" cy="313.49262" r="4.28342" fill="#fff" />
                                        <circle cx="211.78243" cy="313.49262" r="4.28342" fill="#fff" />
                                        <rect x="450.20268" y="441.30559" width="324" height="41.02752" rx="20.51374"
                                            fill="#e6e6e6" />
                                        <rect x="450.20268" y="509.19439" width="324" height="41.02752" rx="20.51374"
                                            fill="#e6e6e6" />
                                        <rect x="450.20268" y="577.08318" width="324" height="41.02752" rx="20.51374"
                                            fill="#e6e6e6" />
                                        <path
                                            d="M611.97238,611.93791a110,110,0,1,1-110-110A110.03475,110.03475,0,0,1,611.97238,611.93791Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#fff" />
                                        <path
                                            d="M552.81243,630.76c-1.20147,70.35088-105.847,70.33985-107.03823-.00226C446.97559,560.40911,551.62115,560.42014,552.81243,630.76Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#419EBD" />
                                        <path
                                            d="M497.16883,652.596l-.05566-2c13.78447,1.84042,16.75312-15.24334,2.85156-8.85644l3.17334-23.19043,1.98145.27148-2.69922,19.72657C517.05748,636.36835,511.64336,654.59479,497.16883,652.596Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#2f2e41" />
                                        <rect x="326.5386" y="538.29199" width="10.77148" height="2" fill="#2f2e41" />
                                        <rect x="292.5386" y="538.29199" width="10.77148" height="2" fill="#2f2e41" />
                                        <path
                                            d="M516.6423,705.58794c2.1001-14.17-14.48975-11.37-23.27-11.51a9.518,9.518,0,0,0-9.27979,11.5,52.0381,52.0381,0,0,0-18.49023,10.2,110.47619,110.47619,0,0,0,70.43018.78A51.77082,51.77082,0,0,0,516.6423,705.58794Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#2f2e41" />
                                        <path
                                            d="M474.13161,605.44681c9.9184-5.73223,21.94644-1.3346,32.19177,1.06973,19.43829,5.9755,46.6969-1.50709,49.06067-24.66246,2.45987-21.53491-26.99912-33.5371-36.41493-12.10366l2.74162-.35832c-30.53675-45.38215-83.65461-43.661-83.30532,18.73141-.11322,1.92749,2.88716,1.921,2.99939-.00115-1.12422-36.76687,23.20862-61.89966,57.60511-38.27162,7.87161,5.74339,14.24117,13.35026,20.11041,21.05552,3.58221,4.01447,4.36067-13.27437,21.50583-8.746,18.86876,7.31,12.712,34.32848-4.15335,40.25979-20.83849,10.64622-43.40723-9.9954-63.8555.435a1.50174,1.50174,0,0,0,1.5143,2.59173Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#2f2e41" />
                                        <path
                                            d="M501.97238,724.93791a112.90489,112.90489,0,1,1,34.98755-5.52685A112.72659,112.72659,0,0,1,501.97238,724.93791Zm0-220a107.01519,107.01519,0,1,0,107,107A107.12118,107.12118,0,0,0,501.97238,504.93791Z"
                                            transform="translate(-188.38023 -82.22977)" fill="#3f3d56" />
                                    </svg>

                                    <h5 class="mt-3">Registrar</h5>
                                    <p class="text-muted">Crear registros</p>

                                </a>

                            </div>
                            <div class="col-md-3 d-flex align-items-center justify-content-center">

                                <a class="panel-opcion panel-opcion-btn text-center p-4"
                                    data-target="#contenido-2-consultar">

                                    <svg class="icon-panel" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 748.974 457.275" xmlns:xlink="http://www.w3.org/1999/xlink"
                                        role="img" artist="Katerina Limpitsouni" source="https://undraw.co/">
                                        <g id="Group_201" data-name="Group 201" transform="translate(-382.003 -195.455)">
                                            <g id="Group_200" data-name="Group 200"
                                                transform="translate(382.003 195.455)">
                                                <path id="Path_3120-1858" data-name="Path 3120"
                                                    d="M695.225,508.82,433.394,576.244a34.622,34.622,0,0,1-42.114-24.866L312.1,243.879a34.622,34.622,0,0,1,24.866-42.114l243.591-62.727L642.9,166.948l77.191,299.757A34.622,34.622,0,0,1,695.225,508.82Z"
                                                    transform="translate(-311.003 -139.037)" fill="#f2f2f2" />
                                                <path id="Path_3121-1859" data-name="Path 3121"
                                                    d="M338.989,210.925a24.655,24.655,0,0,0-17.708,29.99l79.185,307.5a24.655,24.655,0,0,0,29.99,17.708L692.287,498.7a24.655,24.655,0,0,0,17.708-29.99L634,173.595l-54.792-24.529Z"
                                                    transform="translate(-310.548 -138.556)" fill="#fff" />
                                                <path id="Path_3122-1860" data-name="Path 3122"
                                                    d="M629.927,168.5l-40.522,10.435a11.518,11.518,0,0,1-14.026-8.282l-7.707-29.929a.72.72,0,0,1,.989-.837l61.379,27.258a.72.72,0,0,1-.113,1.355Z"
                                                    transform="translate(-298.695 -139)" fill="#f2f2f2" />
                                                <path id="Path_3123-1861" data-name="Path 3123"
                                                    d="M612.519,418.284l-119.208,30.7a5.759,5.759,0,0,1-2.872-11.154l119.208-30.7a5.759,5.759,0,1,1,2.872,11.154Z"
                                                    transform="translate(-302.605 -126.189)" fill="#ccc" />
                                                <path id="Path_3124-1862" data-name="Path 3124"
                                                    d="M640.149,430.592,497.936,467.214a5.759,5.759,0,1,1-2.872-11.154l142.213-36.622a5.759,5.759,0,0,1,2.872,11.154Z"
                                                    transform="translate(-302.384 -125.599)" fill="#ccc" />
                                                <circle id="Ellipse_44" data-name="Ellipse 44" cx="20.355"
                                                    cy="20.355" r="20.355" transform="translate(121.697 319.055)"
                                                    fill="#419EBD" />
                                                <path id="Path_3125-1863" data-name="Path 3125"
                                                    d="M604.421,374.437,446.1,415.191a17.835,17.835,0,0,1-21.694-12.812L391.229,273.49A17.835,17.835,0,0,1,404.041,251.8l158.32-40.754a17.835,17.835,0,0,1,21.694,12.812l33.178,128.889A17.835,17.835,0,0,1,604.421,374.437Z"
                                                    transform="translate(-307.183 -135.611)" fill="#fff" />
                                                <path id="Path_3126-1864" data-name="Path 3126"
                                                    d="M604.421,374.437,446.1,415.191a17.835,17.835,0,0,1-21.694-12.812L391.229,273.49A17.835,17.835,0,0,1,404.041,251.8l158.32-40.754a17.835,17.835,0,0,1,21.694,12.812l33.178,128.889A17.835,17.835,0,0,1,604.421,374.437ZM404.563,253.826a15.737,15.737,0,0,0-11.3,19.142l33.178,128.889a15.737,15.737,0,0,0,19.142,11.3L603.9,372.407a15.737,15.737,0,0,0,11.3-19.142L582.025,224.376a15.737,15.737,0,0,0-19.142-11.3Z"
                                                    transform="translate(-307.183 -135.611)" fill="#e6e6e6" />
                                                <path id="Path_411-1865" data-name="Path 411"
                                                    d="M550.66,252.63l-79.9,20.568a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l81.335-20.937c3.286,1.665,2.421,5.07.091,5.67Z"
                                                    transform="translate(-303.514 -133.861)" fill="#f2f2f2" />
                                                <path id="Path_412-1866" data-name="Path 412"
                                                    d="M554.1,266l-79.9,20.568a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l81.335-20.937c3.286,1.665,2.421,5.07.091,5.67Z"
                                                    transform="translate(-303.349 -133.22)" fill="#f2f2f2" />
                                                <path id="Path_413-1867" data-name="Path 413"
                                                    d="M461.146,298.825,436.761,305.1a3.1,3.1,0,0,1-3.776-2.23L425.577,274.1a3.1,3.1,0,0,1,2.23-3.776l24.385-6.277a3.105,3.105,0,0,1,3.776,2.23l7.408,28.777a3.1,3.1,0,0,1-2.23,3.776Z"
                                                    transform="translate(-305.513 -133.047)" fill="#419EBD" />
                                                <path id="Path_414-1868" data-name="Path 414"
                                                    d="M562.854,293.445,440.909,324.835a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l123.38-31.76c3.286,1.665,2.421,5.07.091,5.67Z"
                                                    transform="translate(-304.946 -131.904)" fill="#f2f2f2" />
                                                <path id="Path_415-1869" data-name="Path 415"
                                                    d="M566.3,306.822,444.353,338.213a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l123.38-31.76c3.286,1.665,2.421,5.07.091,5.67Z"
                                                    transform="translate(-304.781 -131.263)" fill="#f2f2f2" />
                                                <path id="Path_416-1870" data-name="Path 416"
                                                    d="M569.739,320.192,447.794,351.582a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l123.379-31.76c3.286,1.665,2.421,5.07.091,5.67Z"
                                                    transform="translate(-304.616 -130.621)" fill="#f2f2f2" />
                                                <path id="Path_417-1871" data-name="Path 417"
                                                    d="M573.183,333.569,451.237,364.959a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l123.38-31.76C576.377,329.564,575.513,332.969,573.183,333.569Z"
                                                    transform="translate(-304.45 -129.98)" fill="#f2f2f2" />
                                                <path id="Path_418-1872" data-name="Path 418"
                                                    d="M576.624,346.939,454.679,378.329a2.862,2.862,0,0,1-3.467-1.8,2.757,2.757,0,0,1,1.942-3.5l123.38-31.76C579.819,342.934,578.954,346.339,576.624,346.939Z"
                                                    transform="translate(-304.285 -129.339)" fill="#f2f2f2" />
                                                <path id="Path_395-1873" data-name="Path 395"
                                                    d="M448.363,470.511a2.111,2.111,0,0,1-1.335-.092l-.026-.011-5.545-2.351a2.126,2.126,0,1,1,1.664-3.913l3.593,1.528,4.708-11.076a2.125,2.125,0,0,1,2.787-1.124h0l-.028.072.029-.073a2.127,2.127,0,0,1,1.124,2.788l-5.539,13.023a2.126,2.126,0,0,1-1.431,1.224Z"
                                                    transform="translate(-304.809 -123.966)" fill="#fff" />
                                            </g>
                                            <g id="Group_199" data-name="Group 199"
                                                transform="translate(673.007 225.872) rotate(-8)">
                                                <g id="Group_198" data-name="Group 198"
                                                    transform="translate(125.896 0) rotate(19)">
                                                    <path id="Path_3127-1874" data-name="Path 3127"
                                                        d="M304.956,386.7H34.583A34.622,34.622,0,0,1,0,352.114V34.583A34.622,34.622,0,0,1,34.583,0H286.121l53.418,42.577V352.114A34.622,34.622,0,0,1,304.956,386.7Z"
                                                        transform="translate(0 0)" fill="#e6e6e6" />
                                                    <path id="Path_3128-1875" data-name="Path 3128"
                                                        d="M24.627,0A24.655,24.655,0,0,0,0,24.627V342.158a24.655,24.655,0,0,0,24.627,24.627H295a24.655,24.655,0,0,0,24.627-24.627V37.418L272.683,0Z"
                                                        transform="translate(9.956 9.956)" fill="#fff" />
                                                    <path id="Path_3129-1876" data-name="Path 3129"
                                                        d="M128.856,11.518H5.759A5.759,5.759,0,0,1,5.759,0h123.1a5.759,5.759,0,0,1,0,11.518Z"
                                                        transform="translate(123.512 90.767)" fill="#419EBD" />
                                                    <path id="Path_3130-1877" data-name="Path 3130"
                                                        d="M152.612,11.518H5.759A5.759,5.759,0,0,1,5.759,0H152.612a5.759,5.759,0,1,1,0,11.518Z"
                                                        transform="translate(123.512 110.204)" fill="#419EBD" />
                                                    <path id="Path_3131-1878" data-name="Path 3131"
                                                        d="M128.852,0H5.758a5.758,5.758,0,1,0,0,11.517H128.852a5.759,5.759,0,0,0,0-11.517Z"
                                                        transform="translate(123.517 177.868)" fill="#ccc" />
                                                    <path id="Path_3132-1879" data-name="Path 3132"
                                                        d="M152.609,0H5.758a5.759,5.759,0,1,0,0,11.517h146.85a5.759,5.759,0,1,0,0-11.517Z"
                                                        transform="translate(123.517 197.307)" fill="#ccc" />
                                                    <path id="Path_3133-1880" data-name="Path 3133"
                                                        d="M128.856,11.518H5.759A5.759,5.759,0,0,1,5.759,0h123.1a5.759,5.759,0,0,1,0,11.518Z"
                                                        transform="translate(123.512 264.975)" fill="#ccc" />
                                                    <path id="Path_3134-1881" data-name="Path 3134"
                                                        d="M152.612,11.518H5.759A5.759,5.759,0,0,1,5.759,0H152.612a5.759,5.759,0,1,1,0,11.518Z"
                                                        transform="translate(123.512 284.411)" fill="#ccc" />
                                                    <circle id="Ellipse_44-2" data-name="Ellipse 44" cx="20.355"
                                                        cy="20.355" r="20.355" transform="translate(57.655 85.89)"
                                                        fill="#419EBD" />
                                                    <path id="Path_395-2-1882" data-name="Path 395"
                                                        d="M6.909,15.481a2.111,2.111,0,0,1-1.27-.422l-.023-.017L.832,11.382A2.126,2.126,0,0,1,3.419,8.008l3.1,2.376L13.839.832A2.125,2.125,0,0,1,16.819.439h0L16.774.5l.047-.063a2.127,2.127,0,0,1,.393,2.98L8.6,14.649a2.126,2.126,0,0,1-1.691.829Z"
                                                        transform="translate(69.085 98.528)" fill="#fff" />
                                                    <path id="Path_3135-1883" data-name="Path 3135"
                                                        d="M40.707,20.359A20.354,20.354,0,0,1,20.356,40.721a4.372,4.372,0,0,1-.524-.021A20.353,20.353,0,1,1,40.707,20.359Z"
                                                        transform="translate(59.75 172.987)" fill="#419EBD" />
                                                    <circle id="Ellipse_44-3" data-name="Ellipse 44" cx="20.355"
                                                        cy="20.355" r="20.355" transform="translate(57.655 260.097)"
                                                        fill="#419EBD" />
                                                    <path id="Path_3136-1884" data-name="Path 3136"
                                                        d="M53.362,43.143H11.518A11.518,11.518,0,0,1,0,31.625V.72A.72.72,0,0,1,1.167.156l52.642,41.7a.72.72,0,0,1-.447,1.284Z"
                                                        transform="translate(285.137 0.805)" fill="#ccc" />
                                                </g>
                                            </g>
                                            <path id="Path_3140-1885" data-name="Path 3140"
                                                d="M754.518,518.049a9.158,9.158,0,0,1-12.587,3.05L635.078,455.923a9.158,9.158,0,0,1,9.538-15.637l106.852,65.176a9.158,9.158,0,0,1,3.049,12.587Z"
                                                transform="translate(123.58 101.359)" fill="#3f3d56" />
                                            <path id="Path_3141-1886" data-name="Path 3141"
                                                d="M688.648,486.5a73.265,73.265,0,1,1-24.4-100.7A73.265,73.265,0,0,1,688.648,486.5ZM579.19,419.73a54.949,54.949,0,1,0,75.524-18.3,54.949,54.949,0,0,0-75.524,18.3Z"
                                                transform="translate(82.597 67.737)" fill="#3f3d56" />
                                            <circle id="Ellipse_44-4" data-name="Ellipse 44" cx="57.007"
                                                cy="57.007" r="57.007"
                                                transform="translate(672.542 442.858) rotate(19)" fill="#419EBD" />
                                        </g>
                                    </svg>

                                    <h5 class="mt-3">Consultar</h5>
                                    <p class="text-muted">Buscar registros</p>

                                </a>

                            </div>
                            <div class="col-md-3 d-flex align-items-center justify-content-center">

                                <a class="panel-opcion panel-opcion-btn text-center p-4"
                                    data-target="#contenido-2-exportar">

                                    <svg class="icon-panel" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 753.87469 703.82827" xmlns:xlink="http://www.w3.org/1999/xlink"
                                        role="img" artist="Katerina Limpitsouni" source="https://undraw.co/">
                                        <path
                                            d="M578.47505,103.95771l-23.06843,12.58664L271.19846,271.61447,248.13,284.2011a48.1793,48.1793,0,0,0-19.1955,65.29607L440.57765,737.39072a48.17922,48.17922,0,0,0,65.296,19.19561l.05958-.03251L836.15907,576.37545l.05958-.03251a48.17924,48.17924,0,0,0,19.19553-65.296L643.77106,123.15338A48.17929,48.17929,0,0,0,578.47505,103.95771Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#f2f2f2" />
                                        <path
                                            d="M585.11115,116.11916l-27.323,14.908L282.08828,281.455,254.7657,296.36278a34.30947,34.30947,0,0,0-13.66965,46.4988L452.73916,730.75513a34.30947,34.30947,0,0,0,46.4988,13.66952l.05958-.0325L829.5234,564.21377l.06-.03274a34.30935,34.30935,0,0,0,13.66926-46.49851L631.60954,129.789A34.30936,34.30936,0,0,0,585.11115,116.11916Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#fff" />
                                        <path
                                            d="M589.66653,236.52147,466.505,303.72109a8.01411,8.01411,0,1,1-7.677-14.07012l123.16157-67.19962a8.01411,8.01411,0,1,1,7.677,14.07012Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#f2f2f2" />
                                        <path
                                            d="M631.641,244.43106,479.45984,327.46442a8.01411,8.01411,0,0,1-7.677-14.07012l152.18119-83.03336a8.01411,8.01411,0,1,1,7.677,14.07012Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#f2f2f2" />
                                        <path
                                            d="M415.87223,275.74837l-113.5479,61.95419a3.84082,3.84082,0,0,0-1.53151,5.21006L349.14436,431.53a3.84075,3.84075,0,0,0,5.21,1.5317l113.5479-61.95419a3.84075,3.84075,0,0,0,1.53153-5.21l-48.35154-88.61735A3.84081,3.84081,0,0,0,415.87223,275.74837Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#f2f2f2" />
                                        <path
                                            d="M650.7763,348.96263,483.723,440.11051a8.01411,8.01411,0,1,1-7.677-14.07012l167.05327-91.14788a8.01411,8.01411,0,1,1,7.677,14.07012Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#f2f2f2" />
                                        <path
                                            d="M692.7508,356.87223,496.67791,463.85384a8.01411,8.01411,0,0,1-7.677-14.07012L685.07384,342.80211a8.01411,8.01411,0,1,1,7.677,14.07012Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#f2f2f2" />
                                        <circle cx="197.03853" cy="382.67177" r="34" fill="#f2f2f2" />
                                        <path
                                            d="M928.81234,263.78816H552.494a48.17927,48.17927,0,0,0-48.125,48.12512V753.78907a48.17922,48.17922,0,0,0,48.125,48.12506H928.81234a48.17922,48.17922,0,0,0,48.125-48.12506V311.91328A48.17927,48.17927,0,0,0,928.81234,263.78816Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <path
                                            d="M928.81283,277.64235H552.494a34.30947,34.30947,0,0,0-34.271,34.27093V753.78907A34.30947,34.30947,0,0,0,552.494,788.06H928.81283a34.30936,34.30936,0,0,0,34.27051-34.27088V311.91328A34.30937,34.30937,0,0,0,928.81283,277.64235Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#fff" />
                                        <path
                                            d="M875.14319,385.51745H734.84151a8.01411,8.01411,0,0,1,0-16.02823H875.14319a8.01412,8.01412,0,1,1,0,16.02823Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#419EBD" />
                                        <path
                                            d="M908.20141,412.56508H734.84151a8.01411,8.01411,0,1,1,0-16.02822h173.3599a8.01411,8.01411,0,0,1,0,16.02822Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#419EBD" />
                                        <path
                                            d="M703.79234,336.71073H574.44224a3.8408,3.8408,0,0,0-3.83984,3.84v100.95a3.84075,3.84075,0,0,0,3.83984,3.84h129.3501a3.84076,3.84076,0,0,0,3.83984-3.84v-100.95A3.84081,3.84081,0,0,0,703.79234,336.71073Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <path
                                            d="M609.92406,398.70111a34.087,34.087,0,0,1-8.804,23.076c-5.656,6.20712-14.07618,10.3236-22.57327,8.62043-7.82416-1.56829-14.18219-8.4067-13.389-16.6795a12.356,12.356,0,0,1,15.2668-11.09515c7.43265,1.92885,10.39415,12.64095,4.20051,17.669-1.4862,1.2065-3.62136-.90359-2.12132-2.12132,4.0944-3.32385,2.8295-10.5954-2.11244-12.419-5.75371-2.12311-11.84978,2.44324-12.26355,8.32554-.49057,6.97428,4.85221,12.22646,11.40422,13.463,7.08789,1.3377,14.11532-2.29,18.91808-7.29718a30.95507,30.95507,0,0,0,8.474-21.54183,1.5009,1.5009,0,0,1,3,0Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#2f2e41" />
                                        <circle cx="416.15529" cy="266.1673" r="53.51916" fill="#419EBD" />
                                        <path
                                            d="M636.47981,387.08916l-.05566-2c3.7207-.10352,7.001-.33692,9.46582-2.1377a6.14794,6.14794,0,0,0,2.38134-4.52832,3.51432,3.51432,0,0,0-1.15283-2.89453c-1.63623-1.38184-4.269-.93457-6.188-.05469l-1.65478.75879,3.17334-23.19043,1.98144.27149-2.69922,19.72656c2.60743-.7666,5.02344-.43652,6.67823.96094a5.471,5.471,0,0,1,1.86035,4.49218,8.13264,8.13264,0,0,1-3.2002,6.07325C643.90266,386.88115,639.78694,386.99638,636.47981,387.08916Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#2f2e41" />
                                        <rect x="431.16715" y="256.92907" width="10.77148" height="2"
                                            fill="#2f2e41" />
                                        <rect x="397.16715" y="256.92907" width="10.77148" height="2"
                                            fill="#2f2e41" />
                                        <path
                                            d="M609.57212,445.34074a53.00636,53.00636,0,0,1,12.89014-5.93,8.56789,8.56789,0,0,1,.02-4.71,9.42609,9.42609,0,0,1,9.12988-6.63h13.04a9.45955,9.45955,0,0,1,9.15039,6.64,8.532,8.532,0,0,1,.01953,4.7,53.16732,53.16732,0,0,1,12.89014,5.93Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#2f2e41" />
                                        <path
                                            d="M700.52232,344.39072a11.57143,11.57143,0,0,0-3.52979-2.87,8.36739,8.36739,0,0,0-3.8501-.95,8.77158,8.77158,0,0,0-5.10986,1.72c-4.07031,2.88-6.89014,9.09-6.89014,16.28,0,9.02,4.43995,16.5,10.21,17.80005a8.25321,8.25321,0,0,0,1.79.2c6.60987,0,12-8.07,12-18C705.14243,352.81077,703.33238,347.68076,700.52232,344.39072Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#3f3d56" />
                                        <path
                                            d="M590.6024,341.86076h-.00977a8.57836,8.57836,0,0,0-4.4502-1.29,8.36738,8.36738,0,0,0-3.85009.95,11.57143,11.57143,0,0,0-3.52979,2.87l-.01025.01c-2.79981,3.29-4.60987,8.42-4.60987,14.17,0,7.76,3.27979,14.38,7.87989,16.91a8.54175,8.54175,0,0,0,4.12011,1.09,7.72431,7.72431,0,0,0,.96-.06h.00976c6.16016-.74,11.03027-8.5,11.03027-17.94C598.14243,351.01072,595.01255,344.52073,590.6024,341.86076Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#3f3d56" />
                                        <path
                                            d="M582.77242,373.76a1.50127,1.50127,0,0,0,1.42151-1.98,58.49864,58.49864,0,1,1,112.68726-6.5747,1.50006,1.50006,0,0,0,2.93554.61914A61.50091,61.50091,0,1,0,581.35116,372.739,1.50077,1.50077,0,0,0,582.77242,373.76Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#3f3d56" />
                                        <path
                                            d="M666.10324,329.57746c2.11924,2.89278,1.07447,6.79121-1.15837,9.28528-2.90548,3.24541-7.53877,3.45016-11.5618,2.8478-4.51431-.67591-9.3026-2.7909-13.87293-1.3656-3.89537,1.2148-6.67418,4.74793-10.7211,5.63537-3.589.787-7.88081-.25477-9.139-4.08016-.60459-1.83823,2.29142-2.6261,2.89284-.79751.81395,2.47478,4.32865,2.42543,6.34145,1.74012,3.22689-1.09867,5.71374-3.77105,8.8854-5.04749,3.73933-1.50491,7.79621-.82549,11.60323.03181,3.58831.808,7.718,2.006,11.29267.49665,2.64515-1.1169,4.74985-4.635,2.84717-7.23211-1.14219-1.5591,1.45985-3.05738,2.59042-1.51416Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#2f2e41" />
                                        <path
                                            d="M874.932,513.49157H684.63034a8.01411,8.01411,0,1,1,0-16.02823H874.932a8.01412,8.01412,0,0,1,0,16.02823Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <path
                                            d="M907.99023,540.5392H684.63034a8.01412,8.01412,0,1,1,0-16.02823H907.99023a8.01412,8.01412,0,1,1,0,16.02823Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <path
                                            d="M874.932,610.705H684.63034a8.01411,8.01411,0,1,1,0-16.02822H874.932a8.01411,8.01411,0,1,1,0,16.02822Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <path
                                            d="M907.99023,637.75267H684.63034a8.01411,8.01411,0,1,1,0-16.02823H907.99023a8.01411,8.01411,0,1,1,0,16.02823Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <circle cx="386.2497" cy="420.61448" r="34" fill="#e6e6e6" />
                                        <circle cx="386.2497" cy="518.61448" r="34" fill="#e6e6e6" />
                                        <path
                                            d="M874.932,708.705H684.63034a8.01411,8.01411,0,1,1,0-16.02822H874.932a8.01411,8.01411,0,1,1,0,16.02822Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <path
                                            d="M907.99023,735.75267H684.63034a8.01411,8.01411,0,1,1,0-16.02823H907.99023a8.01411,8.01411,0,1,1,0,16.02823Z"
                                            transform="translate(-223.06266 -98.08587)" fill="#e6e6e6" />
                                        <circle cx="386.2497" cy="616.61448" r="34" fill="#e6e6e6" />
                                    </svg>

                                    <h5 class="mt-3">Exportar</h5>
                                    <p class="text-muted">Descargar registros</p>

                                </a>

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid panel-seccion" id="contenido-2-exportar" style="position: relative;" hidden>
        <div class="row mt-4 ml-1 mr-1">
            <div class="col-12">
                <div class="box_block">
                    <div class="card-body" style="background-color: #fff">

                        <div class="table-responsive mt-2 ">
                            <div class="col-12 d-flex justify-content-md-end justify-content-start estilo-info">
                                EXPORTAR
                            </div>

                        </div>


                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid panel-seccion" id="contenido-2-consultar" style="position: relative;" hidden>
        <div class="row mt-4 ml-1 mr-1">
            <div class="col-12">
                <div class="box_block">
                    <div class="card-body" style="background-color: #fff">

                        <div class="row mb-2 d-flex align-items-center">
                            <!-- Botón a la izquierda -->
                            <div class="col-12 col-md-6 mb-md-0 d-flex justify-content-start">
                                <a href="<?php echo e(route('docente.create')); ?>" id="nuevoRegistroBtns"
                                    class="btn w-100 mt-3 btn-color d-flex justify-content-between align-items-center"
                                    type="button">
                                    <i class="ti ti-user-plus me-2 mx-2" style="font-size:24px"></i> ORDENAR POR
                                </a>
                            </div>

                            <!-- Buscador a la derecha -->
                            <div class="col-12 col-md-6 d-flex justify-content-md-end justify-content-start estilo-info">
                                <form id="formBuscar" method="GET" class="w-100" style="max-width: 100%;">
                                    <div class="input-group">
                                        <input id="inputBuscar" name="buscarpor" class="form-control mt-3"
                                            type="search" placeholder="Ingrese N.° de DNI o Apellidos"
                                            aria-label="Search" value="<?php echo e($buscarpor); ?>" autocomplete="off"
                                            style="border-color: #F59617; font-size:16px !important;">
                                        <button class="btn btn-color mt-3" type="submit"
                                            style="border-top-left-radius: 0 !important; border-bottom-left-radius: 0 !important; ">
                                            <i class="ti ti-search" style="font-size:24px"></i>
                                        </button>

                                    </div>
                                </form>
                            </div>
                        </div>

                        <div class="row form-bordered align-items-center"></div>

                        <div id="tabla-wrapper" style="position: relative; min-height: 200px;">

                            <?php echo $__env->make('ccomponentes.loader', ['id' => 'loaderTabla'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

                            <div id="tabla-docentes" style="position: relative;">
                                <?php echo $__env->make('ceinformacion.docentes.docente', [
                                    'docente' => $docente,
                                ], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                            </div>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>

<?php $__env->stopSection(); ?>

<?php $__env->startSection('scripts'); ?>

    <?php if(session('modal_success_docente')): ?>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    icon: 'success',
                    title: '¡Éxito!',
                    text: '<?php echo e(session('modal_success_docente')); ?>',
                    confirmButtonColor: '#28a745',
                    timer: 1200,
                    showConfirmButton: false
                });
            });
        </script>
    <?php endif; ?>

    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const btnRegistrar = document.getElementById('btnRegistrar');
            const botones = document.querySelectorAll('.panel-opcion-btn');
            const secciones = document.querySelectorAll('.panel-seccion');
            const loader1 = document.getElementById('loaderPrincipalRep');
            const contenido1 = document.getElementById('contenido-principal');

            btnRegistrar.addEventListener('click', () => {
                try {
                    if (loader1) loader1.style.display = 'flex';
                    if (contenido1) contenido1.style.opacity = '1';
                } finally {
                    // 🔥 SOLO se oculta cuando TODO terminó

                }
            });

            botones.forEach(btn => {

                btn.addEventListener('click', () => {

                    secciones.forEach(sec => {
                        sec.hidden = true;
                        sec.classList.remove('visible');
                    });

                    const target = document.querySelector(btn.dataset.target);

                    if (target) {
                        target.hidden = false;

                        setTimeout(() => {
                            target.classList.add('visible');
                        }, 10);
                    }

                    botones.forEach(b => {
                        b.classList.remove('panel-opcion-active');
                        b.classList.add('panel-opcion-inactive');
                    });

                    btn.classList.remove('panel-opcion-inactive');
                    btn.classList.add('panel-opcion-active');

                });

            });

        });
    </script>

    <script>
        document.addEventListener('DOMContentLoaded', () => {

            const inputBuscar = document.getElementById('inputBuscar');
            const formBuscar = document.getElementById('formBuscar');
            const tabla = document.getElementById('tabla-docentes');
            const loader = document.getElementById('loaderTabla');

            let debounceTimer = null;

            function mostrarLoader() {
                loader.style.display = 'flex';
                tabla.style.opacity = '0.5'; // feedback visual extra
            }

            function ocultarLoader() {
                loader.style.display = 'none';
                tabla.style.opacity = '1';
            }

            async function cargarTabla(buscar = '', page = 1) {
                // 🔥 SIEMPRE se ejecuta
                mostrarLoader();

                const url =
                    `<?php echo e(route('docente.index')); ?>?buscarpor=${encodeURIComponent(buscar)}&page=${page}`;

                try {
                    const response = await fetch(url, {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest'
                        }
                    });

                    if (!response.ok) throw new Error('Error en la respuesta');

                    const html = await response.text();
                    tabla.innerHTML = html;

                } catch (error) {
                    console.error(error);
                    alert('Error al cargar los datos');
                } finally {
                    // 🔥 SOLO se oculta cuando TODO terminó
                    ocultarLoader();
                }
            }

            // 🔎 Buscar (debounce)
            inputBuscar.addEventListener('input', () => {
                clearTimeout(debounceTimer);
                debounceTimer = setTimeout(() => {
                    cargarTabla(inputBuscar.value.trim(), 1);
                }, 400);
            });

            // 📝 Submit
            formBuscar.addEventListener('submit', (e) => {
                e.preventDefault();
                cargarTabla(inputBuscar.value.trim(), 1);
            });

            // 📄 Paginación
            document.addEventListener('click', (e) => {
                const link = e.target.closest('.pagination a');
                if (!link) return;

                e.preventDefault();

                const url = new URL(link.href);
                const page = url.searchParams.get('page');

                cargarTabla(inputBuscar.value.trim(), page);
            });

        });
    </script>

<?php $__env->stopSection(); ?>

<?php echo $__env->make('cplantilla.bprincipal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ronaldoroblesromero/Herd/edukaSchool/resources/views/ceinformacion/docentes/registrar.blade.php ENDPATH**/ ?>