...
function saveReportToHistory(params, totalRegistros) {
    ...
    console.log('Tipo reporte from params:', params.tipo_reporte);
    ...
    console.log('Sending data for preview report:', saveData);
    ...
    .then(response => {
        console.log('Preview report response status:', response.status);
        return response.json();
    })
    .then(data => {
        console.log('Preview report response data:', data);
        if (data.success) {
            console.log('Preview report saved successfully:', data.reporte_id);

            // Now save Excel report automatically for ALL report types (not just general)
            if (params.tipo_reporte !== 'general') {
                const excelSaveData = {
                    ...saveData,
                    formato: 'excel' // Indicate this is for Excel
                };

                console.log('Sending data for Excel report:', excelSaveData);

                return fetch('/asistencia/api/guardar-reporte-excel', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken || ''
                    },
                    body: JSON.stringify(excelSaveData)
                });
            }
        } else {
            throw new Error('Error saving preview report: ' + data.message);
        }
    })
    .then(excelResponse => {
        if (excelResponse) {
            console.log('Excel report response status:', excelResponse.status);
            return excelResponse.json();
        }
    })
    .then(excelData => {
        if (excelData) {
            console.log('Excel report response data:', excelData);
            if (excelData.success) {
                console.log('Excel report saved successfully:', excelData.reporte_id);
            } else {
                console.error('Error saving Excel report:', excelData.message);
            }
        }
    ...
    .catch(error => {
        ...
        alert('Error al guardar los reportes: ' + error.message);
...
function exportarReporte(event) {
    ...
    // For both PDF and Excel, open in new tab to ensure proper download handling
