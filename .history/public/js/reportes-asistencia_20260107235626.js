...
function saveReportToHistory(params, totalRegistros) {
    ...
    // Only save Excel reports with applied filters, not preview reports
    const saveData = {
    ...
    console.log('Sending data for Excel report with filters:', saveData);

    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
    console.log('CSRF Token:', csrfToken ? 'Present' : 'Missing');

    // Save Excel report with applied filters
    fetch('/asistencia/api/guardar-reporte-excel', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        console.log('Excel report response data:', data);
        if (data.success) {
            console.log('Excel report with filters saved successfully:', data.reporte_id);
        } else {
            console.error('Error saving Excel report:', data.message);
        }
    ...
    .catch(error => {
        console.error('Error saving Excel report:', error);
        alert('Error al guardar el reporte Excel: ' + error.message);
