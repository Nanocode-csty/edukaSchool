    ...
    if (data.cursos?.length) {
        ...
        if (select) {
            ...
            data.cursos.forEach(c => {
                ...
                option.setAttribute('data-nivel', c.grado?.nivel_id || '');
                select.appendChild(option);
            });
    ...
    if (data.estudiantes?.length) {
        ...
        if (select) {
            ...
            data.estudiantes.forEach(e => {
                ...
                const option = document.createElement('option');
                option.value = e.estudiante_id;
                option.textContent = nombre;
                option.setAttribute('data-curso', e.matricula?.curso_id || '');
                option.setAttribute('data-nivel', e.matricula?.nivel_id || '');
                select.appendChild(option);
            });
