...
class CompleteDatabaseSeeder extends Seeder
{
    public function run()
    {
        ...
        $seccion = InfSeccion::find(1); // 'A' seccion
            ...
            echo "Grado created with ID: {$grado->id_grado}\n";
        } else {
            echo "Grado already exists with ID: {$grado->id_grado}\n";
        }

        // Create seccion
        $seccion = InfSeccion::where('nombre', 'A')->first();
        if (!$seccion) {
            echo "Creating seccion...\n";
            $seccion = InfSeccion::create([
                'nombre' => 'A',
                'descripcion' => 'Sección A',
                'capacidad_maxima' => 30
            ]);
            echo "Seccion created with ID: {$seccion->id_seccion}\n";
        } else {
            echo "Seccion already exists with ID: {$seccion->id_seccion}\n";
        }
