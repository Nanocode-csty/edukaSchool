...
class PresentationSeeder extends Seeder
{
    ...
    private function configurarAnioAcademico()
    {
        ...
        $fechaFin = Carbon::create($anioActual + 1, 1, 20); // Fin en enero del siguiente año
    ...
    private function crearEstudianteParaCurso($curso, $numero)
    {
        ...
        $persona = Persona::firstOrCreate(
            ...
            [
                ...
                'apellidos' => 'del ' . $curso->grado->nombre . ' ' . $curso->seccion->nombre,
    ...
    /**
     * Crea sesiones de clase para todo el año académico
     */
    private function crearSesionesClase()
    {
        $this->command->info('Creando sesiones de clase para todo el año académico...');
        ...
        // Crear sesiones para todo el año académico (desde marzo hasta enero)
        $fechaInicio = Carbon::parse($anioLectivo->fecha_inicio);
        $fechaFin = Carbon::parse($anioLectivo->fecha_fin);
    ...
    /**
     * Genera datos de asistencia realistas para todo el año académico
     */
    private function generarDatosAsistencia()
    {
        $this->command->info('Generando datos de asistencia realistas para todo el año académico...');
        ...
        // Generar asistencia para todo el año académico
        $fechaInicio = Carbon::parse($anioLectivo->fecha_inicio);
        $fechaFin = Carbon::parse($anioLectivo->fecha_fin);
    ...
    private function crearDatosRepresentantes()
    {
        ...
        foreach ($estudiantes as $estudiante) {
            ...
            $dniRepresentante = 'REP' . str_pad($representantesCreados + 1, 3, '0', STR_PAD_LEFT);
            ...
            Usuario::firstOrCreate(
                ['persona_id' => $personaRepresentante->id_persona],
                [
                    'username' => 'representante' . ($representantesCreados + 1),
                    'password_hash' => Hash::make('password'),
        ...
        if ($rolRepresentante) {
            $this->command->info("✓ Rol representante encontrado: {$rolRepresentante->id}");
            $representantes = InfRepresentante::with('persona')->get();
            foreach ($representantes as $rep) {
                if ($rep->persona) {
                    DB::table('persona_roles')->updateOrInsert(
                        [
                            'id_persona' => $rep->persona->id_persona,
                            'id_rol' => $rolRepresentante->id_rol
                        ],
            ...
            $this->command->info("⚠ Rol representante no encontrado, omitiendo asignación de roles");
        }
    }
}
