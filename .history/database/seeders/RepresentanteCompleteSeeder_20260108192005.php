...
class RepresentanteCompleteSeeder extends Seeder
{
    ...
    {
        $this->command->info('Creando datos de asistencia por asignatura...');
        ...
        // Obtener matrículas de los estudiantes
        $matriculas = Matricula::whereIn('estudiante_id', $estudiantesIds)->get();

        if ($matriculas->isEmpty()) {
            $this->command->error('No hay matrículas para los estudiantes');
            return;
        }

        // Obtener asignaturas disponibles
        $cursoAsignaturas = CursoAsignatura::with('asignatura')->get();
        ...
        // Crear asistencia para los últimos 30 días por asignatura
        for ($i = 0; $i < 30; $i++) {
            $fecha = now()->subDays($i);
            ...
            if ($fecha->dayOfWeek >= 1 && $fecha->dayOfWeek <= 5) {
                foreach ($matriculas as $matricula) {
                    foreach ($cursoAsignaturas as $cursoAsignatura) {
                        // Verificar si ya existe asistencia para esta fecha, matrícula y asignatura
                        ...
                        if (!$asistenciaExistente) {
                            ...
                            AsistenciaAsignatura::create([
                                ...
                                'justificacion' => $esPresente ? null : 'Ausencia registrada por seeder',
                                'usuario_registro' => 1
                            ]);
                        }
                    }
        ...
        $this->command->info('Datos de asistencia por asignatura creados para los últimos 30 días');
    }
