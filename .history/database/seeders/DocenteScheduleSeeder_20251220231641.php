...
class DocenteScheduleSeeder extends Seeder
{
    ...
    private function crearCursosAsignaturaDocente()
    {
        ...
        $cursosAsignatura = CursoAsignatura::where('profesor_id', $this->docente->profesor_id)->get();

        if ($cursosAsignatura->isEmpty()) {
            $this->command->info('No hay cursos asignados al docente. Creando asignaciones de prueba...');
            ...
            foreach ($cursos->take(4) as $index => $curso) {
                ...
                try {
                    $cursoAsignatura = CursoAsignatura::create([
                    ...
                    $cursosAsignatura->push($cursoAsignatura);
                } catch (\Exception $e) {
                    $this->command->warn("No se pudo crear asignación para {$asignatura->nombre} en {$diaSemana}: " . $e->getMessage());
                }
            }
        }

        $this->command->info('Cursos asignados al docente: ' . $cursosAsignatura->count());
