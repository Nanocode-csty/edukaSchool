...
class CompleteDatabaseSeeder extends Seeder
{
    public function run()
    {
        ...
        // Use existing grado and seccion
        $grado = InfGrado::find(15); // 'Primero' grado
        $seccion = InfSeccion::find(1); // 'A' seccion

        if (!$grado) {
            echo "ERROR: Grado with ID 15 not found!\n";
            return;
        }
        if (!$seccion) {
            echo "ERROR: Seccion with ID 1 not found!\n";
            return;
        }

        echo "Using existing grado 'Primero' with ID: {$grado->id_grado}\n";
        echo "Using existing seccion 'A' with ID: {$seccion->id_seccion}\n";
        ...
        // Step 3: Update existing matriculas for each student
        echo "Step 3: Updating matriculas...\n";
        ...
        foreach ($estudiantes as $estudiante) {
            ...
            if ($matricula) {
                ...
                echo "✓ Updated matricula for {$estudiante->persona->nombres} {$estudiante->persona->apellidos}: ";
                ...
                echo "✓ Created matricula for {$estudiante->persona->nombres} {$estudiante->persona->apellidos}: ";
                echo "Grado {$matricula->idGrado}, Sección {$matricula->idSeccion}\n";
            }
        }
