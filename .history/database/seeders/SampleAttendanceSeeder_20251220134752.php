...
class SampleAttendanceSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Obtener algunas matrículas para crear datos de ejemplo (reducido para mejor rendimiento)
        $matriculas = Matricula::take(10)->get();

        // Obtener algunos cursos asignatura
        $cursosAsignatura = CursoAsignatura::take(3)->get();

        // Crear registros de asistencia para el último mes (menos registros para mejor rendimiento)
    ...
    {
        // Probabilidades realistas basadas en factores de asistencia:
        // 85% Asistió (A, factor 1.00), 10% Falta (F, factor 0.00),
        // 3% Tardanza (T, factor 0.50), 2% Justificada (J, factor 0.75)
        $rand = rand(1, 100);

        $codigo = 'A'; // default = Asistió
        if ($rand <= 85) {
            $codigo = 'A'; // Asistió - factor 1.00
        } elseif ($rand <= 95) {
            $codigo = 'F'; // Falta - factor 0.00
        } elseif ($rand <= 98) {
            $codigo = 'T'; // Tardanza - factor 0.50
        } else {
            $codigo = 'J'; // Justificada - factor 0.75
        }
