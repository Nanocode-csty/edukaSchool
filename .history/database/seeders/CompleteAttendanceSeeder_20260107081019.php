...
class CompleteAttendanceSeeder extends Seeder
{
    public function run()
    {
        ...
        $this->command->info("Tipos de asistencia disponibles: " . implode(', ', $tiposAsistencia->keys()->toArray()));
        ...
        foreach ($sesiones as $sesion) {
            ...
            foreach ($estudiantes as $matricula) {
                ...
                if (!$tipoAsistencia) {
                    ...
                    $this->command->error("Tipos disponibles: " . implode(', ', $tiposAsistencia->keys()->toArray()));
                    continue;
                }

                $this->command->info("Tipo seleccionado: {$tipoAsistencia->codigo} (ID: {$tipoAsistencia->tipo_asistencia_id})");
    ...
    private function generarAsistenciaRealista($tiposAsistencia)
    {
        ...
        // Si no se encontró ninguno, devolver el primero disponible
        return $tiposAsistencia->first();
    }
