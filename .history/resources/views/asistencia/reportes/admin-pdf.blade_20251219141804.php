            ...
            <div class="stats-grid">
                <div class="stats-row">
                    <div class="stats-cell">
                        <strong>{{ $asistencias->count() }}</strong><br>
                        <small>Total de Registros</small>
                    </div>
                    <div class="stats-cell">
                        <strong>{{ $asistencias->filter(function($asistencia) { return $asistencia->tipoAsistencia->codigo === 'P'; })->count() }}</strong><br>
                        <small>Presentes</small>
                    </div>
                    <div class="stats-cell">
                        <strong>{{ $asistencias->filter(function($asistencia) { return $asistencia->tipoAsistencia->codigo === 'A'; })->count() }}</strong><br>
                        <small>Ausentes</small>
                    </div>
                    <div class="stats-cell">
                        <strong>{{ $asistencias->count() > 0 ? round(($asistencias->filter(function($asistencia) { return $asistencia->tipoAsistencia->codigo === 'P'; })->count() / $asistencias->count()) * 100, 1) : 0 }}%</strong><br>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            {{ $asistencia->tipoAsistencia->codigo }}
                        @endif
                    </td>
                </tr>
                @endforeach
