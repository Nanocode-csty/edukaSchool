...
<body>
    <div class="header">
        ...
        <p><strong>Institución Educativa EDUKA</strong></p>
    ...
    <div class="periodo-info">
        ...
        <br>
        <strong>Tipo de Reporte:</strong> {{ ucfirst(str_replace('_', ' ', $request->tipo_reporte ?? 'general')) }}
        <br>
        <strong>Generado por:</strong> Administrador del Sistema
    </div>

    @if(isset($filtrosAplicados) && !empty($filtrosAplicados))
    <div class="info-section">
        <h3>Filtros Aplicados</h3>
        <div style="display: table; width: 100%;">
            @foreach($filtrosAplicados as $filtro => $valor)
            <div style="display: table-row;">
                <div style="display: table-cell; padding: 4px 8px; font-weight: bold; width: 150px;">
                    {{ $filtro }}:
                </div>
    ...
    @if($asistencias->isNotEmpty())
        @php
            ...
            foreach($asistencias as $asistencia) {
                ...
                switch($codigo) {
                    case 'A': // A = Asistió (Present)
                        $totalPresentes++;
                        break;
                    case 'F': // F = Falta (Absent)
                        $totalAusentes++;
                        break;
                    case 'T': // T = Tarde (Late)
                        $totalTardanzas++;
                        break;
                    case 'J': // J = Justificado (Justified)
                        $totalJustificados++;
                        break;
                }
            }

            $tasaAsistencia = $totalRegistros > 0 ? round((($totalPresentes + $totalJustificados) / $totalRegistros) * 100, 1) : 0;
        @endphp
        <div class="info-section">
        ...
        <h3>Detalle de Asistencias Diarias</h3>
        <table class="table">
            <thead>
                <tr>
                    <th>Fecha</th>
