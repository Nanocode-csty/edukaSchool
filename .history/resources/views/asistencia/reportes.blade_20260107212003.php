...
@php
    ...
    })->with(['grado', 'seccion'])->get()->map(function($curso) {
        $curso->nombre_completo = $curso->grado->nombre . ' ' . $curso->seccion->nombre;
        return $curso;
    })->unique('curso_id');
...
function handleReportTypeChange() {
    ...
    switch(tipo) {
        case 'por_curso':
            html = '<div class="form-group"><label>Curso</label><select class="form-control filter-select" id="curso_id"><option value="">Todos los cursos</option>';
            @foreach($cursos as $curso)
                html += '<option value="{{ $curso->curso_id }}">{{ $curso->nombre_completo }}</option>';
            @endforeach
            html += '</select></div>';
            break;
        case 'por_estudiante':
            html = '<div class="form-group"><label>Estudiante</label><select class="form-control filter-select" id="estudiante_id"><option value="">Todos los estudiantes</option>';
            @foreach($estudiantes as $estudiante)
                html += '<option value="{{ $estudiante->estudiante_id }}">{{ $estudiante->persona->nombres }} {{ $estudiante->persona->apellidos }}</option>';
            @endforeach
            html += '</select></div>';
            break;
        case 'por_docente':
            html = '<div class="form-group"><label>Docente</label><select class="form-control filter-select" id="docente_id"><option value="">Todos los docentes</option>';
            @foreach($docentes as $docente)
                html += '<option value="{{ $docente->profesor_id }}">{{ $docente->persona->nombres }} {{ $docente->persona->apellidos }}</option>';
            @endforeach
            html += '</select></div>';
            break;
        case 'comparativo':
