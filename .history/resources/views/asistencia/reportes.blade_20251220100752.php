        ...
        <div class="col-12">
            <div class="box_block">
                ...
                function configurarFiltrosCascada() {
                    ...
                    // Función para mostrar/ocultar opciones considerando TODAS las selecciones actuales
                    function filtrarOpcionesBidireccional() {
                        ...
                        // Filtrar cursos
                        filtrarSelect(cursoSelect, (option) => {
                            if (option.value === '') return true;

                            const dataNivel = option.getAttribute('data-nivel');

                            // Si hay nivel seleccionado, debe coincidir
                            if (nivelSeleccionado && dataNivel !== nivelSeleccionado) return false;

                            // Si hay estudiante seleccionado, el curso debe ser el del estudiante
                            if (estudianteSeleccionado) {
                                const estudianteOption = Array.from(estudianteSelect.options).find(opt => opt.value === estudianteSeleccionado);
                                if (estudianteOption) {
                                    const estudianteCurso = estudianteOption.getAttribute('data-curso');
                                    if (estudianteCurso && option.value !== estudianteCurso) return false;
                                }
                            }
                        ...
                        // Filtrar estudiantes
                        filtrarSelect(estudianteSelect, (option) => {
                            if (option.value === '') return true;

                            const dataCurso = option.getAttribute('data-curso');

                            // Si hay curso seleccionado, debe coincidir
                            if (cursoSeleccionado && dataCurso !== cursoSeleccionado) return false;

                            // Si hay nivel seleccionado, el curso del estudiante debe ser de ese nivel
                            if (nivelSeleccionado) {
                                const cursoOption = Array.from(cursoSelect.options).find(opt => opt.value === dataCurso);
                                if (cursoOption) {
                                    const cursoNivel = cursoOption.getAttribute('data-nivel');
                                    if (cursoNivel && cursoNivel !== nivelSeleccionado) return false;
                                }
                            }
                        ...
                        // Para docentes, por ahora mostramos todos (podríamos filtrar por cursos que enseñan)
                        filtrarSelect(docenteSelect, (option) => {
                            return option.value === '' || true; // Mostrar todos por ahora
                        });
                    }

                    // Función auxiliar para filtrar un select
                    function filtrarSelect(selectElement, filtroCallback) {
                        ...
                        options.forEach(option => {
                            if (option.value === '') {
                                ...
                                const mostrar = filtroCallback(option);
                    ...
                    [nivelSelect, cursoSelect, estudianteSelect, docenteSelect].forEach(select => {
                        select.addEventListener('change', function() {
                            // Aplicar filtrado bidireccional a todos los selects
                            filtrarOpcionesBidireccional();

                            // Reinicializar Select2 para los selects afectados
