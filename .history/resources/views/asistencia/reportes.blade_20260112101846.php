            ...
            <div class="box_block">
                ...
                <div class="collapse show" id="collapseReportesAsistencia">
                    <div class="card card-body rounded-0 border-0 pt-0 pb-2" style="background: transparent;">
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-12">
                                                <div class="custom-control custom-checkbox mb-3">
                                                    <input type="checkbox" class="custom-control-input" id="incluir_estadisticas">
                                                    <label class="custom-control-label fw-bold text-dark" for="incluir_estadisticas" style="cursor: pointer; font-size: 14px;">
                                                        <i class="fas fa-chart-pie text-primary mr-2" style="font-size: 16px;"></i>Incluir estadísticas detalladas
                                                    </label>
                                                    <small class="form-text text-muted d-block ml-4">
                                                        <i class="fas fa-info-circle mr-1"></i>
                                                        Incluye resumen de presentes/ausentes y porcentajes de asistencia en el reporte.
                                                    </small>
                                                </div>
                                                <div class="custom-control custom-checkbox mb-3">
                                                    <input type="checkbox" class="custom-control-input" id="incluir_graficos">
                                                    <label class="custom-control-label fw-bold text-dark" for="incluir_graficos" style="cursor: pointer; font-size: 14px;">
                                                        <i class="fas fa-chart-bar text-success mr-2" style="font-size: 16px;"></i>Incluir gráficos (solo PDF)
                                                    </label>
                                                    <small class="form-text text-muted d-block ml-4">
                                                        <i class="fas fa-info-circle mr-1"></i>
                                                        Agrega gráficos de barras y pastel para visualizar la asistencia (solo disponible en PDFs).
                                                    </small>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        @if($ultimosReportes && $ultimosReportes->count() > 0)
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header" style="background: #e9ecef;">
                                        <h5 class="mb-0">
                                            <i class="fas fa-history mr-2"></i>Últimos Reportes Generados
                                        </h5>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-start">
                                                            <div>
                                                                <h6 class="card-title mb-1">
                                                                    <i class="fas fa-{{ $reporte->formato === 'pdf' ? 'file-pdf' : 'file-excel' }} mr-1"></i>
                                                                ...
                                                                <p class="card-text small mb-2">
                                                                    <strong>{{ $reporte->formato === 'pdf' ? 'PDF' : 'Excel' }}</strong> •
                                                                    {{ $reporte->registros_totales ?? 0 }} registros
                                                                </p>
                                                            ...
                                                            <div>
                                                                <a href="{{ route('asistencia.reportes.descargar', $reporte->id) }}"
...
$(document).ready(function() {
    ...
    $('#incluir_estadisticas').on('change', function() {
        console.log('Checkbox estadísticas:', $(this).is(':checked'));
    });

    $('#incluir_graficos').on('change', function() {
        console.log('Checkbox gráficos:', $(this).is(':checked'));
    });
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success && response.reporte_id) {
                ...
                // Mostrar vista previa del reporte en la página (igual para PDF y Excel)
                var formatoTexto = response.formato === 'pdf' ? 'PDF' : 'Excel';
                var iconoFormato = response.formato === 'pdf' ? 'fas fa-file-pdf' : 'fas fa-file-excel';
                $('#vista-previa-reporte').html(`
                    <div class="alert alert-success">
                        <h5><i class="${iconoFormato} mr-2"></i>Reporte generado exitosamente</h5>
                        <p><strong>Tipo:</strong> ${response.tipo_reporte || 'N/A'}</p>
                        ...
                        <div class="mt-3">
                            <a href="{{ url('/') }}/asistencia/reportes/descargar/${response.reporte_id}" class="btn btn-primary btn-sm mr-2" target="_blank">
                                <i class="fas fa-download mr-1"></i>Descargar ${formatoTexto}
                            </a>
                            <button class="btn btn-outline-secondary btn-sm" onclick="limpiarVistaPrevia()">
                                <i class="fas fa-times mr-1"></i>Limpiar Vista Previa
                            </button>
                        </div>
                    </div>
                `);
