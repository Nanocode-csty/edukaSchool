...
function cargarEstudiantesReporte() {
...
function configurarCascadaComparativo() {
    // Configurar filtrado en cascada para modo comparativo

    // Docente → Estudiantes, Asignaturas
    $('#docente_reporte').on('change', function() {
        ...
        if (docenteId) {
            $.get('{{ route("asistencia.api.estudiantes") }}', { docente_id: docenteId }, function(data) {
                ...
                $('#estudiante_reporte').trigger('change');
            });
        } else {
            cargarEstudiantesReporte();
        }
        ...
        if (docenteId) {
            $.get('{{ route("asistencia.api.asignaturas") }}', { docente_id: docenteId }, function(data) {
                ...
                $('#asignatura_reporte').trigger('change');
            });
        }
    });

    // Grado → Estudiantes
    $('#grado_reporte').on('change', function() {
...
function descargarReporte() {
    ...
    Swal.fire({
        icon: 'info',
        title: 'Descargando...',
        text: 'La descarga del reporte comenzará en breve.',
        timer: 1500,
        showConfirmButton: false
    });

    // Simular descarga (aquí iría la lógica real para descargar el archivo)
    setTimeout(function() {
        // Crear un enlace temporal para simular la descarga
        const link = document.createElement('a');
        link.href = '#'; // En una implementación real, aquí iría la URL del archivo generado
        link.download = `reporte_asistencia_${filtros.tipo_reporte}_${filtros.fecha_inicio}_${filtros.fecha_fin}.${filtros.formato}`;
