    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success && response.reporte_id) {
                // Descargar el reporte generado automáticamente
                window.location.href = '{{ route("asistencia.reportes.descargar", ":reporteId") }}'.replace(':reporteId', response.reporte_id);
                Swal.close();
            } else {
                Swal.fire({
                    icon: 'error',
        ...
        error: function(xhr) {
            $('#btnGenerarReporte').prop('disabled', false).html('<i class="fas fa-file-export mr-1"></i>Generar Reporte');
            Swal.close();
            console.error('Error al generar reporte:', xhr.responseText);
            Swal.fire({
                icon: 'error',
                title: 'Error',
