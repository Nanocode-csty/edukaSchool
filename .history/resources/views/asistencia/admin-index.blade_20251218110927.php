                                            ...
                                            <div class="row g-4 mb-4">
                                                <div class="col-lg-3">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="estudiante_id" id="buscar_estudiante" class="form-control"
                                                                style="border-radius: 8px; border: 2px solid #e3f2fd;">
                                                            <option value="">Todos los estudiantes</option>
                                                            @if(isset($estudiantes) && $estudiantes->count() > 0)
                                                                @foreach($estudiantes as $estudiante)
                                                                    <option value="{{ $estudiante->matricula_id }}" {{ ($filtros['estudiante_id'] ?? '') == $estudiante->matricula_id ? 'selected' : '' }}>
                                                                        {{ $estudiante->persona->nombres ?? 'N/A' }} {{ $estudiante->persona->apellidos ?? 'N/A' }} - {{ $estudiante->persona->dni ?? 'N/A' }}
                                                                    </option>
                                                                @endforeach
                                                            @endif
                                                        </select>
                                                        ...
                                                        <div class="row g-3">
                                                            ...
                                                            <div class="col-md-12 mt-3">
                                                                ...
                                                                <select name="estudiante_id" id="estudiante_id" class="form-control select-search" style="width: 100% !important;">
                                                                    ...
                                                                    @foreach($estudiantes as $estudiante)
                                                                        <option value="{{ $estudiante->matricula_id }}" {{ ($filtros['estudiante_id'] ?? '') == $estudiante->matricula_id ? 'selected' : '' }}>
                                                                            {{ $estudiante->persona->nombres ?? 'N/A' }} {{ $estudiante->persona->apellidos ?? 'N/A' }} - {{ $estudiante->persona->dni ?? 'N/A' }} ({{ $estudiante->curso->grado->nombre ?? 'N/A' }} - {{ $estudiante->curso->seccion->nombre ?? 'N/A' }})
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
...
@section('scripts')
    <script>
        ...
        $('#profesor_id').on('change', function() {
            ...
            $('#estudiante_id').empty().append('<option value="">Todos los estudiantes</option>').trigger('change');
