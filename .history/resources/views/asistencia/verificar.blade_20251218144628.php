                                            ...
                                            <div class="row">
                                                <div class="col-12">
                                                    <div class="d-flex justify-content-center gap-3 flex-wrap">
                                                        ...
                                                        <button type="button" class="btn btn-outline-danger btn-lg px-4" onclick="limpiarFiltrosVerificar()">
                                                ...
                                                <div class="row g-4 mb-4">
                                                    ...
                                                    <div class="col-lg-3">
                                                        <div class="form-group">
                                                            ...
                                                            <select name="grado_id" id="grado_id" class="form-control select-search"
                                                                    style="border-radius: 8px; border: 2px solid #e3f2fd; width: 100% !important;">
                                    ...
                                    @foreach($justificaciones as $index => $justificacion)
                                        <tr class="{{ $justificacion->estado === 'pendiente' ? 'table-warning' : '' }}">
                                            ...
                                            <td class="align-middle">
                                                <div>
                                                    <strong>{{ $justificacion->matricula->estudiante->persona->nombres ?? $justificacion->matricula->estudiante->nombres ?? 'N/A' }} {{ $justificacion->matricula->estudiante->persona->apellidos ?? $justificacion->matricula->estudiante->apellidos ?? 'N/A' }}</strong><br>
                                                    <small class="text-muted">
                                                    ...
                                                    <small class="text-muted">
                                                        ...
                                                        {{ $justificacion->usuarioCreador ? ($justificacion->usuarioCreador->persona ? $justificacion->usuarioCreador->persona->nombres . ' ' . $justificacion->usuarioCreador->persona->apellidos : $justificacion->usuarioCreador->nombres . ' ' . $justificacion->usuarioCreador->apellidos) : 'N/A' }}
                                                    </small>
                                                </div>
                                            </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#grado_id').select2({
                ...
                language: {
                    noResults: function() {
                        return "No se encontraron grados";
                    },
                    searching: function() {
                        return "Buscando...";
                    }
        ...
        function aplicarFiltrosVerificar() {
            // Show loading in the table
            const tablaCard = document.querySelector('.card-body[style*="border: 2px solid #86D2E3"]');
            if (tablaCard) {
                tablaCard.innerHTML = `
                    <div class="text-center py-5">
            ...
            // Make AJAX request
            fetch('/asistencia/verificar?' + params.toString(), {
                method: 'GET',
                headers: {
            ...
            .then(html => {
                ...
                const newContent = newDoc.querySelector('[style*="padding-left:0.966666666rem"]');

                if (newContent) {
                    const currentContent = document.querySelector('[style*="padding-left:0.966666666rem"]');
                    if (currentContent) {
                ...
                const newUrl = '{{ route("asistencia.verificar") }}?' + params.toString();
                window.history.pushState({}, '', newUrl);
            })
            .catch(error => {
                console.error('Error:', error);
                // Show error message
                const tablaCard = document.querySelector('.card[style*="border: 2px solid #86D2E3"]');
                if (tablaCard) {
                    tablaCard.innerHTML = `
                        ...
                        <div class="card-body" style="border: 2px solid #86D2E3; border-top: none; border-radius: 0px 0px 4px 4px !important; padding: 0;">
                            <div class="text-center py-5">
                                ...
                                <button onclick="aplicarFiltrosVerificar()" class="btn btn-primary">Reintentar</button>
                            </div>
                        </div>
                    `;
                }
