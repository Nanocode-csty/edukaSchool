        ...
        <div class="col-12">
            <div class="box_block">
                ...
                function configurarFiltrosCascada() {
                    ...
                    // Función para aplicar filtros bidireccionales
                    function aplicarFiltrosBidireccionales() {
                        ...
                        // Filtrar cursos basado en nivel y estudiante seleccionados
                        filtrarCursos(nivelSeleccionado, estudianteSeleccionado);

                        // Filtrar estudiantes basado en curso y nivel seleccionados
                        filtrarEstudiantes(cursoSeleccionado, nivelSeleccionado);
                    ...
                    function filtrarCursos(nivelId, estudianteId) {
                        const cursoOptions = cursoSelect.querySelectorAll('option');
                        ...
                        cursoOptions.forEach(option => {
                            ...
                            // Si hay nivel seleccionado, el curso debe ser de ese nivel
                            if (nivelId && cursoNivel !== nivelId) {
                                mostrar = false;
                            }

                            // Si hay estudiante seleccionado, el curso debe ser el del estudiante
                            if (estudianteId && mostrar) {
                                const estudianteOption = Array.from(estudianteSelect.options).find(opt => opt.value === estudianteId);
                                if (estudianteOption) {
                                    const estudianteCurso = estudianteOption.getAttribute('data-curso');
                                    if (estudianteCurso && option.value !== estudianteCurso) {
                                        mostrar = false;
                                    }
                                }
                            }
                            ...
                            console.log(`Curso ${option.text}: mostrar=${mostrar}, nivel=${cursoNivel}, nivelSeleccionado=${nivelId}`);
                        });
                    }

                    // Función para filtrar estudiantes
                    function filtrarEstudiantes(cursoId, nivelId) {
                        const estudianteOptions = estudianteSelect.querySelectorAll('option');

                        estudianteOptions.forEach(option => {
                            if (option.value === '') {
                                option.style.display = 'block';
                                return;
                            }
                            ...
                            // Si hay curso seleccionado, el estudiante debe estar en ese curso
                            if (cursoId && estudianteCurso !== cursoId) {
                                mostrar = false;
                            }

                            // Si hay nivel seleccionado, el curso del estudiante debe ser de ese nivel
                            if (nivelId && mostrar) {
                                const cursoOption = Array.from(cursoSelect.options).find(opt => opt.value === estudianteCurso);
                                if (cursoOption) {
                                    const cursoNivel = cursoOption.getAttribute('data-nivel');
                                    if (cursoNivel && cursoNivel !== nivelId) {
                                        mostrar = false;
                                    }
                                }
                            }
                            ...
                            option.style.display = mostrar ? 'block' : 'none';
                            console.log(`Estudiante ${option.text}: mostrar=${mostrar}, curso=${estudianteCurso}, cursoSeleccionado=${cursoId}, nivel=${nivelId}`);
                        });
                    }
                    ...
                    nivelSelect.addEventListener('change', aplicarFiltrosBidireccionales);
                    cursoSelect.addEventListener('change', aplicarFiltrosBidireccionales);
                    estudianteSelect.addEventListener('change', aplicarFiltrosBidireccionales);
                    docenteSelect.addEventListener('change', aplicarFiltrosBidireccionales);
                }
