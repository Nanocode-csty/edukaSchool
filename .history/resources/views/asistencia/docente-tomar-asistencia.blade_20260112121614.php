...
function loadSessionsForDay(date, container) {
    // This would normally load sessions via AJAX
    // For now, show a placeholder
    const dateStr = date.toISOString().split('T')[0];
    container.innerHTML = `<small style="color: #6c757d;">Cargando sesiones...</small>`;

    // Simulate loading sessions (replace with actual AJAX call)
    setTimeout(() => {
        // This would be replaced with actual session data
        const sessionCount = Math.floor(Math.random() * 4); // Random for demo
        if (sessionCount > 0) {
...
function updateDaySelection(selectedCard, selectedDate) {
    ...
    document.querySelectorAll('.day-card').forEach(card => {
        ...
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const isToday = card.dataset.date === today.toISOString().split('T')[0];

        // Remove selection indicators
        const oldIndicators = card.querySelectorAll('.selection-indicator');
        oldIndicators.forEach(indicator => indicator.remove());

        // Reset styles based on whether it's today or not
        card.style.borderColor = isToday ? '#007bff' : '#dee2e6';
        card.style.borderWidth = isToday ? '3px' : '2px';
        card.style.backgroundColor = isToday ? '#f8f9fa' : 'white';
        card.style.boxShadow = isToday ? '0 0 10px rgba(0, 123, 255, 0.1)' : 'none';
    });
    ...
    selectedIndicator.style.cssText = `
        ...
        z-index: 10;
    `;
    selectedIndicator.textContent = isToday ? 'Hoy' : 'Seleccionado';
...
function filterSessionsByDateAjax(dateStr) {
    ...
    if (sessionsContainer) {
        const originalText = sessionsContainer.innerHTML;
        sessionsContainer.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Cargando sesiones...';
        ...
        .then(html => {
            ...
            if (newSessionsList && currentSessionsList) {
                currentSessionsList.innerHTML = newSessionsList.innerHTML;
            }

            // Update the header with the new date
            const newHeader = doc.querySelector('.card-header h5');
            if (newHeader) {
                sessionsContainer.innerHTML = newHeader.innerHTML;
            }
        ...
        .catch(error => {
            ...
            sessionsContainer.innerHTML = originalText;
            Swal.fire({
                icon: 'error',
                title: 'Error',
