...
@section('contenidoplantilla')
    ...
    <script>
        ...
        function aprobarTodasPendientes() {
            if (confirm('¿Está seguro de aprobar todas las justificaciones pendientes? Esta acción no se puede deshacer.')) {
                // Show loading
                ...
                fetch('{{ route("asistencia.aprobar-todas-pendientes") }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        alert(`Se aprobaron ${data.aprobadas} justificaciones exitosamente.`);
                        location.reload();
                    } else {
        ...
        function exportarJustificaciones() {
            ...
            form.method = 'GET';
            form.action = '{{ route("asistencia.exportar-justificaciones") }}';
        ...
        function enviarNotificaciones() {
            if (confirm('¿Enviar recordatorios a representantes con justificaciones pendientes?')) {
                const btn = event.target;
                const originalText = btn.innerText;
                btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Enviando...';

                fetch('{{ route("asistencia.enviar-recordatorios") }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Accept': 'application/json',
                ...
                .then(data => {
                    if (data.success) {
                        alert(`Se enviaron ${data.enviados} recordatorios exitosamente.`);
                    } else {
                        alert('Error: ' + (data.message || 'Ocurrió un error'));
                    }
                    btn.innerHTML = originalText;
