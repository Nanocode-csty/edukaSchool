                    ...
                    <div class="col-12">
                        <div class="quick-nav d-flex justify-content-center">
                            <div class="nav nav-pills" role="tablist">
                                <a class="nav-link active" href="{{ route('asistencia.representante.index') }}" onclick="loadContent('asistencias'); return false;">
                                    <i class="fas fa-calendar-check mr-2"></i>Asistencias
                                </a>
                                <a class="nav-link" href="{{ route('notas.misEstudiantes') }}" onclick="loadContent('calificaciones'); return false;">
...
function loadContent(type) {
    ...
    if (type === 'asistencias') {
        $('a[href*="asistencia.representante.index"]').addClass('active');
    } else if (type === 'calificaciones') {
        $('a[href*="notas.misEstudiantes"]').addClass('active');
    }

    // Mostrar loading
    const contentArea = $('.card-body').find('.table-responsive, .row:not(.mb-4):last');
    contentArea.html('<div class="text-center py-5"><div class="spinner-border text-primary" role="status"><span class="sr-only">Cargando...</span></div><p class="mt-2">Cargando contenido...</p></div>');
    ...
    if (type === 'asistencias') {
        ...
        window.location.href = '{{ route("notas.misEstudiantes") }}';
    }
}
</script>
<style>
...
.quick-nav .nav-pills .nav-link.active {
    ...
    border-color: #28aece;
    color: white;
    box-shadow: 0 4px 12px rgba(40, 174, 206, 0.4);
}
