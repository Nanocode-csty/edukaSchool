...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const waitLibs = setInterval(() => {
            if (window.$?.fn?.select2 && window.Chart) {
        ...
        setTimeout(() => {
            clearInterval(waitLibs);
            console.warn('Initializing without library check');
            initReports();
        }, 3000);
    });
    ...
    function setupEvents() {
        ...
        // Date changes for dynamic filters
        $('#fecha_inicio, #fecha_fin').on('change', updateDynamicFilters);
    ...
    function handleReportTypeChange() {
        ...
        switch(tipo) {
            ...
            case 'comparativo':
                ...
                `;
                setTimeout(() => updateDynamicFilters(), 200);
                break;
        }

        container.html(tipo === 'comparativo' ?
            '<div class="alert alert-info"><i class="fas fa-calendar-alt mr-2"></i>Selecciona fechas para cargar filtros disponibles.</div>' : html);
        ...
        setTimeout(() => {
            ...
            // Setup cascading filters for comparative
            if (tipo === 'comparativo') {
    ...
    function updateDynamicFilters() {
        ...
        $('#filtroContainer').html('<div class="text-center py-3"><div class="spinner-border spinner-border-sm"></div><small> Cargando filtros...</small></div>');
