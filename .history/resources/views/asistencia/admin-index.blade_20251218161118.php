                                            ...
                                            <div class="row g-4 mb-4">
                                                <div class="col-lg-3">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="estudiante_id" id="estudiante_id" class="form-control select-search"
                                                            ...
                                                            @foreach($estudiantes as $estudiante)
                                                                <option value="{{ $estudiante->estudiante_id }}" {{ ($filtros['estudiante_id'] ?? '') == $estudiante->estudiante_id ? 'selected' : '' }}>
                                                                    {{ $estudiante->persona->nombres ?? 'N/A' }} {{ $estudiante->persona->apellidos ?? 'N/A' }} - DNI: {{ $estudiante->persona->dni ?? 'N/A' }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                            ...
                                            <div class="row">
                                                <div class="col-12">
                                                    <div class="d-flex justify-content-center gap-3 flex-wrap">
                                                        ...
                                                        <button type="button" class="btn btn-outline-danger btn-lg px-4" onclick="limpiarFiltros()">
...
@section('scripts')
    ...
    <script>
        // Inicializar Select2 directamente con opciones de búsqueda
        $(document).ready(function() {
            console.log('Initializing Select2 for admin view...');

            // Inicializar todos los select con clase select-search
            $('.select-search').each(function() {
                const $select = $(this);
                const elementId = $select.attr('id');

                // Configurar placeholder específico según el elemento
                let placeholder = 'Seleccionar...';
                if (elementId === 'profesor_id') placeholder = 'Buscar profesor...';
                else if (elementId === 'curso_id') placeholder = 'Buscar curso...';
                else if (elementId === 'estudiante_id') placeholder = 'Buscar estudiante...';
                else if (elementId === 'asignatura_id') placeholder = 'Buscar asignatura...';

                // Inicializar Select2 directamente con búsqueda siempre habilitada
                $select.select2({
            ...
            console.log('Select2 initialization completed for admin view');
        });
        ...
        function aplicarFiltros() {
            ...
            // Filtros rápidos
            const estudianteIdElement = document.getElementById('estudiante_id');
        ...
        function limpiarFiltros() {
            ...
            document.getElementById('tipo_asistencia_id').value = '';
            document.getElementById('fecha_desde').value = '';
            document.getElementById('fecha_hasta').value = '';

            // Limpiar filtros avanzados si están visibles
            ...
            if (profesorSelect) profesorSelect.value = '';
            if (cursoSelect) cursoSelect.value = '';
            if (asignaturaSelect) asignaturaSelect.value = '';
            if (ordenarSelect) ordenarSelect.value = 'fecha';
            ...
            // Aplicar filtros con parámetros vacíos (esto recargará la tabla vía AJAX)
            aplicarFiltros();
        }
        ...
        // Función para actualizar estudiantes basado en filtros actuales
        function actualizarEstudiantes() {
            ...
            fetch('/asistencia/api/estudiantes-filtrados?' + params.toString())
                .then(response => response.json())
                .then(data => {
                    // Limpiar estudiantes actuales
                    $('#estudiante_id').empty().append('<option value="">Todos los estudiantes</option>');

                    // Agregar estudiantes filtrados
                    data.forEach(estudiante => {
                        const option = new Option(
                            `${estudiante.persona.nombres} ${estudiante.persona.apellidos} - DNI: ${estudiante.persona.dni}`,
                            estudiante.estudiante_id,
                            false,
                            false
                        );
                        $('#estudiante_id').append(option);
                    });
        ...
        // Función para actualizar cursos basado en filtros actuales
        function actualizarCursos() {
            ...
            fetch('/asistencia/api/cursos-filtrados?' + params.toString())
                .then(response => response.json())
                .then(data => {
                    // Limpiar cursos actuales
                    $('#curso_id').empty().append('<option value="">Todos los cursos</option>');

                    // Agregar cursos filtrados
                    data.forEach(curso => {
                        const option = new Option(
                            `${curso.grado.nombre} - ${curso.seccion.nombre}`,
                            curso.curso_id,
        ...
        // Función para actualizar asignaturas basado en filtros actuales
        ...
        $('#profesor_id').on('change', function() {
            ...
            if (profesorId) {
                // Si hay profesor seleccionado, filtrar cursos y asignaturas
                actualizarCursos();
                actualizarAsignaturas();
            } else {
                // Si no hay profesor, mostrar todos
            ...
            // Actualizar estudiantes después de cambiar profesor
        ...
        $('#curso_id').on('change', function() {
            ...
            if (cursoId) {
                // Si hay curso seleccionado, filtrar asignaturas
                actualizarAsignaturas();
            }

            // Actualizar estudiantes después de cambiar curso
            actualizarEstudiantes();
        });

        $('#asignatura_id').on('change', function() {
            // Actualizar estudiantes después de cambiar asignatura
            actualizarEstudiantes();
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (ctxTendencia) {
                ...
                // Get current date and go back 30 days
                ...
                for (let i = 29; i >= 0; i--) {
                    ...
                    // For now, generate random data. In a real implementation, you'd fetch this from the server
                    porcentajes.push(Math.floor(Math.random() * 40) + 60); // Random between 60-100
                }
                ...
                new Chart(ctxTendencia.getContext('2d'), {
                    ...
                    options: {
                        ...
                        scales: {
                            y: {
                                beginAtZero: true,
                                max: Math.max(100, Math.max(...porcentajes) + 10),
                                ticks: {
