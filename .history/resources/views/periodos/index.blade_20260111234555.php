                            ...
                            <div class="col-md-4">
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn btn-primary flex-fill" id="btnAplicarFiltros" title="Aplicar filtros">
                                        <i class="fas fa-search"></i> <span id="btnAplicarText">Aplicar</span>
                                        <div id="loadingSpinner" class="d-none" style="display: inline-block; margin-left: 8px;">
    ...
    $('#btnAplicarFiltros').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Extraer el contenido de la tabla y estadísticas del HTML
                const parser = new DOMParser();
                const doc = parser.parseFromString(response, 'text/html');

                // Actualizar tabla
                const newTableBody = doc.querySelector('#tbodyPeriodos');
                if (newTableBody) {
                    $('#tbodyPeriodos').html(newTableBody.innerHTML);
                }

                // Actualizar estadísticas
                const newStats = doc.querySelector('#estadisticasContainer');
                if (newStats) {
                    $('#estadisticasContainer').html(newStats.innerHTML);
                }

                // Actualizar paginación
                const newPagination = doc.querySelector('#paginacionContainer');
                if (newPagination) {
                    $('#paginacionContainer').html(newPagination.innerHTML);
                }
