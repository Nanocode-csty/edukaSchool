...
$(document).ready(function() {
    ...
    function obtenerAnioSeleccionado() {
        ...
        // Patrón 1: Solo números al inicio (ej: "2025")
        let match = anioTexto.match(/^(\d{4})/);
        if (match) return match[1];

        // Patrón 2: Buscar cualquier secuencia de 4 dígitos
        match = anioTexto.match(/(\d{4})/);
        if (match) return match[1];

        // Fallback: usar el valor del option (que es el ID)
        const anioId = selectedOption.val();
        if (anioId) {
            // Hacer una petición AJAX para obtener el nombre del año
            // Por ahora, devolver el año actual como fallback
            console.warn('No se pudo extraer el año de:', anioTexto);
        }
