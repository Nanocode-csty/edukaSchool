...
$(document).ready(function() {
    ...
    // Función para obtener el año del año lectivo seleccionado (con AJAX si es necesario)
    function obtenerAnioSeleccionado(callback) {
        ...
        // Intentar extraer del texto del option primero
        const anioTexto = selectedOption.text().trim();
        const match = anioTexto.match(/^(\d{4})/);
        if (match) {
            anioSeleccionadoActual = match[1];
            if (callback) callback(anioSeleccionadoActual);
            return anioSeleccionadoActual;
        }

        // Si no se pudo extraer del texto, hacer petición AJAX
        $.ajax({
            ...
            success: function(data) {
                if (data.nombre_anio) {
                    ...
                    const matchAjax = data.nombre_anio.match(/^(\d{4})/);
                    anioSeleccionadoActual = matchAjax ? matchAjax[1] : new Date().getFullYear();
                } else {
                    anioSeleccionadoActual = new Date().getFullYear();
                }
                if (callback) callback(anioSeleccionadoActual);
