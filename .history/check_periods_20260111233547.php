...
if ($anio2026) {
    ...
    // Caso 3: Crear un período válido (debería pasar) - período que no existe
    echo "\nCaso 3: Intentar crear período válido (PREINSCRIPCION para 2027)\n";
    $anio2027 = InfAnioLectivo::create([
        'nombre' => '2027-2028',
        'descripcion' => 'Año Lectivo 2027-2028',
        'fecha_inicio' => Carbon\Carbon::create(2027, 3, 1),
        'fecha_fin' => Carbon\Carbon::create(2027, 12, 20),
    ]);
    $conflicts = $method->invoke($controller, 'PREINSCRIPCION', '2027-01-01', '2027-02-28', $anio2027->ano_lectivo_id);
    if (empty($conflicts)) {
        echo "✅ CORRECTO: No detectó conflictos para período válido\n";
    } else {
        echo "❌ ERROR: Detectó conflicto innecesario: " . implode(', ', $conflicts) . "\n";
    }
    // Limpiar el año lectivo de prueba
    $anio2027->delete();
