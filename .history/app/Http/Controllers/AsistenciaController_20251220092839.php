...
class AsistenciaController extends Controller
{
    ...
    public function getEstadisticasFiltradas(Request $request)
    {
        try {
            ...
            // Calcular estadísticas adicionales de valor
            $totalEstudiantesUnicos = $asistenciasFiltradas->unique('matricula.matricula_id')->count();
            $promedioAsistenciaDiaria = $totalEstudiantesUnicos > 0 ?
                round(($asistenciasFiltradas->where('tipoAsistencia.codigo', 'P')->count() / $totalEstudiantesUnicos) * 100, 1) : 0;

            // Alertas de riesgo (estudiantes con baja asistencia)
            ...
            if ($totalEstudiantesUnicos > 0) {
                ...
                foreach ($asistenciasPorEstudiante as $matriculaId => $asistenciasEstudiante) {
                    ...
                    $presentesEst = $asistenciasEstudiante->where('tipoAsistencia.codigo', 'P')->count();
                    $porcentajeEst = $totalAsistenciasEst > 0 ? ($presentesEst / $totalAsistenciasEst) * 100 : 0;
