...
$estudiante = DB::table('estudiantes as e')
    ...
    ->where(function ($q) use ($buscarpor) {
        $q->where('p.dni', 'like', '%' . $buscarpor . '%')
          ->orWhere('p.apellidos', 'like', '%' . $buscarpor . '%');
    })
    ...
    public function generarFicha($id)
    {
        ...
        $representantes = DB::table('estudiante_representante as er')
            ->join('representantes as r', 'er.representante_id', '=', 'r.representante_id')
            ->join('personas as p', 'r.persona_id', '=', 'p.id_persona')
            ->where('er.estudiante_id', $id)
            ->select(
                'er.viveConEstudiante',
                'er.es_principal',
                'r.representante_id',
                'r.ocupacion',                 // padre, madre, apoderado, etc
                'r.parentesco',
                'p.id_persona',
                'p.dni',
                'p.nombres',
        ...
        $pdf = PDF::loadView('ceinformacion.estudiantes.ficha', compact('estudiante', 'persona', 'representantes', 'qr', 'codigo'))->setPaper('A4', 'portrait');
    ...
    public function apiIndex(Request $request)
    {
        try {
            ...
            if ($request->filled('nivel_id')) {
                \Log::info('Applying nivel_id filter:', $request->nivel_id);
            ...
            if ($request->filled('grado_id')) {
                \Log::info('Applying grado_id filter:', $request->grado_id);
            ...
            if ($request->filled('seccion_id')) {
                \Log::info('Applying seccion_id filter:', $request->seccion_id);
                $hasMatriculasJoin = collect($query->getQuery()->joins ?? [])->pluck('table')->contains('matriculas');
                if (!$hasMatriculasJoin) {
                    $query->join('matriculas', 'estudiantes.estudiante_id', '=', 'matriculas.estudiante_id');
                }
            ...
            if ($request->filled('docente_id')) {
                \Log::info('Applying docente_id filter:', $request->docente_id);
            ...
            if ($request->filled('asignatura_id')) {
                \Log::info('Applying asignatura_id filter:', $request->asignatura_id);
            ...
            \Log::info('Query executed, results:', $estudiantes->count());
            ...
            \Log::info('Mapped results:', count($estudiantes));
