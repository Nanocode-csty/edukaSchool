...
public function store(Request $request)
{
    ...
    public function apiIndex()
    {
        try {
            ...
            // Log for debugging
            \Log::info('Total estudiantes in DB: ' . $estudiantes->count());
            \Log::info('Estudiantes with persona_id: ' . $estudiantes->whereNotNull('persona_id')->count());

            // Filter for students with persona_id (since there's no estado field)
            $estudiantes = InfEstudiante::whereNotNull('persona_id')
                ->with('persona')
                ->get();

            \Log::info('Estudiantes with persona_id: ' . $estudiantes->count());

            $result = $estudiantes->map(function($estudiante) {
                $nombres = '';
                $apellidos = '';

                // Try to get names from persona relationship first
                if ($estudiante->persona) {
                    $nombres = $estudiante->persona->nombres;
                    $apellidos = $estudiante->persona->apellidos;
                }

                // Fallback to old columns if persona data is not available
                if (empty($nombres) && isset($estudiante->nombres)) {
                    $nombres = $estudiante->nombres;
                }
                if (empty($apellidos) && isset($estudiante->apellidos)) {
                    $apellidos = $estudiante->apellidos;
                }

                // If still no names, use a placeholder
                if (empty($nombres) && empty($apellidos)) {
                    $nombres = 'Estudiante';
                    $apellidos = 'ID: ' . $estudiante->estudiante_id;
                }
            ...
            \Log::info('Sample result: ' . json_encode($result->take(3)->toArray()));
