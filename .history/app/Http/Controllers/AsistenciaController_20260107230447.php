...
class AsistenciaController extends Controller
{
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            $reporteGenerado = ReporteGenerado::create([
                'usuario_id' => Auth::id(),
                'tipo_reporte' => $this->generarNombreReporte($tipoReporteOriginal, 'excel', $request->fecha_inicio, $request->fecha_fin),
                'fecha_inicio' => $fechaInicio,
                'fecha_fin' => $fechaFin,
                'formato' => 'xlsx', // Guardar como 'xlsx' para consistencia
                'registros_totales' => $asistencias->count(),
                'archivo_path' => null, // No guardamos archivo físico para descargas directas
                'archivo_nombre' => $this->generarNombreReporte($tipoReporteOriginal, 'excel', $request->fecha_inicio, $request->fecha_fin),
                'fecha_generacion' => now(),
                'filtros_aplicados' => json_encode($request->all())
            ]);
    ...
    public function guardarReporteGenerado(Request $request)
    {
        ...
        try {
            $reporte = ReporteGenerado::create([
                ...
                'archivo_nombre' => $this->generarNombreReporte($request->tipo_reporte, 'preview', $request->fecha_inicio, $request->fecha_fin),
                'fecha_generacion' => now()
            ]);
