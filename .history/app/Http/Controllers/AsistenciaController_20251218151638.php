...
class AsistenciaController extends Controller
{
    ...
    public function reporteGeneral(Request $request)
    {
        ...
        // Obtener estudiantes matriculados según el filtro (incluir diferentes estados de matrícula)
        ...
        // Si no hay matrículas, intentar obtener al menos estudiantes sin filtro de matrícula
        if ($matriculas->isEmpty()) {
            // Obtener todos los estudiantes para mostrar estadísticas básicas
            ...
            $datosGrafico = ['labels' => [], 'porcentajes' => []];
        ...
        // Calcular estadísticas basadas en estudiantes matriculados
        ...
        // Estadísticas por curso - mostrar todos los cursos con sus estudiantes matriculados
        $estadisticasPorCurso = [];
        if ($cursoId) {
            // Si hay filtro de curso específico
            $curso = \App\Models\InfCurso::with(['grado', 'seccion'])->find($cursoId);
            if ($curso) {
                // Contar estudiantes matriculados en este curso
                $totalEstudiantes = \App\Models\Matricula::where('curso_id', $cursoId)
                    ->where('estado', 'Matriculado')
                    ->count();

                // Filtrar asistencias para este curso
            ...
            // Estadísticas por todos los cursos - obtener cursos que tienen matrículas activas
            ...
            foreach ($cursos as $curso) {
                // Contar estudiantes matriculados en este curso
                $totalEstudiantes = \App\Models\Matricula::where('curso_id', $curso->curso_id)
                    ->where('estado', 'Matriculado')
                    ->count();
        ...
        // Datos para gráficos - siempre generar datos para el rango de fechas
        $datosGrafico = [
            'labels' => [],
            'porcentajes' => [],
        ];

        // Generar datos de tendencia por día (siempre mostrar el rango completo)
        $periodo = Carbon::parse($fechaInicio);
        while ($periodo->lte($fechaFin)) {
            $datosGrafico['labels'][] = $periodo->format('d/m');
