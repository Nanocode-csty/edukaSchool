...
class NotasController extends Controller
{
    ...
    {
        \Log::info('verNotasEstudiante called', ['estudiante_id' => $id, 'user_id' => auth()->id(), 'user_rol' => auth()->user()->rol]);

        $esAccesoAutorizado = false;
        $rolUsuario = auth()->user()->rol;

        // Si el usuario es representante, verificar si tiene permiso para este estudiante
        if ($rolUsuario === 'Representante' && auth()->user()->persona->representante) {
            ...
            \Log::info('Permission check result', ['has_permission' => $esRepresentanteDelEstudiante]);
    ...
    {
        // TEMPORAL: Bypass role check for testing
        // Verificar que el usuario tenga rol de Representante
        // if (!auth()->user() || !auth()->user()->hasRole('representante')) {
        //     return redirect()->route('rutarrr1')
        //         ->with('error', 'No tiene permisos para acceder a esta sección. Esta función es exclusiva para representantes.');
        // }

        // TEMPORAL: Use first representante for testing
        $representante = \App\Models\InfRepresentante::first();
        ...
        return view('asistencia.representante-calificaciones', compact('estudiantesRepresentados'));
    }
}
