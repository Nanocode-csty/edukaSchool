...
class AsistenciaController extends Controller
{
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            // Debug log
            \Log::info('Guardando reporte Excel descarga directa - INICIO', [
                'tipo_reporte_original' => $tipoReporteOriginal,
                'nombre_descriptivo' => $nombreDescriptivo,
                'request_tipo_reporte' => $request->tipo_reporte,
                'request_all' => $request->all(),
                'registros_count' => $asistencias->count()
            ]);
            ...
            $reporteGenerado = ReporteGenerado::create([
                ...
                'fecha_inicio' => $fechaInicio,
                'fecha_fin' => $fechaFin,
                'formato' => 'xlsx', // Guardar como 'xlsx' para consistencia
                'registros_totales' => $asistencias->count(),
                'archivo_path' => null, // No guardamos archivo físico para descargas directas
                'archivo_nombre' => $nombreDescriptivo,
                'fecha_generacion' => now(),
                'filtros_aplicados' => json_encode($request->all())
            ]);

            \Log::info('Reporte Excel guardado - EXITO', [
                'reporte_id' => $reporteGenerado->id,
                'tipo_reporte_guardado' => $reporteGenerado->tipo_reporte,
                'formato_guardado' => $reporteGenerado->formato
            ]);
