...
class AsistenciaController extends Controller
{
    ...
    public function representanteDashboard()
    {
        ...
        // Debug: Check user and relationships
        $user = Auth::user();
        \Log::info('User in representanteDashboard', [
            'user_id' => $user->usuario_id ?? 'N/A',
            'username' => $user->username ?? 'N/A',
            'has_persona' => isset($user->persona),
            'persona_id' => $user->persona->id_persona ?? 'N/A',
            'has_representante' => isset($user->persona) && isset($user->persona->representante),
            'representante_id' => $user->persona->representante->representante_id ?? 'N/A'
        ]);

        if (!$user->persona || !$user->persona->representante) {
            return view('asistencia.representante-dashboard', [
                'error' => 'No se encontró información de representante para este usuario.',
                'debug' => [
                    'user_id' => $user->usuario_id,
                    'has_persona' => isset($user->persona),
                    'has_representante' => isset($user->persona) && isset($user->persona->representante)
        ...
        try {
            ...
            $niveles = InfNivel::whereHas('grados', function($q) use ($fechaInicio, $fechaFin) {
                $q->whereHas('cursos', function($cq) use ($fechaInicio, $fechaFin) {
                    $cq->whereHas('matriculas', function($mq) use ($fechaInicio, $fechaFin) {
                        $mq->whereHas('asistenciasDiarias', function($aq) use ($fechaInicio, $fechaFin) {
            ...
            $cursos = InfCurso::with(['grado', 'seccion', 'aula'])
                ->whereHas('matriculas', function($q) use ($fechaInicio, $fechaFin) {
                    $q->whereHas('asistenciasDiarias', function($aq) use ($fechaInicio, $fechaFin) {
                        $aq->whereBetween('fecha', [$fechaInicio, $fechaFin]);
                    });
                })
                ->orderBy('grado_id')
            ...
            $docentes = InfDocente::with(['persona'])
                ->whereHas('cursoAsignaturas', function($q) use ($fechaInicio, $fechaFin) {
                    $q->whereHas('sesionesClases', function($sq) use ($fechaInicio, $fechaFin) {
                        $sq->whereHas('asistenciasDiarias', function($aq) use ($fechaInicio, $fechaFin) {
                            $aq->whereBetween('fecha', [$fechaInicio, $fechaFin]);
                        });
                    });
                })
