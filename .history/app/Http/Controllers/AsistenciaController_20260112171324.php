...
class AsistenciaController extends Controller
{
    ...
    public function procesarVerificacion(Request $request)
    {
        try {
            ...
            // Si se aprueba, crear registros de asistencia justificada para todas las sesiones del día
            if ($nuevoEstado === 'aprobado') {
                \Log::info('Justificación aprobada - creando registros justificados', [
                ...
                try {
                    // Obtener todas las sesiones del estudiante para la fecha justificada
                    $sesionesDia = \App\Models\SesionClase::with('cursoAsignatura')
                        ->whereHas('cursoAsignatura.curso.matriculas', function($query) use ($justificacion) {
                            $query->where('matricula_id', $justificacion->matricula_id);
                        })
                        ->whereDate('fecha', $justificacion->fecha)
                        ->get();

                    \Log::info('Sesiones encontradas para justificar', [
                        'sesiones_count' => $sesionesDia->count(),
                        'fecha' => $justificacion->fecha
                    ]);

                    // Obtener el tipo de asistencia "Justificado"
                    $tipoJustificado = TipoAsistencia::where('codigo', 'J')->first();

                    if (!$tipoJustificado) {
                        \Log::error('Tipo de asistencia "Justificado" (J) no encontrado');
                        throw new \Exception('Tipo de asistencia justificado no configurado');
                    }

                    $registrosCreados = 0;
                    foreach ($sesionesDia as $sesion) {
                        // Crear registro de asistencia justificada
                        $asistenciaJustificada = AsistenciaAsignatura::updateOrCreate(
                            [
                                'matricula_id' => $justificacion->matricula_id,
                            ...
                            [
                                'tipo_asistencia_id' => $tipoJustificado->tipo_asistencia_id,
                                'hora_registro' => now()->format('H:i:s'),
                                'estado' => 'Justificada',
                                'usuario_registro' => Auth::id(),
                                'justificacion' => 'Justificación aprobada - ' . $justificacion->motivo,
                            ]
                        );

                        $registrosCreados++;
                        \Log::info('Registro justificado creado', [
                            'asistencia_id' => $asistenciaJustificada->asistencia_id,
                            'sesion_id' => $sesion->id,
                            'curso_asignatura' => $sesion->cursoAsignatura->asignatura->nombre
                        ]);
                    }

                    \Log::info('Justificación aplicada exitosamente', [
                        'justificacion_id' => $justificacion->id,
                        'registros_creados' => $registrosCreados,
                        'sesiones_justificadas' => $sesionesDia->count()
                    ]);

                } catch (\Exception $justifError) {
                    \Log::error('Error creando registros justificados: ' . $justifError->getMessage());
                    // No fallar la operación de aprobación por este error
                    // La justificación se mantiene aprobada pero sin registros automáticos
                }
            }
            ...
            $puedeEditar = Auth::user()->hasRole('Administrador') || $diasDesdeSesion <= 7;
                    ...
                    $estudiantes = $sesion->cursoAsignatura->curso->matriculas()
                        ...
                        ->map(function($matricula) use ($sesion) {
                            ...
                            $matricula->estado_asistencia = $asistenciaExistente ? $asistenciaExistente->estado : null;
                            $matricula->justificada = $asistenciaExistente && $asistenciaExistente->estado === 'Justificada';
                            return $matricula;
                        });
            ...
            if ($esEdicion) {
                ...
                foreach ($estudiantesSesion as $matricula) {
                    ...
                    if ($asistenciaData && isset($asistenciaData['tipo_asistencia']) && $asistenciaData['tipo_asistencia']) {
                        ...
                        // Verificar si ya existe una asistencia justificada para este estudiante en esta sesión
                        $asistenciaExistente = \App\Models\AsistenciaAsignatura::where('matricula_id', $matricula->matricula_id)
                            ->where('curso_asignatura_id', $sesion->cursoAsignatura->curso_asignatura_id)
                            ->where('fecha', $sesion->fecha)
                            ->first();

                        // Si la asistencia está justificada, no permitir cambios
                        if ($asistenciaExistente && $asistenciaExistente->estado === 'Justificada') {
                            \Log::warning("Intento de modificar asistencia justificada para estudiante {$matricula->matricula_id}");
    ...
    public function representanteDetalle(Request $request, $estudiante_id)
    {
        try {
            ...
            \Log::info('Buscando estudiante', [
                'representante_id' => $representante->representante_id,
                'estudiante_id' => $estudiante_id
            ]);

            // Primero verificar si existe la relación en la tabla pivote
            $tieneRelacion = \DB::table('estudiante_representante')
                ->where('estudiante_id', $estudiante_id)
                ->where('representante_id', $representante->representante_id)
                ->exists();

            \Log::info('Verificación de relación estudiante-representante', [
                'estudiante_id' => $estudiante_id,
                'representante_id' => $representante->representante_id,
                'tiene_relacion' => $tieneRelacion
            ]);

            if (!$tieneRelacion) {
                \Log::warning('Estudiante no pertenece al representante', [
                    'estudiante_id' => $estudiante_id,
                    'representante_id' => $representante->representante_id
                ]);
                abort(404, 'El estudiante no está asociado con su cuenta de representante.');
            }

            // Ahora buscar el estudiante con todas las relaciones necesarias
            $estudiante = \App\Models\InfEstudiante::with([
                'persona',
                'matricula' => function($query) {
                    $query->with(['grado', 'seccion', 'curso.anoLectivo']);
                }
            ])->find($estudiante_id);

            \Log::info('Estudiante encontrado con relaciones', [
                'estudiante_id' => $estudiante ? $estudiante->estudiante_id : null,
                'tiene_persona' => $estudiante && $estudiante->relationLoaded('persona') ? true : false,
                'persona_data' => $estudiante && $estudiante->persona ? $estudiante->persona->toArray() : null,
                'tiene_matricula' => $estudiante && $estudiante->relationLoaded('matricula') ? true : false
            ]);

            \Log::info('Resultado de búsqueda de estudiante', [
                'estudiante_encontrado' => $estudiante ? true : false,
                'estudiante_id' => $estudiante ? $estudiante->estudiante_id : null,
                'tiene_persona' => $estudiante && $estudiante->persona ? true : false
            ]);
            ...
            \Log::info('Estudiante encontrado correctamente', [
                'estudiante_id' => $estudiante->estudiante_id,
                'estudiante_persona' => $estudiante->persona ? $estudiante->persona->nombres . ' ' . $estudiante->persona->apellidos : 'Sin persona'
            ]);
            ...
            // Obtener el ID de matrícula del estudiante (si existe)
            $matricula_id = $estudiante->matricula ? $estudiante->matricula->matricula_id : 0;
            ...
            $queryEstadisticas = AsistenciaAsignatura::where('matricula_id', $matricula_id)
                ->whereMonth('fecha', $mes)
                ->whereYear('fecha', $anio);
    ...
    /**
     * Solicitar justificación de asistencia por representante
     */
    public function representanteSolicitarJustificacion(Request $request)
    {
        try {
            ...
            if (!$tieneRelacion) {
                return response()->json(['success' => false, 'message' => 'El estudiante no está asociado con tu cuenta.'], 403);
            }

            // Obtener la matrícula del estudiante para la fecha de la falta
            $matricula = \App\Models\Matricula::where('estudiante_id', $request->estudiante_id)
                ->first();

            if (!$matricula) {
                return response()->json(['success' => false, 'message' => 'El estudiante no tiene matrícula registrada.'], 400);
            }

            // TODO: Verificar que no exista ya una justificación para esa fecha
            // $justificacionExistente = JustificacionAsistencia::where('matricula_id', $matricula->matricula_id)
            //     ->where('fecha_falta', $request->fecha_falta)
            //     ->first();

            // if ($justificacionExistente) {
            //     return response()->json(['success' => false, 'message' => 'Ya existe una justificación para esta fecha.'], 400);
            // }
            ...
            try {
                $admins = \App\Models\Usuario::whereHas('persona.roles', function($query) {
                ...
                foreach ($admins as $admin) {
                    Notificacion::create([
                        ...
                        'titulo' => 'Nueva Justificación Solicitada',
                        'mensaje' => "Se ha solicitado una justificación de inasistencia para el estudiante {$matricula->estudiante->persona->nombres} {$matricula->estudiante->persona->apellidos} correspondiente a la fecha {$request->fecha_falta}.",
                        'datos' => [
                            'justificacion_id' => $justificacion->id,
                ...
                \Log::info('Notificaciones creadas para administradores', ['admins_count' => $admins->count()]);
            } catch (\Exception $notifError) {
                \Log::error('Error creando notificaciones: ' . $notifError->getMessage());
                // No fallar la operación por error en notificaciones
            }

            \Log::info('Justificación solicitada exitosamente', [
                'justificacion_id' => $justificacion->id,
                'estudiante_id' => $request->estudiante_id,
                'representante_id' => $representante->representante_id
            ]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Justificación solicitada exitosamente. Será revisada por el administrador.'
            ]);
            ...
            \Log::error('Error solicitando justificación: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
            return response()->json([
                'success' => false,
    ...
    /**
     * Exportar reporte de asistencia para representante
     */
    public function representanteExportarReporte(Request $request, $estudiante_id)
    {
        try {
            // Verificar autenticación
            if (!Auth::check()) {
            ...
            // Obtener estudiante con sus datos
            $estudiante = \App\Models\InfEstudiante::with([
                'persona',
                'matricula' => function($query) {
                    $query->with(['grado', 'seccion', 'curso.anoLectivo']);
            ...
            $estadisticas = [
                ...
                'tardes' => $asistencias->where('tipoAsistencia.codigo', 'T')->count(),
            ...
            try {
                $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('asistencia.reporte_representante', compact(
                    'estudiante',
                    'asistencias',
                    'estadisticas',
                    'justificaciones',
                ...
                $filename = 'reporte_asistencia_' . $estudiante->persona->nombres . '_' . $estudiante->persona->apellidos . '_' . $mes . '_' . $anio . '.pdf';

                return $pdf->download($filename);

            } catch (\Exception $pdfError) {
                \Log::error('Error generando PDF: ' . $pdfError->getMessage());
                // Fallback: retornar vista HTML con mensaje
                return view('asistencia.reporte_representante', compact(
                    'estudiante',
                    'asistencias',
                    'estadisticas',
                    'justificaciones',
                    ...
                    'mes',
                    'anio'
                ))->with('error_pdf', 'No se pudo generar el PDF automáticamente. Use Ctrl+P para imprimir como PDF.');
            }

        } catch (\Exception $e) {
            \Log::error('Error en representanteExportarReporte: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
