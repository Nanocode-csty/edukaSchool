        ...
        try {
            ...
            $estudiantes = $sesion->cursoAsignatura->curso->matriculas()
                ...
                ->map(function($matricula) use ($sesion) {
                    // Verificar si ya tiene asistencia registrada para esta sesión
                    ...
                    $matricula->asistencia_id = $asistenciaExistente ? $asistenciaExistente->asistencia_id : null;
    ...
    public function guardarReporteGenerado(Request $request)
    {
        try {
            ...
            if ($request->formato === 'pdf') {
                ...
                // Return test response to check if variables are available
                return response()->json([
                    'success' => true,
                    'message' => 'Test response - variables available',
                    'data' => [
                        'curso_id' => $curso->curso_id,
                        'curso_name' => $curso->grado->nombre . ' ' . $curso->seccion->nombre,
                        'fecha_inicio' => $fecha_inicio,
                        'fecha_fin' => $fecha_fin,
                        'asistencias_count' => $asistencias->count()
                    ]
                ...
                // Test if view exists and can be loaded
                try {
                    $pdf = Pdf::loadView('asistencia.reportes.docente-curso-pdf', compact('curso', 'asistencias', 'fecha_inicio', 'fecha_fin'));
