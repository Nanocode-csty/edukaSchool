...
class AsistenciaController extends Controller
{
    ...
    {
        try {
            // Use Eloquent relationships instead of raw joins to avoid column issues
            ...
            $totalPresentes = $asistenciasAll->where('tipoAsistencia.codigo', 'A')->count();
            $totalAusentes = $asistenciasAll->where('tipoAsistencia.codigo', 'F')->count();
            ...
            $asistencias = $query->orderBy('asistenciasasignatura.fecha', 'desc')->paginate(20);
    ...
    public function exportarPDFAdmin(Request $request)
    {
        ...
        try {
            ...
            // Si es una petición AJAX, guardar el reporte y devolver JSON con URL
            if ($request->ajax() || $request->expectsJson()) {
                ...
                $reporteGenerado = ReporteGenerado::create([
                    ...
                    'fecha_fin' => $fechaFin,
                    'formato' => 'pdf', // Cambiar a minúsculas para consistencia
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            if ($request->ajax() || $request->expectsJson()) {
                // Generar nombre único para el archivo
                $nombreArchivo = 'reporte_asistencia_' . $request->fecha_inicio . '_a_' . $request->fecha_fin . '_' . time() . '.xlsx';
                $rutaArchivo = 'reportes/' . $nombreArchivo;
            ...
            $reporteGenerado = ReporteGenerado::create([
                'usuario_id' => Auth::id(),
                'tipo_reporte' => $this->generarNombreReporte($request->tipo_reporte, 'xlsx', $request->fecha_inicio, $request->fecha_fin),
                'fecha_inicio' => $fechaInicio,
                'fecha_fin' => $fechaFin,
                'formato' => 'xlsx',
                'registros_totales' => $asistencias->count(),
                'archivo_path' => $rutaArchivo,
                'archivo_nombre' => $this->generarNombreReporte($request->tipo_reporte, 'xlsx', $request->fecha_inicio, $request->fecha_fin),
                'fecha_generacion' => now(),
                'filtros_aplicados' => json_encode($request->all())
            ]);
            ...
            // Guardar el reporte en la base de datos con el archivo físico
            ...
            \Log::info('EXPORTANDO EXCEL - Guardando archivo y reporte', [
            ...
            \Log::info('EXCEL EXPORTADO - Archivo y reporte guardados', [
                'reporte_id' => $reporteGenerado->id,
                'tipo_reporte_guardado' => $reporteGenerado->tipo_reporte,
                'formato' => $reporteGenerado->formato,
                'archivo_path' => $reporteGenerado->archivo_path
            ]);
            ...
            // Leer y enviar el archivo guardado
            readfile(storage_path('app/public/' . $rutaArchivo));
            exit;
    ...
    public function descargarReporteHistorial($reporteId)
    {
        ...
        try {
            ...
            // Si el archivo ya existe, descargarlo directamente
            if ($reporte->archivo_path && file_exists(storage_path('app/public/' . $reporte->archivo_path))) {
                $nombreArchivo = $reporte->archivo_nombre ?: 'reporte_asistencia.' . strtolower($reporte->formato);
                return response()->download(
                    storage_path('app/public/' . $reporte->archivo_path),
                    $nombreArchivo
                );
            }

            // Si no existe el archivo, intentar regenerarlo
            ...
            $requestData = array_merge($filtros, [
                ...
                'formato' => strtolower($reporte->formato) // Convertir a minúsculas para consistencia
            ]);
    ...
    public function guardarReporteExcel(Request $request)
    {
        ...
        try {
            ...
            // Debug log - more detailed
            \Log::info('Guardando reporte Excel automatico desde vista previa', [
                'tipo_reporte_original' => $tipoReporteOriginal,
                'nombre_descriptivo_generado' => $nombreDescriptivo,
                'fecha_inicio' => $request->fecha_inicio,
                'fecha_fin' => $request->fecha_fin,
            ...
            \Log::info('Reporte Excel automatico guardado exitosamente', [
                'reporte_id' => $reporte->id,
                'tipo_reporte_guardado' => $reporte->tipo_reporte,
            ...
            return response()->json([
                'success' => true,
                'message' => 'Reporte Excel guardado automáticamente.',
                'reporte_id' => $reporte->id
            ]);

        } catch (\Exception $e) {
            \Log::error('Error guardando reporte Excel automatico', [
                'error' => $e->getMessage(),
                'request' => $request->all()
            ]);
            return response()->json([
                'success' => false,
                'message' => 'Error al guardar el reporte Excel: ' . $e->getMessage()
            ], 500);
        }
    }
