...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            // Debug: Log user and persona data
            \Log::info('Profile update - User and Persona data', [
                'user_id' => $user->usuario_id,
                'persona_id' => $user->persona_id,
                'persona_exists' => $persona ? 'YES' : 'NO',
                'persona_foto_url_before' => $persona ? $persona->foto_url : 'N/A',
                'has_file_photo' => $request->hasFile('photo'),
                'file_photo_valid' => $request->hasFile('photo') ? $request->file('photo')->isValid() : 'N/A',
            ...
            if ($request->hasFile('photo') && $request->file('photo')->isValid()) {
                \Log::info('Photo upload detected in update method', [
                ...
                if ($persona->foto_url && Storage::disk('public')->exists($persona->foto_url)) {
                    Storage::disk('public')->delete($persona->foto_url);
                    \Log::info('Old photo deleted', ['old_path' => $persona->foto_url]);
                }

                // Guardar nueva foto
                $path = $request->file('photo')->store('profile-photos', 'public');
                $personaData['foto_url'] = $path;
                \Log::info('New photo saved', ['new_path' => $path, 'full_url' => asset('storage/' . $path)]);
            } else {
                \Log::info('No photo file detected in update method', [
                    'has_file' => $request->hasFile('photo'),
                    'is_valid' => $request->hasFile('photo') ? $request->file('photo')->isValid() : 'N/A'
                ]);
            }
     ...
     */
    private function getRoleSpecificValidationRules(User $user)
    {
        $rules = [];
     ...
     * Actualizar datos específicos del rol
     */
    private function updateRoleSpecificData(User $user, array $validatedData)
    {
