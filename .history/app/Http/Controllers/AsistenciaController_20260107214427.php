...
class AsistenciaController extends Controller
{
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            foreach ($asistencias as $asistencia) {
                ...
                $sheet->setCellValue('C' . $row, trim($apellidos));
    ...
    /**
     * Ver detalles de un reporte generado
     */
    public function verReporte($reporteId)
    {
        ...
        try {
            // Buscar el reporte
            $reporte = ReporteGenerado::with(['usuario.persona'])->findOrFail($reporteId);

            // Regenerar los datos del reporte usando los filtros guardados
            // Los filtros están almacenados como JSON, necesitamos decodificarlos
            ...
            // Generar estadísticas usando los mismos filtros
            ...
            if (isset($filtros['tipo_reporte'])) {
                switch ($filtros['tipo_reporte']) {
                    case 'por_curso':
                        if (isset($filtros['curso_id'])) {
                            $curso = InfCurso::find($filtros['curso_id']);
                            if ($curso) {
                                $query->whereHas('matricula', function($q) use ($curso) {
                                    $q->where('idGrado', $curso->grado_id)
                                      ->where('idSeccion', $curso->seccion_id);
                    ...
                    case 'por_estudiante':
                        if (isset($filtros['estudiante_id'])) {
                            $query->whereHas('matricula.estudiante', function($q) use ($filtros) {
                    ...
                    case 'por_docente':
                        if (isset($filtros['docente_id'])) {
                            $query->whereHas('cursoAsignatura', function($q) use ($filtros) {
                                $q->where('profesor_id', $filtros['docente_id']);
                            });
                        }
                        break;
                }
            ...
            $totalTardanzas = $asistencias->where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'T')->first()->id ?? 3)->count();
            $totalJustificados = $asistencias->where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'J')->first()->id ?? 4)->count();
            ...
            $estadisticas = [
                ...
                'total_tardanzas' => $totalTardanzas,
                'total_justificados' => $totalJustificados,
                'porcentaje_asistencia' => $totalRegistros > 0 ? round(($totalPresentes / $totalRegistros) * 100, 1) . '%' : '0%'
            ];

            return view('asistencia.ver-reporte', compact('reporte', 'asistencias', 'estadisticas'));
