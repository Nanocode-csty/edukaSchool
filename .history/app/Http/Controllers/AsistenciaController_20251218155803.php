...
class AsistenciaController extends Controller
{
    ...
    public function reporteGeneral(Request $request)
    {
        ...
        $tipoAsistencia = $request->get('tipo_asistencia');

        // Si no hay fechas, usar los últimos 30 días por defecto
        ...
        $isDocente = $user->rol === 'Profesor';

        // Para docentes, limitar a sus propias asignaturas
        if ($isDocente && $user->docente) {
            $docenteId = $user->docente->profesor_id;

            // Si el docente intenta filtrar por otro profesor, mostrar error
            if ($profesorId && $profesorId != $docenteId) {
                return redirect()->back()->with('error', 'No tiene permisos para ver reportes de otros profesores.');
            }

            // Forzar filtro por el docente actual
            $profesorId = $docenteId;
        }

        // Obtener estudiantes matriculados según el filtro y permisos
        $matriculasQuery = Matricula::whereIn('estado', ['Matriculado', 'Pre-inscrito', 'Regular'])
            ->with(['estudiante', 'curso.grado', 'curso.seccion']);

        if ($cursoId) {
            $matriculasQuery->where('curso_id', $cursoId);
        }

        // Aplicar filtro de profesor si está especificado
        if ($profesorId) {
            $matriculasQuery->whereHas('curso.cursoAsignaturas', function ($q) use ($profesorId) {
                $q->where('profesor_id', $profesorId);
            });
        }

        // Aplicar filtro de asignatura si está especificado
        if ($asignaturaId) {
            $matriculasQuery->whereHas('cursoAsignaturas', function ($q) use ($asignaturaId) {
        ...
        // Si no hay matrículas, mostrar vista básica
        if ($matriculas->isEmpty()) {
            $estudiantes = \App\Models\InfEstudiante::with(['persona'])->get();
            ...
            $datosGrafico = $this->generarDatosGraficoVacios($fechaInicio, $fechaFin);

            $cursos = \App\Models\InfCurso::with(['grado', 'seccion'])->get();
        ...
        $asistenciasQuery = AsistenciaAsignatura::whereBetween('fecha', [$fechaInicio, $fechaFin])
            ->whereIn('matricula_id', $matriculas->pluck('matricula_id'))
            ->with(['tipoAsistencia', 'matricula']);
        ...
        $totalPresentes = $asistencias->filter(function ($a) {
            return optional($a->tipoAsistencia)->codigo === 'A';
        })->count();

        $totalAusentes = $asistencias->filter(function ($a) {
            return optional($a->tipoAsistencia)->codigo === 'F';
        })->count();

        $totalTardanzas = $asistencias->filter(function ($a) {
            return optional($a->tipoAsistencia)->codigo === 'T';
        })->count();

        $totalJustificadas = $asistencias->filter(function ($a) {
            return optional($a->tipoAsistencia)->codigo === 'J';
        })->count();

        // Si no hay registros de asistencia, mostrar estadísticas basadas en estudiantes matriculados
        // Esto es más informativo que mostrar todo en 0
        if ($totalRegistros === 0 && $totalEstudiantes > 0) {
            // Calcular estadísticas estimadas basadas en el comportamiento típico
            // O simplemente mostrar que no hay datos registrados
            $estadisticas = [
                'total_estudiantes' => $totalEstudiantes,
                'total_presentes' => 0, // No hay registros
                'total_ausentes' => 0,  // No hay registros
                'total_tardanzas' => 0, // No hay registros
                'total_justificadas' => 0, // No hay registros
                'total_registros' => 0, // No hay registros
                'porcentaje_general' => 0, // No hay registros para calcular
                'mensaje_info' => 'No hay registros de asistencia para el período seleccionado. Los estudiantes aparecen como matriculados pero no se ha registrado asistencia aún.',
            ];
        } else {
        ...
        if ($cursoId) {
            ...
            if ($curso) {
                $totalEstudiantesCurso = \App\Models\Matricula::where('curso_id', $cursoId)
                    ->whereIn('estado', ['Matriculado', 'Pre-inscrito', 'Regular'])
                    ->count();
                ...
                $justificadas = $asistenciasCurso->filter(function ($a) {
                    return optional($a->tipoAsistencia)->codigo === 'J';
                })->count();

                $totalRegistrosCurso = $asistenciasCurso->count();
                ...
                $estadisticasPorCurso[] = [
                    ...
                    'tardanzas' => $tardanzas,
                    'justificadas' => $justificadas,
                    'total_registros' => $totalRegistrosCurso,
                    'porcentaje' => $totalRegistrosCurso > 0 ? round(($presentes / $totalRegistrosCurso) * 100, 1) : 0,
                ];
            }
        } else {
            ...
            $cursos = \App\Models\InfCurso::with(['grado', 'seccion'])
                ->whereHas('matriculas', function ($q) {
                    $q->whereIn('estado', ['Matriculado', 'Pre-inscrito', 'Regular']);
                })
                ->get();

            foreach ($cursos as $curso) {
                $totalEstudiantesCurso = \App\Models\Matricula::where('curso_id', $curso->curso_id)
                    ->whereIn('estado', ['Matriculado', 'Pre-inscrito', 'Regular'])
                    ->count();

                $asistenciasCurso = $asistencias->filter(function ($a) use ($curso) {
                    return $a->matricula->curso_id == $curso->curso_id;
                });
        ...
        // Datos para gráficos - generar datos para el rango de fechas seleccionado
        $datosGrafico = $this->generarDatosGraficoTendencia($fechaInicio, $fechaFin, $asistencias);
    ...
    /**
     * Generar datos para gráfico de tendencia por días
     */
    private function generarDatosGraficoTendencia($fechaInicio, $fechaFin, $asistencias)
    {
        $datosGrafico = [
            'labels' => [],
            'porcentajes' => [],
        ...
        // Generar datos de tendencia por día para el rango seleccionado
        $periodo = Carbon::parse($fechaInicio)->copy();
        while ($periodo->lte($fechaFin)) {
            $datosGrafico['labels'][] = $periodo->format('d/m');
            ...
            $porcentajeDia = $totalDia > 0 ? round(($presentesDia / $totalDia) * 100, 1) : 0;
            $datosGrafico['porcentajes'][] = $porcentajeDia;
    ...
    /**
     * Generar datos vacíos para gráfico cuando no hay datos
     */
    private function generarDatosGraficoVacios($fechaInicio, $fechaFin)
    {
        $datosGrafico = [
            'labels' => [],
            'porcentajes' => [],
    ...
    /**
     * Dashboard principal de asistencia - redirige según rol del usuario
     */
    public function dashboard(Request $request)
    {
        ...
        switch ($user->rol) {
            ...
            case 'Profesor':
                return $this->dashboardProfesor($request);
            case 'Representante':
                return $this->dashboardRepresentante($request);
            default:
                abort(403, 'Rol no autorizado para acceder al dashboard de asistencia');
        }
    }
    ...
    private function dashboardAdmin(Request $request)
    {
        ...
        $estadisticas = [
            'total_estudiantes' => \App\Models\InfEstudiante::where('situacion_academica', 'Regular')->count(),
            'total_profesores' => \App\Models\InfDocente::where('estado', 'Activo')->count(),
            'total_asistencias_hoy' => \App\Models\AsistenciaAsignatura::whereDate('fecha', today())->count(),
            'total_justificaciones_pendientes' => \App\Models\JustificacionAsistencia::where('estado', 'pendiente')->count(),
        ];

    // Asistencias de hoy
    $asistenciasHoy = \App\Models\AsistenciaAsignatura::with([
        'matricula.estudiante.persona',
        'cursoAsignatura.asignatura',
        'cursoAsignatura.profesor.persona',
        'tipoAsistencia'
    ])
    ...
    ->get();
        ...
        // Alertas recientes
        $alertasRecientes = \App\Models\JustificacionAsistencia::with(['matricula.estudiante.persona'])
            ->where('estado', 'pendiente')
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();

        return view('asistencia.dashboard-admin', compact('estadisticas', 'asistenciasHoy', 'alertasRecientes'));
    }

    /**
     * Dashboard para Profesores
     */
    private function dashboardProfesor(Request $request)
    {
        $user = auth()->user();
        $profesor = $user->docente;

        if (!$profesor) {
            abort(403, 'No se encontró vinculación de docente para este usuario');
        }

        // Sesiones de hoy del profesor
        $sesionesHoy = \App\Models\SesionClase::with(['cursoAsignatura.curso.grado', 'cursoAsignatura.curso.seccion', 'cursoAsignatura.asignatura'])
            ->whereHas('cursoAsignatura', function ($q) use ($profesor) {
                $q->where('profesor_id', $profesor->profesor_id);
            })
            ->whereDate('fecha', today())
            ->orderBy('hora_inicio')
            ->get();
        ...
        $estadisticas = $this->calcularEstadisticasDia($profesor->profesor_id, today()->format('Y-m-d'));

        // Próximas sesiones
        $proximasSesiones = \App\Models\SesionClase::with(['cursoAsignatura.curso.grado', 'cursoAsignatura.curso.seccion', 'cursoAsignatura.asignatura'])
            ->whereHas('cursoAsignatura', function ($q) use ($profesor) {
                $q->where('profesor_id', $profesor->profesor_id);
            })
            ->where('fecha', '>=', today())
            ->where('fecha', '<=', today()->addDays(7))
            ->orderBy('fecha')
            ->orderBy('hora_inicio')
            ->take(10)
            ->get();

        return view('asistencia.dashboard-profesor', compact('sesionesHoy', 'estadisticas', 'proximasSesiones'));
    }

    /**
     * Dashboard para Representantes
     */
    private function dashboardRepresentante(Request $request)
    {
        $user = auth()->user();

        // Obtener estudiantes representados
        $estudiantesRepresentados = \App\Models\InfEstudianteRepresentante::where('representante_id', $user->representante_id)
            ->with(['estudiante'])
            ->get()
            ->map(function ($relacion) {
                if (!$relacion->estudiante) return null;
        ...
        // Estadísticas de asistencia de los estudiantes
        $estadisticas = [];
        foreach ($estudiantesRepresentados as $estudianteData) {
            $matriculaIds = collect($estudianteData['matriculas'])->pluck('matricula_id');

            $asistencias = \App\Models\AsistenciaAsignatura::whereIn('matricula_id', $matriculaIds)
                ->whereMonth('fecha', now()->month)
                ->get();

            $totalAsistencias = $asistencias->count();
            $asistenciasPositivas = $asistencias->filter(function ($asistencia) {
                return optional($asistencia->tipoAsistencia)->codigo !== 'F'; // No faltas
            })->count();

            $porcentaje = $totalAsistencias > 0 ? round(($asistenciasPositivas / $totalAsistencias) * 100, 1) : 0;

            $estadisticas[$estudianteData['estudiante']->estudiante_id] = [
                'total' => $totalAsistencias,
                'positivas' => $asistenciasPositivas,
                'porcentaje' => $porcentaje,
            ];
        }

        // Justificaciones pendientes
        $justificacionesPendientes = \App\Models\JustificacionAsistencia::whereHas('matricula', function ($query) use ($estudiantesRepresentados) {
            $estudianteIds = $estudiantesRepresentados->pluck('estudiante.estudiante_id');
        ...
        return view('asistencia.dashboard-representante', compact('estudiantesRepresentados', 'estadisticas', 'justificacionesPendientes'));
    }
    ...
    public function adminIndex(Request $request)
    {
        ...
        // Filtro por estudiante (desde búsqueda)
        if ($request->filled('estudiante_id')) {
            $query->where('matricula_id', $request->estudiante_id);
            $filtros['estudiante_id'] = $request->estudiante_id;
        }

        // Filtro por búsqueda de estudiante (DNI, nombre, apellido)
        if ($request->filled('filtro_estudiante_tipo') && $request->filled('filtro_estudiante_valor')) {
            $tipo = $request->filtro_estudiante_tipo;
            $valor = $request->filtro_estudiante_valor;

            $query->whereHas('matricula.estudiante.persona', function ($sq) use ($tipo, $valor) {
                $sq->where($tipo, 'like', "%{$valor}%");
            });

            $filtros['filtro_estudiante_tipo'] = $tipo;
            $filtros['filtro_estudiante_valor'] = $valor;
        }
        ...
        if ($request->filled('buscar')) {
            ...
            $query->where(function ($q) use ($buscar) {
                $q->whereHas('matricula.estudiante.persona', function ($sq) use ($buscar) {
                ...
                ->orWhereHas('cursoAsignatura.profesor.persona', function ($sq) use ($buscar) {
                    $sq->where('nombres', 'like', "%{$buscar}%")
                       ->orWhere('apellidos', 'like', "%{$buscar}%");
                });
            });
        ...
        // Datos para los filtros - Solo mostrar entidades relacionadas con registros de asistencia
        ...
        $cursos = \App\Models\InfCurso::with(['grado', 'seccion'])
            ...
            ->whereExists(function ($query) {
                $query->select(\DB::raw(1))
                      ->from('matriculas')
                      ->whereColumn('matriculas.curso_id', 'cursos.curso_id')
                      ->where('matriculas.estado', 'Matriculado')
                      ->whereExists(function ($subQuery) {
                          $subQuery->select(\DB::raw(1))
                                   ->from('asistenciasasignatura')
                                   ->whereColumn('asistenciasasignatura.matricula_id', 'matriculas.matricula_id');
                      });
            })
        ...
        // Get students with their course information - Solo estudiantes con registros de asistencia
        $estudiantes = \App\Models\InfEstudiante::with(['persona', 'matriculas' => function($query) {
                $query->where('estado', 'Matriculado')
                      ...
                      ->with(['curso.grado', 'curso.seccion']);
            }])
    ...
    public function verificar(Request $request)
    {
        ...
        $query = \App\Models\JustificacionAsistencia::with(['matricula.estudiante.persona', 'usuarioCreador']);
        ...
        // Obtener estudiantes que tienen justificaciones
        $estudiantes = \App\Models\InfEstudiante::with(['persona', 'matriculas' => function($query) {
                $query->where('estado', 'Matriculado')
                      ->whereExists(function ($subQuery) {
                          $subQuery->select(\DB::raw(1))
                                   ->from('justificaciones_asistencia')
                                   ->whereColumn('justificaciones_asistencia.matricula_id', 'matriculas.matricula_id');
                      })
                      ->with(['curso.grado', 'curso.seccion']);
            }])
            ->join('personas', 'estudiantes.persona_id', '=', 'personas.id_persona')
        ...
        return view('asistencia.verificar', compact('justificaciones', 'estadisticas', 'estudiantes', 'filtros'));
    }
    ...
    /**
     * API: Obtener estudiantes filtrados por profesor/curso/asignatura
     */
    public function getEstudiantesFiltrados(Request $request)
    {
        $query = \App\Models\InfEstudiante::with(['persona', 'matriculas' => function($query) {
                $query->where('estado', 'Matriculado')
                      ->whereExists(function ($subQuery) {
                          $subQuery->select(\DB::raw(1))
                                   ->from('asistenciasasignatura')
        ...
        if ($request->filled('profesor_id')) {
            $query->whereHas('matriculas.cursoAsignaturas', function ($q) use ($request) {
                $q->where('profesor_id', $request->profesor_id);
            });
        }
        ...
        $estudiantes = $query->select('estudiantes.*')
            ->distinct()
            ->orderBy('personas.apellidos')
            ->orderBy('personas.nombres')
            ->get();
    ...
    /**
     * API: Obtener cursos filtrados por profesor/asignatura
     */
    public function getCursosFiltrados(Request $request)
    {
        $query = \App\Models\InfCurso::with(['grado', 'seccion'])
            ->join('grados', 'cursos.grado_id', '=', 'grados.grado_id')
            ->join('secciones', 'cursos.seccion_id', '=', 'secciones.seccion_id')
            ->whereExists(function ($query) {
                $query->select(\DB::raw(1))
        ...
        if ($request->filled('profesor_id')) {
            $query->whereExists(function ($subQuery) use ($request) {
                $subQuery->select(\DB::raw(1))
                         ->from('cursoasignaturas')
                         ->whereColumn('cursoasignaturas.curso_id', 'cursos.curso_id')
                         ->where('cursoasignaturas.profesor_id', $request->profesor_id);
            });
        }
        ...
        if ($request->filled('asignatura_id')) {
            $query->whereExists(function ($subQuery) use ($request) {
                $subQuery->select(\DB::raw(1))
                         ->from('cursoasignaturas')
                         ->whereColumn('cursoasignaturas.curso_id', 'cursos.curso_id')
                         ->where('cursoasignaturas.asignatura_id', $request->asignatura_id);
            });
        }

        $cursos = $query->select('cursos.*')
            ->distinct()
            ->orderBy('grados.nombre')
            ->orderBy('secciones.nombre')
            ->get();
    ...
    /**
     * API: Obtener asignaturas filtradas por profesor/curso
     */
    public function getAsignaturasFiltradas(Request $request)
    {
        $query = \App\Models\InfAsignatura::whereExists(function ($query) {
                $query->select(\DB::raw(1))
                      ->from('cursoasignaturas')
                      ->whereColumn('cursoasignaturas.asignatura_id', 'asignaturas.asignatura_id')
                      ->whereExists(function ($subQuery) {
    ...
    /**
     * API: Buscar estudiantes por DNI, nombre o apellido
     */
    public function buscarEstudiantes(Request $request)
    {
        ...
        // Base query: estudiantes que tienen registros de asistencia
        $estudiantesQuery = \App\Models\Matricula::with(['estudiante.persona', 'curso.grado', 'curso.seccion'])
            ->join('estudiantes', 'matriculas.estudiante_id', '=', 'estudiantes.estudiante_id')
            ->join('personas', 'estudiantes.persona_id', '=', 'personas.id_persona')
            ->where('matriculas.estado', 'Matriculado')
            ->whereExists(function ($subQuery) {
                $subQuery->select(\DB::raw(1))
                    ->from('asistencia_asignatura')
                    ->whereColumn('asistencia_asignatura.matricula_id', 'matriculas.matricula_id');
            });

        // Aplicar filtros adicionales si existen
        if ($profesorId) {
            $estudiantesQuery->whereHas('cursoAsignaturas', function ($q) use ($profesorId) {
                $q->where('profesor_id', $profesorId);
            });
        }

        if ($cursoId) {
            $estudiantesQuery->where('matriculas.curso_id', $cursoId);
        }

        if ($asignaturaId) {
            $estudiantesQuery->whereHas('cursoAsignaturas', function ($q) use ($asignaturaId) {
                $q->where('asignatura_id', $asignaturaId);
            });
        }
        ...
        $estudiantesQuery->where(function ($q) use ($query) {
            $q->where('personas.dni', 'like', "%{$query}%")
              ->orWhere('personas.nombres', 'like', "%{$query}%")
              ->orWhere('personas.apellidos', 'like', "%{$query}%");
        });
    ...
    /**
     * API: Obtener tabla de asistencias filtrada (para AJAX)
     */
    public function getTablaAsistencias(Request $request)
    {
        ...
        // Filtros (mismo código que en adminIndex)
        $filtros = [];
        ...
        // Devolver vista parcial de la tabla
        return view('asistencia.partials.tabla-asistencias', compact('asistencias', 'estadisticas'))->render();
    }
}
