        ...
        try {
            ...
            if ($request->ajax() || $request->expectsJson()) {
                ...
                $reporteGenerado = ReporteGenerado::create([
                    'usuario_id' => Auth::id(),
                    'tipo_reporte' => $this->generarNombreReporte($request->tipo_reporte, 'pdf', $request->fecha_inicio, $request->fecha_fin),
                    'fecha_inicio' => $fechaInicio,
                    'fecha_fin' => $fechaFin,
                    'formato' => 'PDF',
                    'registros_totales' => $asistencias->count(),
                    'archivo_path' => $rutaArchivo,
                    'archivo_nombre' => $this->generarNombreReporte($request->tipo_reporte, 'pdf', $request->fecha_inicio, $request->fecha_fin),
