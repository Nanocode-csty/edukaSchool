...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            $totalEstudiantesUnicos = $asistenciasAll->unique('matricula.matricula_id')->count();
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            // Optimized query with selected fields only (performance improvement)
            $query = AsistenciaAsignatura::select([
                'asistencias_asignatura.fecha',
                'asistencias_asignatura.tipo_asistencia_id',
                'matriculas.matricula_id',
                'matriculas.idGrado',
                'matriculas.idSeccion',
                'estudiantes.nombres',
                ...
                'grados.nombre as grado_nombre',
                'secciones.nombre as seccion_nombre',
                'tipos_asistencia.nombre as tipo_asistencia_nombre',
                'tipos_asistencia.codigo as tipo_asistencia_codigo',
                'asignaturas.nombre as asignatura_nombre'
            ])
            ->join('matriculas', 'asistencias_asignatura.matricula_id', '=', 'matriculas.matricula_id')
            ->join('estudiantes', 'matriculas.estudiante_id', '=', 'estudiantes.estudiante_id')
            ->join('grados', 'matriculas.idGrado', '=', 'grados.grado_id')
            ->join('secciones', 'matriculas.idSeccion', '=', 'secciones.seccion_id')
            ->join('tipos_asistencia', 'asistencias_asignatura.tipo_asistencia_id', '=', 'tipos_asistencia.id')
            ->leftJoin('curso_asignaturas', 'asistencias_asignatura.curso_asignatura_id', '=', 'curso_asignaturas.curso_asignatura_id')
            ->leftJoin('asignaturas', 'curso_asignaturas.asignatura_id', '=', 'asignaturas.asignatura_id')
            ->whereBetween('asistencias_asignatura.fecha', [$fechaInicio, $fechaFin]);
