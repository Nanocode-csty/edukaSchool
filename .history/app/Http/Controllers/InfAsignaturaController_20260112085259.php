...
class InfAsignaturaController extends Controller
{
    ...
    {
        // Get asignaturas that have attendance records
        $query = InfAsignatura::from('asignaturas as inf_asignaturas')
            ->select('inf_asignaturas.asignatura_id as id', 'inf_asignaturas.nombre')
        ...
        // Join with matriculas for additional filtering
        ...
        if ($needsMatriculaJoin) {
            $query->join('matriculas', 'asistenciasasignatura.matricula_id', '=', 'matriculas.matricula_id');
        ...
        $uniqueAsignaturas = $asignaturas->unique('id')->values();

        return response()->json($uniqueAsignaturas);
    }
}
