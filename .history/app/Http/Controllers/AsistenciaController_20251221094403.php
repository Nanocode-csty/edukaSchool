...
class AsistenciaController extends Controller
{
    ...
    public function representanteIndex()
    {
        ...
        // Verificar que el usuario tenga relación con representante
        if (!Auth::user()->persona || !Auth::user()->persona->representante) {
            abort(500, 'Tu cuenta de representante no está completamente configurada. Contacta al administrador.');
        }
    ...
    if (!$user) {
        abort(500, 'Usuario representante no existe.');
    }
...
if (!$user) {
    abort(500, 'No hay usuario autenticado.');
}
...
if (!$user->persona->representante) {
    abort(500, 'Persona no tiene representante asociado.');
}
