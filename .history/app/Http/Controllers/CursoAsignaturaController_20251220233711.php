...
class CursoAsignaturaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Allow multiple subjects for the same course at the same time (different classrooms)
        $confCurso = CursoAsignatura::where('curso_id', $cursoId)
            ->where('asignatura_id', $asignaturaId) // Same subject would be duplicate
            ->where('dia_semana', $dia)
            ->where(function ($q) use ($inicio, $fin) {
                $q->whereBetween('hora_inicio', [$inicio, $fin])
                    ->orWhereBetween('hora_fin', [$inicio, $fin])
        ...
        if ($confCurso) {
            return redirect()->back()->with('error', 'Esta asignatura ya está programada para este curso en ese horario.')->withInput();
        }
