...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            // Las estadísticas SIEMPRE muestran los totales del conjunto filtrado SIN el filtro de tipo de asistencia
            $statsQuery = AsistenciaAsignatura::with(['tipoAsistencia']);

            // Aplicar TODOS los filtros EXCEPTO el de tipo de asistencia
            if ($request->filled('fecha_inicio') && $request->filled('fecha_fin')) {
                $statsQuery->whereBetween('fecha', [$request->fecha_inicio, $request->fecha_fin]);
            } elseif ($request->filled('fecha_inicio')) {
                $statsQuery->where('fecha', '>=', $request->fecha_inicio);
            } elseif ($request->filled('fecha_fin')) {
                $statsQuery->where('fecha', '<=', $request->fecha_fin);
            }

            // NO aplicar filtro de tipo_asistencia para las estadísticas
            ...
            // Aplicar el MISMO filtro de fecha que la consulta principal
            if ($request->filled('fecha_inicio') && $request->filled('fecha_fin')) {
                $statsQuery->whereBetween('fecha', [$request->fecha_inicio, $request->fecha_fin]);
            } elseif ($request->filled('fecha_inicio')) {
                $statsQuery->where('fecha', '>=', $request->fecha_inicio);
                ...
                // Si no hay filtros de fecha, mostrar todo el historial disponible (igual que la consulta principal)
                // No aplicar ningún filtro de fecha por defecto
            }
            ...
            // Debug detallado de las estadísticas
            ...
            // Verificar si hay registros con computa_falta = 1
            $registrosAusentes = AsistenciaAsignatura::whereHas('tipoAsistencia', function($q) {
                $q->where('computa_falta', 1);
            })->count();
            \Log::info('Total registros marcados como ausentes en BD:', $registrosAusentes);

            // Verificar distribución de tipos de asistencia en el rango de fechas
            $distribucion = AsistenciaAsignatura::with('tipoAsistencia')
                ->where('fecha', '>=', $request->fecha_inicio ?? '2000-01-01')
                ->where('fecha', '<=', $request->fecha_fin ?? '2100-12-31')
                ->selectRaw('tipo_asistencia_id, COUNT(*) as cantidad')
                ->groupBy('tipo_asistencia_id')
                ->get();

            \Log::info('Distribución de tipos de asistencia:', $distribucion->map(function($item) {
                $tipo = $item->tipoAsistencia;
                return [
                    'tipo' => $tipo ? $tipo->codigo : 'N/A',
                    'computa_falta' => $tipo ? $tipo->computa_falta : 'N/A',
