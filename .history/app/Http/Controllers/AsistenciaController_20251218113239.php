...
class AsistenciaController extends Controller
{
    ...
    /**
     * API: Obtener estudiantes por filtros
     */
    public function getEstudiantesPorFiltros(Request $request)
    {
        $query = \App\Models\Matricula::with(['estudiante.persona', 'curso.grado', 'curso.seccion'])
            ->join('estudiantes', 'matriculas.estudiante_id', '=', 'estudiantes.estudiante_id')
            ->join('personas', 'estudiantes.persona_id', '=', 'personas.id_persona')
            ->where('matriculas.estado', 'Matriculado');
        ...
        // Aplicar filtros
        if ($request->filled('profesor_id')) {
            $query->whereHas('cursoAsignaturas', function ($q) use ($request) {
                $q->where('profesor_id', $request->profesor_id);
            });
        }

        if ($request->filled('curso_id')) {
            $query->where('matriculas.curso_id', $request->curso_id);
        }
        ...
        $estudiantes = $query->select('matriculas.*')
            ->distinct()
            ->orderBy('personas.apellidos')
            ->orderBy('personas.nombres')
            ->get();
