...
class AsistenciaController extends Controller
{
    ...
    public function reportes()
    {
        ...
        $anioActual = InfAnioLectivo::where('estado', 'Activo')->first();
        $fechaInicioAnio = $anioActual ? $anioActual->fecha_inicio : now()->startOfYear();
        $fechaFinAnio = $anioActual ? $anioActual->fecha_fin : now()->endOfYear();
        ...
        $totalTardanzas = AsistenciaDiaria::where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'T')->first()->id ?? 3)
            ->whereBetween('fecha', [$fechaInicioAnio, $fechaFinAnio])
            ->count();
        $totalJustificaciones = JustificacionAsistencia::where('estado', 'Aprobada')
            ->whereBetween('fecha_falta', [$fechaInicioAnio, $fechaFinAnio])
            ->count();
