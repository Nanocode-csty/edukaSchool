...
class AsistenciaController extends Controller
{
    ...
    public function reportes()
    {
        ...
        $cursos = InfCurso::with(['grado', 'seccion'])->orderBy('curso_id')->get();
        $estudiantes = InfEstudiante::with(['persona'])->orderBy('estudiante_id')->get();
        $docentes = InfDocente::with(['persona'])->orderBy('profesor_id')->get();
    ...
    public function exportarPDFAdmin(Request $request)
    {
        ...
        try {
            ...
            // Usar AsistenciaDiaria para mostrar todas las asistencias diarias completas
            $query = AsistenciaDiaria::with(['matricula.estudiante.persona', 'matricula.grado', 'matricula.seccion', 'tipoAsistencia']);
            ...
            switch ($tipoReporte) {
                case 'por_curso':
                    if ($request->filled('curso_id')) {
                        // Obtener el curso para filtrar por grado y sección
                ...
                case 'por_docente':
                    // Para AsistenciaDiaria, filtrar por docente requiere buscar sesiones de clase
            ...
            $pdf = Pdf::loadView('asistencia.reportes.admin-pdf', compact('asistencias', 'request', 'fechaGeneracion'));
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            // Calcular estadísticas desde la colección paginada actual
            $currentPageItems = $asistencias->items();
            $totalRegistros = count($currentPageItems);
            $totalPresentes = collect($currentPageItems)->where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'P')->first()->id ?? 1)->count();
            $totalAusentes = collect($currentPageItems)->where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'A')->first()->id ?? 2)->count();
            $totalTardanzas = collect($currentPageItems)->where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'T')->first()->id ?? 3)->count();
            $totalJustificados = collect($currentPageItems)->where('tipo_asistencia_id', TipoAsistencia::where('codigo', 'J')->first()->id ?? 4)->count();
