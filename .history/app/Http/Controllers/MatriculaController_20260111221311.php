...
class MatriculaController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $matricula = Matricula::with(['estudiante.persona', 'grado', 'seccion'])
            ...
            switch ($filtro) {
                ...
                case 'numero_asc':
                    ...
                    break;
                case 'preinscrito':
                    // Para preinscripciones, incluir también el año anterior (para período de preinscripción)
    ...
    public function create(Request $request)
    {
        try {
            ...
            if ($dni) {
                $estudiante = InfEstudiante::whereHas('persona', function($query) use ($dni) {
                    $query->where('dni', $dni)->where('estado', 'Activo');
                })->first();
                ...
                if (!$estudiante) {
                    ...
                    if (!$grado) {
                        ...
                        $secciones = collect(); // Empty collection
                    } else {
            ...
            \Log::error('Error en MatriculaController@create: ' . $e->getMessage() . ' - Line: ' . $e->getLine() . ' - File: ' . basename($e->getFile()));
    ...
    {
        try {
            $matricula = Matricula::with(['estudiante.persona', 'grado', 'seccion'])->findOrFail($id);
    ...
    /**
     * Mostrar información de períodos académicos
     */
    public function periodos()
    {
        try {
            $periodoActual = PeriodoMatricula::getPeriodoActual();
            $todosPeriodos = PeriodoMatricula::getPeriodosAnioActual();
            $proximosPeriodos = PeriodoMatricula::getProximosPeriodos(60); // Próximos 60 días

            // Información adicional para la vista
            $anioLectivoActual = $todosPeriodos->first()?->anoLectivo;
            $puedeCrearPreinscripcion = PeriodoMatricula::puedeCrearMatricula('Pre-inscrito');
            $puedeCrearMatricula = PeriodoMatricula::puedeCrearMatricula('Matriculado');

            return view('cmatricula.periodos', compact(
                'periodoActual',
                'todosPeriodos',
                'proximosPeriodos',
                'anioLectivoActual',
                'puedeCrearPreinscripcion',
                'puedeCrearMatricula'
            ));
        } catch (\Exception $e) {
            return redirect()->route('matriculas.index')
                ->with('error', 'Error al cargar información de períodos.');
        }
    }

    /**
     * API: Obtener información del período actual
     */
        ...
        try {
            ...
            if ($periodoActual) {
                return response()->json([
                    'success' => true,
                    'periodo' => [
                        'id' => $periodoActual->periodo_id,
                        'nombre' => $periodoActual->nombre,
                        'tipo' => $periodoActual->tipo_periodo,
                        'fecha_inicio' => $periodoActual->fecha_inicio->format('Y-m-d'),
                        'fecha_fin' => $periodoActual->fecha_fin->format('Y-m-d'),
                        'descripcion' => $periodoActual->descripcion,
                        'esta_activo' => $periodoActual->estaActivo(),
                        'configuracion' => $periodoActual->configuracion
                    ]
                ]);
            } else {
                return response()->json([
