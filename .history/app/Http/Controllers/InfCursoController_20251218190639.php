...
class InfCursoController extends Controller
{
    ...
    {
        $cursos = InfCurso::with(['grado', 'seccion'])->select('cursos.curso_id as id', 'cursos.grado_id', 'cursos.seccion_id')
            ->join('grados', 'cursos.grado_id', '=', 'grados.grado_id')
            ->join('secciones', 'cursos.seccion_id', '=', 'secciones.seccion_id')
            ->orderBy('grados.descripcion')
            ->orderBy('secciones.nombre')
            ->get()
            ->map(function($curso) {
                return [
                    'id' => $curso->id,
                    'nombre' => $curso->grado->descripcion . ' ' . $curso->seccion->nombre
                ];
            });
