...
class AsistenciaController extends Controller
{
    ...
    {
        try {
            $query = AsistenciaDiaria::with(['matricula.estudiante', 'matricula.curso', 'matricula.seccion', 'tipoAsistencia', 'sesionClase.cursoAsignatura.asignatura', 'sesionClase.cursoAsignatura.docente']);
            ...
            if ($request->filled('justificado')) {
                $query->where('justificado', $request->justificado === '1');
            }

            // Filtros avanzados
            if ($request->filled('nivel_id')) {
                $query->whereHas('matricula.curso.grado.nivel', function($q) use ($request) {
                    $q->where('id', $request->nivel_id);
                });
            }

            if ($request->filled('grado_id')) {
                $query->whereHas('matricula.curso.grado', function($q) use ($request) {
                    $q->where('id', $request->grado_id);
                });
            }
