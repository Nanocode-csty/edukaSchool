...
class AsistenciaController extends Controller
{
    ...
    public function reportes()
    {
        ...
        // Obtener datos para filtros adicionales - solo registros con asistencias
        $niveles = InfNivel::whereHas('grados.cursos.matriculas.asistenciasDiarias')
            ->orderBy('nombre')
            ->get();

        $cursos = InfCurso::whereHas('matriculas.asistenciasDiarias')
            ->with(['grado', 'seccion'])
            ->orderBy('curso_id')
            ->get();

        $estudiantes = InfEstudiante::whereHas('matricula.asistenciasDiarias')
            ->with(['persona'])
            ->orderBy('estudiante_id')
            ->get();

        $docentes = InfDocente::whereHas('cursos.asignaturas.asistenciasAsignatura')
            ->with(['persona'])
            ->orderBy('profesor_id')
            ->get();
