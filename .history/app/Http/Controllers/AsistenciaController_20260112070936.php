...
class AsistenciaController extends Controller
{
    ...
    {
        try {
            // Cache tipo asistencia mapping to avoid repeated queries
            static $tipoAsistenciaMap = null;
            if ($tipoAsistenciaMap === null) {
                $tipoAsistenciaMap = TipoAsistencia::pluck('tipo_asistencia_id', 'codigo')->toArray();
            }

            // Build query with selective eager loading
            $query = AsistenciaAsignatura::select([
                ...
                'personas.nombres',
                'personas.apellidos',
                'grados.descripcion as grado_descripcion',
                'grados.grado_id',
            ...
            ->where('matriculas.estado', 'Activo'); // Only active enrollments

            // Apply date filters
            if ($request->filled('fecha_inicio') && $request->filled('fecha_fin')) {
                $fechaInicio = \Carbon\Carbon::parse($request->fecha_inicio)->startOfDay();
                $fechaFin = \Carbon\Carbon::parse($request->fecha_fin)->endOfDay();
                $query->whereBetween('asistenciasasignatura.fecha', [$fechaInicio, $fechaFin]);
            }

            // Apply attendance type filter
            if ($request->filled('tipo_asistencia') && isset($tipoAsistenciaMap[$request->tipo_asistencia])) {
                $query->where('asistenciasasignatura.tipo_asistencia_id', $tipoAsistenciaMap[$request->tipo_asistencia]);
            }
            ...
            if ($request->filled('curso_id')) {
                ...
                if ($curso) {
                    $query->where('matriculas.idGrado', $curso->grado_id)
            ...
            if ($request->filled('estudiante_id')) {
                $query->where('estudiantes.estudiante_id', $request->estudiante_id);
            }
            ...
            if ($request->filled('nivel_id')) {
                $query->where('grados.nivel_id', $request->nivel_id);
            }
            ...
            $asistencias = $query->orderBy('asistenciasasignatura.fecha', 'desc')
                                ->orderBy('asistenciasasignatura.hora_registro', 'desc')
                                ->paginate(20);

            // Get statistics with a separate optimized query
            ...
            $stats = $statsQuery->selectRaw('
                    ...
                    SUM(CASE WHEN tiposasistencia.codigo = "T" THEN 1 ELSE 0 END) as total_tardanzas,
                    SUM(CASE WHEN tiposasistencia.codigo = "J" THEN 1 ELSE 0 END) as total_justificados,
                    AVG(COALESCE(tiposasistencia.factor_asistencia, 0)) as promedio_factor
                ')->first();

            $porcentajeAsistencia = $stats->total_registros > 0
                ? round(($stats->promedio_factor * 100), 1) . '%'
                : '0%';

            // Transform data for frontend
            $data = $asistencias->getCollection()->map(function ($asistencia) {
                return [
                    ...
                    'idGrado' => $asistencia->grado_id,
                    'idSeccion' => $asistencia->seccion_id,
                    'grado_descripcion' => $asistencia->grado_descripcion ?? 'Sin grado',
                    'seccion_nombre' => $asistencia->seccion_nombre ?? 'Sin sección',
                    'nombres' => $asistencia->nombres ?? 'Sin nombre',
                    'apellidos' => $asistencia->apellidos ?? 'Sin apellidos',
                    'grado_nombre' => $asistencia->grado_descripcion ?? 'Sin grado',
                    'seccion_nombre' => $asistencia->seccion_nombre ?? 'Sin sección',
                    'asignatura_nombre' => $asistencia->asignatura_nombre ?? 'Sin asignatura',
                    'tipo_asistencia_nombre' => $asistencia->tipo_asistencia_nombre ?? 'Sin tipo',
                    'tipo_asistencia_codigo' => $asistencia->tipo_asistencia_codigo ?? 'N/A',
                    'estado' => $asistencia->estado ?? 'Activo',
                    'justificado' => ($asistencia->tipo_asistencia_codigo === 'J') ? true : false,
                ];
            });

            // Replace collection with transformed data
            $asistencias->setCollection($data);

            $estadisticas = [
                'total_registros' => (int)$stats->total_registros,
                'total_presentes' => (int)$stats->total_presentes,
                'total_ausentes' => (int)$stats->total_ausentes,
                'total_tardanzas' => (int)$stats->total_tardanzas,
                'total_justificados' => (int)$stats->total_justificados,
                'porcentaje_asistencia' => $porcentajeAsistencia
            ];
            ...
            return response()->json([
                ...
                'estadisticas_adicionales' => [
                    'total_estudiantes_unicos' => 0, // Simplified for performance
                    'promedio_asistencia_diaria' => 0, // Simplified for performance
                    'dias_analizados' => 0, // Simplified for performance
                    'estudiantes_riesgo' => []
                ],
                'filtros_aplicados' => []
            ]);
