...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            // Calcular estadísticas de manera más simple y eficiente
            $filteredQuery = clone $query;
            $totalRegistros = $filteredQuery->count();

            // Calcular presentes y ausentes usando relaciones Eloquent
            $totalPresentes = $filteredQuery->whereHas('tipoAsistencia', function($q) {
                $q->where('computa_falta', 0);
            })->count();

            $totalAusentes = $filteredQuery->whereHas('tipoAsistencia', function($q) {
