...
class AsistenciaController extends Controller
{
    ...
    /**
     * Vista principal para representantes - Muestra lista de estudiantes y estadísticas generales
     * Permite solicitar justificaciones y ver detalles de asistencia
     */
    public function representanteIndex()
    {
        // TEMPORAL: Bypass authentication for testing
        // Verificar permisos
        ...
        if (!Auth::check()) {
            return redirect()->route('login')->with('error', 'Debes estar autenticado como representante para ver esta página.');
        }

        // Verificar que el usuario tiene una persona asociada
        if (!Auth::user()->persona) {
            abort(500, 'Usuario no tiene persona asociada.');
        }

        // Verificar que la persona tiene un representante asociado
        if (!Auth::user()->persona->representante) {
            abort(403, 'Tu usuario no está configurado como representante. Contacta al administrador.');
        }
        ...
        $estudiantes = $representante->estudiantes()
            ->with(['matricula.curso.grado', 'matricula.seccion', 'matricula.curso.anoLectivo'])
        ...
        $estadisticasGenerales = [
            ...
            'total_asistencias_mes' => $estudiantes->sum(function($e) {
                return AsistenciaDiaria::where('matricula_id', $e->matricula->matricula_id)
