...
class AsistenciaController extends Controller
{
    ...
    public function reportes()
    {
        ...
        // Todos los docentes activos (no solo los que tienen asistencia, para que aparezcan en el filtro)
    ...
    {
        try {
            // Usar AsistenciaDiaria para estadísticas filtradas (datos reales de asistencia diaria)
            $query = AsistenciaDiaria::with(['matricula.estudiante.persona', 'matricula.grado', 'matricula.seccion', 'tipoAsistencia', 'sesionClase.cursoAsignatura.asignatura', 'sesionClase.cursoAsignatura.curso']);
            ...
            switch ($tipoReporte) {
                case 'por_curso':
                    if ($request->filled('curso_id')) {
                        $query->whereHas('sesionClase.cursoAsignatura', function($q) use ($request) {
                            $q->where('curso_id', $request->curso_id);
                        });
                    }
                    if ($request->filled('seccion_id')) {
                        $query->whereHas('sesionClase.cursoAsignatura.curso', function($q) use ($request) {
                            $q->where('seccion_id', $request->seccion_id);
                        });
                    }
                    break;
                ...
                default: // general
                    ...
                    if ($request->filled('nivel_id')) {
                        $query->whereHas('sesionClase.cursoAsignatura.curso.grado', function($q) use ($request) {
                            $q->where('nivel_id', $request->nivel_id);
                        });
                    }
                    break;
            ...
            for ($i = 0; $i < 12; $i++) {
                ...
                $presentesMes = $asistenciasMes->where('tipoAsistencia.codigo', 'P')->count();
            ...
            $distribucionTipos = [
                'presente' => $asistenciasFiltradas->where('tipoAsistencia.codigo', 'P')->count(),
                'ausente' => $asistenciasFiltradas->where('tipoAsistencia.codigo', 'A')->count(),
    ...
    {
        try {
            // Usar AsistenciaDiaria para vista previa (datos reales de asistencia diaria)
            $query = AsistenciaDiaria::with(['matricula.estudiante.persona', 'matricula.grado', 'matricula.seccion', 'tipoAsistencia', 'sesionClase.cursoAsignatura.asignatura', 'sesionClase.cursoAsignatura.curso']);
            ...
            // Calcular estadísticas desde la colección completa usando códigos correctos
            $totalRegistros = $asistencias->count();
            $totalPresentes = $asistencias->where('tipoAsistencia.codigo', 'A')->count(); // A = Asistió (Presente)
            $totalAusentes = $asistencias->where('tipoAsistencia.codigo', 'F')->count(); // F = Falta (Ausente)
            $totalTardanzas = $asistencias->where('tipoAsistencia.codigo', 'T')->count(); // T = Tardanza
            $totalJustificados = $asistencias->where('tipoAsistencia.codigo', 'J')->count(); // J = Falta Justificada
