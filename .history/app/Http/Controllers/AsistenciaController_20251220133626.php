...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            // TEMPORAL: Obtener todos los registros sin filtros para debugging
            $asistencias = AsistenciaAsignatura::with(['matricula.estudiante.persona', 'matricula.grado', 'matricula.seccion', 'tipoAsistencia', 'cursoAsignatura.asignatura'])
                ->orderBy('fecha', 'desc')
                ->orderBy('matricula_id')
                ->take(50) // Limitar para testing
                ->get();

            \Log::info('AsistenciaController - getTablaAsistencias: Total records found: ' . $asistencias->count());
