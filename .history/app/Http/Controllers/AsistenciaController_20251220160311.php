...
class AsistenciaController extends Controller
{
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            // Crear el archivo Excel usando la API antigua
            try {
                return Excel::create($archivoNombre, function($excel) use ($query, $filtrosAplicados) {
                    $excel->sheet('Asistencia', function($sheet) use ($query, $filtrosAplicados) {
                ...
                \Log::error('Exportar Excel - Error en creación de Excel', [
                    'error' => $e->getMessage(),
                    'line' => $e->getLine(),
                    'file' => $e->getFile()
                ]);
