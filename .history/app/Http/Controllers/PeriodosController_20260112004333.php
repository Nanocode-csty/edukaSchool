...
class PeriodosController extends Controller
{
    ...
    public function create()
    {
        ...
        if (request('ajax') && request('anio_lectivo_id')) {
            $anioId = request('anio_lectivo_id');

            // Si solo se solicita el nombre del año
            if (request('obtener_anio')) {
                $anio = InfAnioLectivo::find($anioId);
                return response()->json([
                    'nombre_anio' => $anio ? $anio->nombre : null
                ]);
            }

            // Obtener tipos disponibles/bloqueados
            $anio = InfAnioLectivo::with(['periodos' => function($query) {
                $query->select('ano_lectivo_id', 'tipo_periodo', 'estado');
            }])->find($anioId);

            if ($anio) {
                // Contar períodos por tipo
                $periodosPorTipo = $anio->periodos->groupBy('tipo_periodo');

                // Tipos de período disponibles
                $tiposDisponibles = ['PREINSCRIPCION', 'INSCRIPCION', 'MATRICULA', 'ACADEMICO', 'CIERRE'];

                // Determinar qué tipos están disponibles y cuáles bloqueados
                $tiposBloqueados = [];
                $tiposDisponiblesFinal = [];

                foreach ($tiposDisponibles as $tipo) {
                    $periodosDeTipo = $periodosPorTipo->get($tipo, collect());

                    // Si ya existe un período activo de este tipo, está bloqueado
                    $tieneActivo = $periodosDeTipo->contains('estado', 'ACTIVO');

                    if ($tieneActivo) {
                        $tiposBloqueados[] = $tipo;
                    } else {
                        $tiposDisponiblesFinal[] = $tipo;
                    }
                }

                return response()->json([
                    'tipos_disponibles' => $tiposDisponiblesFinal,
                    'tipos_bloqueados' => $tiposBloqueados
                ]);
            }

            return response()->json(['error' => 'Año lectivo no encontrado'], 404);
        }

        // Obtener años lectivos con información de períodos existentes
        $aniosLectivos = InfAnioLectivo::orderBy('nombre')
            ->with(['periodos' => function($query) {
                $query->select('ano_lectivo_id', 'tipo_periodo', 'estado');
            }])
            ->get()
            ->map(function($anio) {
                ...
                // El año lectivo aparece si:
                // 1. No tiene ningún período creado, O
                // 2. Tiene algunos tipos de período creados pero no TODOS los tipos disponibles
                $tiposCreados = $periodosPorTipo->keys()->toArray();
                $totalTiposCreados = count($tiposCreados);
                $totalPeriodosActivos = $anio->periodos->where('estado', 'ACTIVO')->count();

                // Aparece si no tiene todos los tipos de período creados
                $debeAparecer = $totalTiposCreados < count($tiposDisponibles);

                $anio->tipos_disponibles = $tiposDisponiblesFinal;
                $anio->tipos_bloqueados = $tiposBloqueados;
                $anio->debe_aparecer = $debeAparecer;
                $anio->periodos_activos = $totalPeriodosActivos;

                return $anio;
            })
            ->filter(function($anio) {
                return $anio->debe_aparecer;
            })
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'ano_lectivo_id' => 'required|exists:anoslectivos,ano_lectivo_id',
