...
class InfDocenteController extends Controller
{
    ...
    {
        try {
            // Filter for teachers with persona_id (check if estado field exists)
            $docentes = InfDocente::whereNotNull('persona_id')
                ->with('persona')
                ->get();
            ...
            $result = $docentes->map(function($docente) {
                ...
                if (empty($nombres) && isset($docente->nombres)) {
                    $nombres = $docente->nombres;
                }
                if (empty($apellidos) && isset($docente->apellidos)) {
                ...
                if (empty($nombres) && empty($apellidos)) {
                    $nombres = 'Docente';
                    $apellidos = 'ID: ' . $docente->profesor_id;
                }

                return [
                    'id' => $docente->profesor_id,
                    'nombres' => $nombres,
                    'apellidos' => $apellidos
                ];
            })
