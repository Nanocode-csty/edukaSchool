...
class AsistenciaController extends Controller
{
    ...
    public function adminIndex(Request $request)
    {
        ...
        if ($request->filled('buscar_estudiante')) {
            $buscarEstudiante = $request->buscar_estudiante;
            $query->whereHas('matricula.estudiante.persona', function ($sq) use ($buscarEstudiante) {
                $sq->where('dni', 'like', "%{$buscarEstudiante}%")
                   ->orWhere('nombres', 'like', "%{$buscarEstudiante}%")
                   ->orWhere('apellidos', 'like', "%{$buscarEstudiante}%");
            });
            $filtros['buscar_estudiante'] = $buscarEstudiante;
        }
