...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
                ...
                switch ($request->tipo_reporte) {
                    ...
                    case 'comparativo':
                        if ($request->filled('nivel_id')) {
                            $query->join('grados as g2', 'matriculas.idGrado', '=', 'g2.grado_id')
                                  ->where('g2.nivel_id', $request->nivel_id);
                        }
                        if ($request->filled('curso_id')) {
                            $curso = InfCurso::find($request->curso_id);
                            if ($curso) {
            ...
            $asistenciasAll = $query->orderBy('asistenciasasignatura.fecha', 'desc')->get();

            $totalRegistros = $asistenciasAll->count();
            $totalPresentes = $asistenciasAll->where('tipo_asistencia_codigo', 'A')->count();
            $totalAusentes = $asistenciasAll->where('tipo_asistencia_codigo', 'F')->count();
            $totalTardanzas = $asistenciasAll->where('tipo_asistencia_codigo', 'T')->count();
            $totalJustificados = $asistenciasAll->where('tipo_asistencia_codigo', 'J')->count();
            ...
            $asistencias->getCollection()->transform(function ($asistencia) {
                return [
                    ...
                    'apellidos' => ($asistencia->matricula->estudiante->persona->apellido_paterno ?? '') . ' ' . ($asistencia->matricula->estudiante->persona->apellido_materno ?? ''),
        ...
        try {
            ...
            if ($request->ajax() || $request->expectsJson()) {
                ...
                $reporteGenerado = ReporteGenerado::create([
                    'usuario_id' => Auth::id(),
                    'tipo_reporte' => 'PDF Administrativo',
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            // Optimized query with selected fields only (performance improvement)
            ...
            ->leftJoin('cursoasignaturas', 'asistenciasasignatura.curso_asignatura_id', '=', 'cursoasignaturas.curso_asignatura_id')
            ->leftJoin('asignaturas', 'cursoasignaturas.asignatura_id', '=', 'asignaturas.asignatura_id')
            ->whereBetween('asistenciasasignatura.fecha', [$fechaInicio, $fechaFin]);
            ...
            foreach ($asistencias as $asistencia) {
                ...
                $sheet->setCellValue('B' . $row, $asistencia->nombres ?? '');
                $sheet->setCellValue('C' . $row, $asistencia->apellidos ?? '');
                $sheet->setCellValue('D' . $row, $asistencia->grado_nombre ?? '');
                $sheet->setCellValue('E' . $row, $asistencia->seccion_nombre ?? '');
                $sheet->setCellValue('F' . $row, $asistencia->asignatura_nombre ?? '');
                $sheet->setCellValue('G' . $row, $asistencia->tipo_asistencia_nombre ?? '');
                $row++;
            }
