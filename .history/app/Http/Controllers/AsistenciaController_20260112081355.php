...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            // Calcular estadísticas aplicando TODOS los filtros EXCEPTO el de tipo de asistencia
            $statsQuery = AsistenciaAsignatura::with(['tipoAsistencia']);

            // Aplicar TODOS los filtros usando la misma lógica que la query principal
            $fechasDefault = session('asistencia_fechas_default', []);
            ...
            // Aplicar TODOS los otros filtros EXCEPTO tipo de asistencia
            if ($request->filled('curso_id')) {
                $curso = InfCurso::find($request->curso_id);
                if ($curso) {
