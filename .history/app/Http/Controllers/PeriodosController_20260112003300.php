    ...
    public function create()
    {
        ...
        $aniosLectivos = InfAnioLectivo::orderBy('nombre')
            ...
            ->map(function($anio) {
                ...
                // 1. No tiene ningún período, O
                // 2. Tiene algunos períodos pero no todos los tipos activos (puede crear más)
                $totalPeriodosActivos = $anio->periodos->where('estado', 'ACTIVO')->count();
                $totalPeriodosCreados = $anio->periodos->count();

                // Aparece si no tiene todos los tipos activos (máximo 5 tipos activos)
                $debeAparecer = $totalPeriodosActivos < count($tiposDisponibles);
