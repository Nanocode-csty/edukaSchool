...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            // Calcular estadísticas usando CLONACIÓN de la query principal pero SIN filtro de tipo de asistencia
            $statsQuery = clone $query;

            // REMOVER el filtro de tipo de asistencia de las estadísticas para mostrar el total general
            // Esto se hace eliminando la condición where('tipo_asistencia_id', ...)
            $statsQuery->getQuery()->wheres = array_filter($statsQuery->getQuery()->wheres, function($where) {
                return !isset($where['column']) || $where['column'] !== 'tipo_asistencia_id';
            $statsFechaFin = $request->filled('fecha_fin') ? $request->fecha_fin : ($fechasDefault['fecha_fin'] ?? null);
