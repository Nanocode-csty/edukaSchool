...
class AsistenciaController extends Controller
{
    ...
    public function docenteEstadisticas()
    {
        try {
            ...
            $totalPresentes = $asistenciasDocente->whereHas('tipoAsistencia', function($q) {
                $q->where('computa_falta', 0);
            })->count();
            $totalAusentes = $asistenciasDocente->whereHas('tipoAsistencia', function($q) {
                $q->where('computa_falta', 1);
            })->count();

            $estadisticas = [
                'asistencias_por_mes' => [], // Implementar lógica para estadísticas mensuales
                'asistencia_promedio' => $totalAsistencias > 0 ? round(($totalPresentes / $totalAsistencias) * 100, 1) : 0,
