...
class AsistenciaController extends Controller
{
    ...
    public function representanteIndex()
    {
        ...
        $debug = [
            ...
            'persona_id' => Auth::user()->persona_id ?? null,
            'has_persona' => Auth::user()->persona ? 'yes' : 'no',
            'has_representante_relation' => Auth::user()->persona && Auth::user()->persona->representante ? 'yes' : 'no',
            'hasRole_Representante' => Auth::user()->hasRole('Representante') ? 'yes' : 'no',
            'roles' => Auth::user()->getRoleNames()
        ];

        \Log::info('Representante Index Access Debug', $debug);

        // Verificar permisos
        if (!in_array(Auth::user()->rol, ['Representante'])) {
            \Log::warning('Access denied to representante section', $debug);
            abort(403, 'No tienes permisos para acceder a esta sección.');
        }
