...
class AsistenciaController extends Controller
{
                ...
                $estudiantes = $sesion->cursoAsignatura->curso->matriculas()
                    ...
                    ->map(function($matricula) {
                        ...
                        $asistenciaExistente = \App\Models\AsistenciaAsignatura::where('matricula_id', $matricula->matricula_id)
                            ->where('curso_asignatura_id', $matricula->curso_asignatura_id)
                            ->where('fecha', $matricula->fecha)
                            ->first();
    ...
    {
        \Log::info('=== INICIANDO GUARDADO DE ASISTENCIA ===');
        \Log::info('Todos los datos del request:', $request->all());
        \Log::info('Headers del request:', $request->headers->all());
        \Log::info('Método del request:', $request->method());
        \Log::info('URL del request:', $request->fullUrl());

        // Asegurar que siempre se devuelva JSON
        try {
            // Verificar que el request sea AJAX/JSON
            if (!$request->expectsJson()) {
                \Log::warning('Request no espera JSON');
                return response()->json(['success' => false, 'message' => 'Request inválido'], 400);
            }

            // Log detallado de cada campo
            \Log::info('Campo sesion_clase_id:', [
                'valor' => $request->input('sesion_clase_id'),
                'tipo' => gettype($request->input('sesion_clase_id'))
            ]);
            \Log::info('Campo asistencias:', [
                'valor' => $request->input('asistencias'),
                'tipo' => gettype($request->input('asistencias')),
                'es_array' => is_array($request->input('asistencias')),
                'count' => is_array($request->input('asistencias')) ? count($request->input('asistencias')) : 'N/A'
            ]);

            // Verificar datos básicos antes de proceder
            if (!$request->has('sesion_clase_id') || !$request->has('asistencias')) {
                \Log::error('Datos básicos faltantes');
                return response()->json([
                    'success' => false,
                    'message' => 'Datos básicos faltantes'
                ], 400);
            }
            // Verificar autenticación
            if (!Auth::check()) {
