...
class AsistenciaController extends Controller
{
    ...
    public function getTablaAsistencias(Request $request)
    {
        try {
            ...
            \Log::info('Preparing simple JSON response...');

            // For now, return a simple response to test if the issue is with pagination/data
            return response()->json([
                'success' => true,
                'message' => 'Data processed successfully',
                'total_records' => $totalRegistros,
                'paginated_count' => $asistencias->count(),
                'debug' => [
                    'has_data' => $asistencias->isNotEmpty(),
                    'first_item_has_matricula' => $asistencias->isNotEmpty() ? ($asistencias->first()->matricula ? true : false) : null
                ]
            ]);
