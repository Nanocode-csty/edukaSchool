...
class AsistenciaController extends Controller
{
    ...
    public function docenteEditarAsistencia($sesionClase)
    {
        try {
            ...
            // Obtener estudiantes con sus asistencias actuales
            $estudiantes = $sesion->cursoAsignatura->curso->matriculas()
                ->with('estudiante.persona')
                ->where('estado', 'Matriculado')
                ->get()
    ...
    public function docenteGuardarAsistencia(Request $request)
    {
        ...
        try {
            ...
            if ($esEdicion) {
                ...
                // En modo edición, procesar TODOS los estudiantes de la sesión
                $estudiantesSesion = \App\Models\Matricula::where('idGrado', $sesion->cursoAsignatura->curso->grado_id)
                    ->where('idSeccion', $sesion->cursoAsignatura->curso->seccion_id)
                    ->where('estado', 'Matriculado')
                    ->get();

                \Log::info('Estudiantes en la sesión:', $estudiantesSesion->count());

                foreach ($estudiantesSesion as $matricula) {
                    // Buscar si este estudiante tiene asistencia enviada en el formulario
                    $asistenciaData = collect($request->asistencias)->firstWhere('matricula_id', $matricula->matricula_id);
                    ...
                    if ($asistenciaData) {
                        ...
                        \Log::warning("Estudiante {$matricula->matricula_id} no tiene asistencia en el formulario");
                    }
                }
            } else {
