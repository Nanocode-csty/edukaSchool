...
class AsistenciaController extends Controller
{
    ...
    public function procesarVerificacion(Request $request)
    {
        try {
            ...
            $nuevoEstado = $request->accion === 'Aprobar' ? 'aprobado' : 'rechazada';
            ...
            if ($nuevoEstado === 'aprobado') {
                $tipoAsistenciaJustificada = TipoAsistencia::where('codigo', 'J')->first();

                if ($tipoAsistenciaJustificada) {
                    AsistenciaAsignatura::create([
                        'matricula_id' => $justificacion->matricula_id,
                        'curso_asignatura_id' => $justificacion->curso_asignatura_id,
                        'tipo_asistencia_id' => $tipoAsistenciaJustificada->tipo_asistencia_id,
                        'fecha' => $justificacion->fecha_falta,
                        'hora_registro' => now()->format('H:i:s'),
                        'justificacion' => 'Justificación aprobada: ' . ($request->observaciones ?: 'Sin observaciones'),
                        'estado' => 'Justificada',
