...
class AsistenciaController extends Controller
{
    ...
    public function docenteDashboard()
    {
        try {
            ...
            $estadisticas = [
                ...
                'clases_hoy' => \App\Models\SesionClase::whereHas('cursoAsignatura', function($query) use ($docente) {
                    $query->where('profesor_id', $docente->profesor_id);
                })->whereDate('fecha', today())->count(),
                'asistencias_pendientes' => \App\Models\SesionClase::whereHas('cursoAsignatura', function($query) use ($docente) {
                    $query->where('profesor_id', $docente->profesor_id);
                })->whereDate('fecha', today())->where('estado', 'Programada')->count(),
            ];

            return view('asistencia.docente-dashboard', compact('estadisticas'));
     ...
     */
    public function docenteTomarAsistencia()
    {
        try {
            if (!Auth::user()->hasRole('Docente') && !Auth::user()->hasRole('Administrador')) {
            ...
            // Obtener sesiones de clase de hoy
            $sesionesHoy = \App\Models\SesionClase::with(['cursoAsignatura.curso.grado', 'cursoAsignatura.curso.seccion', 'cursoAsignatura.asignatura', 'aula'])
            ...
            return view('asistencia.docente-tomar-asistencia', compact('sesionesHoy'));
    ...
    public function docenteVerAsistencias()
    {
        try {
            ...
            // Obtener sesiones con asistencia registrada
            ...
            return view('asistencia.docente-ver-asistencias', compact('sesiones'));
    ...
    public function docenteReportes()
    {
        try {
            ...
            return view('asistencia.docente-reportes', compact('estadisticas'));
    ...
    public function docenteVerAsistencia($sesionClase)
    {
        try {
            ...
            return view('asistencia.docente-ver-asistencia', compact('sesion', 'asistencias'));
    ...
    public function docenteGuardarAsistencia(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'asistencias.*.tipo_asistencia_id' => 'required|exists:tiposasistencia,tipo_asistencia_id',
            ]);
            ...
            foreach ($request->asistencias as $asistenciaData) {
                \App\Models\AsistenciaAsignatura::updateOrCreate(
                    [
            ...
            DB::rollback();
            \Log::error('Error en docenteGuardarAsistencia: ' . $e->getMessage());
            return response()->json(['error' => 'Error al guardar la asistencia'], 500);
        }
    }
