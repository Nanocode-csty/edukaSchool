...
class InfCursoController extends Controller
{
    ...
    {
        $query = InfCurso::with(['grado'])->select('cursos.curso_id as id', 'cursos.grado_id')
            ->join('grados', 'cursos.grado_id', '=', 'grados.grado_id');

        // Filter by grado if provided
        if ($request->has('grado_id') && $request->grado_id) {
            $query->where('cursos.grado_id', $request->grado_id);
        }

        $cursos = $query->orderBy('grados.descripcion')
            ->get()
            ->map(function($curso) {
                return [
                    'id' => $curso->id,
                    'nombre' => $curso->grado->descripcion
                ];
            });
