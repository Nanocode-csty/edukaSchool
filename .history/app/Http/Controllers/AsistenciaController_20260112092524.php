...
class AsistenciaController extends Controller
{
    ...
    public function guardarReporteGenerado(Request $request)
    {
        try {
            ...
            try {
                ...
                \Log::info('Reporte creado en BD', ['reporte_id' => $reporte->reporte_id]);
    ...
    private function generarReportePDF($reporte, $filtros)
    {
        try {
            ...
            $maxRegistros = 1000; // Máximo 1000 registros por reporte PDF
            ...
            // Verificar límite para PDF
            if ($asistencias->count() >= $maxRegistros) {
                throw new \Exception("El reporte contiene demasiados registros ($maxRegistros+) para generar un PDF. Use Excel o aplique filtros más específicos.");
            }
    ...
    {
        // Obtener datos del reporte (igual que PDF)
        $query = AsistenciaAsignatura::with([
            'matricula.estudiante.persona',
            'matricula.grado',
            'matricula.seccion',
        ...
        if (isset($filtros['tipo_asistencia'])) {
            $tipoAsistencia = TipoAsistencia::where('codigo', $filtros['tipo_asistencia'])->first();
            if ($tipoAsistencia) {
                $query->where('tipo_asistencia_id', $tipoAsistencia->tipo_asistencia_id);
            }
        }
        ...
        if (isset($filtros['grado_id'])) {
            $query->whereHas('matricula', function($q) use ($filtros) {
                $q->where('idGrado', $filtros['grado_id']);
            });
        }
        ...
        if (isset($filtros['asignatura_id'])) {
            $query->whereHas('cursoAsignatura', function($q) use ($filtros) {
                $q->where('asignatura_id', $filtros['asignatura_id']);
            });
        }
        ...
        $sheet->setCellValue('D1', 'Sección');
        $sheet->setCellValue('E1', 'Asignatura');
        $sheet->setCellValue('F1', 'Tipo Asistencia');

        // Data
        $row = 2;
        foreach ($asistencias as $asistencia) {
            $sheet->setCellValue('A' . $row, $asistencia->fecha);
            $sheet->setCellValue('B' . $row, $asistencia->matricula->estudiante->persona->nombres . ' ' . $asistencia->matricula->estudiante->persona->apellidos);
            $sheet->setCellValue('C' . $row, $asistencia->matricula->grado->descripcion ?? '');
            $sheet->setCellValue('D' . $row, $asistencia->matricula->seccion->nombre ?? '');
            $sheet->setCellValue('E' . $row, $asistencia->cursoAsignatura->asignatura->nombre ?? '');
            $sheet->setCellValue('F' . $row, $asistencia->tipoAsistencia->nombre ?? '');
            $row++;
        }

        // Guardar archivo
        $nombreArchivo = 'reporte_' . $reporte->reporte_id . '.xlsx';
        $rutaCompleta = storage_path('app/public/reportes/' . $nombreArchivo);
