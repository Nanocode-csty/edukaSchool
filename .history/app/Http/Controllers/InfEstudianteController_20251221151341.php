    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        if ($request->filled('dniRepresentante1')) {
            $rep1 = InfRepresentante::whereHas('persona', function($query) use ($request) {
                $query->where('dni', $request->dniRepresentante1);
            })->first();
            if ($rep1) {
                ...
                $personaRep1->telefono_alternativo = $request->celularAlternativoRepresentante1;
        ...
        if ($request->filled('dniRepresentante2')) {
            $rep2 = InfRepresentante::whereHas('persona', function($query) use ($request) {
                $query->where('dni', $request->dniRepresentante2);
            })->first();
            if ($rep2) {
                ...
                $personaRep2->telefono_alternativo = $request->celularAlternativoRepresentante2;
    ...
    {
        try {
            $query = InfEstudiante::select('estudiantes.estudiante_id', 'personas.nombres', 'personas.apellidos')
                ->join('personas', 'estudiantes.persona_id', '=', 'personas.id_persona')
                ->join('matriculas', 'estudiantes.estudiante_id', '=', 'matriculas.estudiante_id')
