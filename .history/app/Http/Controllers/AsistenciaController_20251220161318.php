...
class AsistenciaController extends Controller
{
    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            $fechaFin = $request->fecha_fin;
            $timestamp = now()->format('Y-m-d_H-i-s');
            $archivoNombre = "reporte_{$tipoReporte}_{$fechaInicio}_{$fechaFin}_{$timestamp}.csv";
            ...
            // Crear archivo CSV usando funcionalidad nativa de Laravel
            try {
                $export = new AsistenciaExport($query, $filtrosAplicados);
                $data = $export->getData();

                \Log::info('Exportar CSV - Datos preparados', ['filas' => count($data)]);

                // Crear contenido CSV con BOM UTF-8 para compatibilidad con Excel
                $csvContent = "\xEF\xBB\xBF"; // UTF-8 BOM
                foreach ($data as $row) {
                    // Escapar comillas dobles y envolver campos que contienen comas
                    $escapedRow = array_map(function($field) {
                        // Convertir a string y manejar valores null
                        $field = (string) $field;
                ...
                // Guardar archivo CSV
                $archivoPath = "reportes/{$archivoNombre}";
                file_put_contents(storage_path('app/public/' . $archivoPath), $csvContent);
                ...
                \Log::info('Exportar CSV - Archivo creado exitosamente');
                ...
                \Log::error('Exportar CSV - Error en creación', [
                    'error' => $e->getMessage(),
                    'line' => $e->getLine(),
                    'file' => $e->getFile()
                ]);
                return back()->with('error', 'Error al crear archivo CSV: ' . $e->getMessage());
            }
