...
class AsistenciaController extends Controller
{
    ...
    private function generarReporteExcel($reporte, $filtros)
    {
        ...
        if (isset($filtros['estudiante_id']) && $filtros['estudiante_id']) {
            $filtrosTexto[] = 'Estudiante específico';
        }
        if (isset($filtros['grado_id']) && $filtros['grado_id']) {
            $filtrosTexto[] = 'Grado específico';
        }
        if (isset($filtros['docente_id']) && $filtros['docente_id']) {
            $filtrosTexto[] = 'Docente específico';
        }
        if (isset($filtros['asignatura_id']) && $filtros['asignatura_id']) {
            $filtrosTexto[] = 'Asignatura específica';
        }
        if (empty($filtrosTexto)) {
            $filtrosTexto[] = 'Sin filtros adicionales';
        }

        $sheet->setCellValue('A9', 'Filtros:');
        $sheet->setCellValue('B9', implode(', ', $filtrosTexto));
        ...
        $sheet->setCellValue('B16', 'Hora Registro');
        $sheet->setCellValue('C16', 'Estudiante');
        $sheet->setCellValue('D16', 'Grado');
        $sheet->setCellValue('E16', 'Sección');
        $sheet->setCellValue('F16', 'Asignatura');
        $sheet->setCellValue('G16', 'Tipo Asistencia');
        $sheet->setCellValue('H16', 'Estado');
        ...
        $sheet->getStyle('A16:H16')->applyFromArray($headerStyle);
        ...
        $sheet->getStyle('A16:H' . ($row - 1))->applyFromArray($styleArray);
