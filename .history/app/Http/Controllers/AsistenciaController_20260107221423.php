    ...
    public function exportarExcelAdmin(Request $request)
    {
        ...
        try {
            ...
            foreach ($asistencias as $asistencia) {
                // Safe access to student data with null checks
                $estudiante = $asistencia->matricula->estudiante ?? null;
                $persona = $estudiante ? ($estudiante->persona ?? null) : null;

                $nombres = $persona ? ($persona->nombres ?? 'Sin nombre') : 'Sin nombre';

                // Get last names directly from database fields, not from accessor
                $apellidoPaterno = $persona ? ($persona->apellido_paterno ?? '') : '';
                $apellidoMaterno = $persona ? ($persona->apellido_materno ?? '') : '';
                $apellidos = trim($apellidoPaterno . ' ' . $apellidoMaterno);

                // Only show "Sin apellidos" if truly empty, otherwise show what we have
                if (empty($apellidos)) {
