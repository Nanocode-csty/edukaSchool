...
class PeriodoMatricula extends Model
{
    ...
    protected $casts = [
        ...
        'orden' => 'integer',
        'permite_preinscripcion' => 'boolean',
        'permite_inscripcion' => 'boolean',
        'permite_matricula' => 'boolean',
        'clases_activas' => 'boolean'
    ];
    ...
    {
        $anio = $anio ?? date('Y');
        return $query->whereHas('anoLectivo', function($q) use ($anio) {
            $q->where('nombre', 'like', $anio . '%');
        });
    }
    ...
    {
        return self::with('anoLectivo')->activos()->delAnio()->ordenado()->get();
    }
