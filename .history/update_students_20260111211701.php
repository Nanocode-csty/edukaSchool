...
foreach ($estudiantes as $persona) {
    ...
    if (str_starts_with($persona->apellidos, 'del ')) {
        $needsUpdate = true;
    }

    if (str_contains($persona->nombres, 'Estudiante ')) {
        $needsUpdate = true;
    }

    // También verificar si los apellidos parecen ser placeholder (muy cortos o sin sentido)
    if (strlen($persona->apellidos) < 4) {
        $needsUpdate = true;
    }

    // Verificar si los apellidos contienen números o caracteres extraños
