...
$user = App\Models\Usuario::find(2); // docente1
if (!$user) {
    echo 'User with ID 27 not found. Let me check all users:' . PHP_EOL;
    $users = App\Models\Usuario::all();
    foreach ($users as $u) {
        echo 'User ID: ' . $u->usuario_id . ', Username: ' . $u->username . PHP_EOL;
    }
    exit;
}
...
// Check roles table
$allRoles = App\Models\Rol::all();
echo 'All roles in database:' . PHP_EOL;
foreach ($allRoles as $role) {
    echo '  - Role ID: ' . $role->id . ', Name: ' . $role->nombre . PHP_EOL;
}
