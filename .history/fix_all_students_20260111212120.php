...
foreach ($estudiantes as $persona) {
    ...
    // Verificar si contiene patrones de grados específicos
    if (preg_match('/del [1-6]°|del \d+°/i', $persona->apellidos)) {
        $needsUpdate = true;
    }

    // Verificar nombres placeholder más específicos
    if (preg_match('/estudiante \d+/i', $persona->nombres)) {
        $needsUpdate = true;
    }
