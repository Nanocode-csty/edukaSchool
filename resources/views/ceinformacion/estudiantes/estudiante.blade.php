<div id="paginacion-estudiantes" class="table-responsive mt-2">
    <table id="add-row" class="table-hover table" style="border-radius: 8px; overflow: hidden;">
        ...
        <tbody style="font-family: 'Quicksand', sans-serif !important;">
            ...
            @forelse ($estudiante as $index => $itemEstudiante)
                <tr class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">

                    <td class="text-center">{{ $itemEstudiante->persona->dni }}</td>
                    <td>
                        {{ ucwords(strtolower($itemEstudiante->persona->nombrecompleto)) }}
                    </td>
                    <td class="text-center">
                        <span class="badge badge-info"
                            style="background-color: #eee3d465; font-weight:bold; color:black; font-size:13px">{{ $itemEstudiante->persona->telefono_formato }}</span>
                    </td>
                    <td class="text-center g-4 ">

                        <a href="{{ route('estudiantes.ficha', $itemEstudiante->estudiante_id) }}"
                        ...
                        <a class="btn btn-sm btn-outline-primary toggle-action"
                            data-target="#collapseRow{{ $index }}">
                            <i class="ti ti-list-details mx-1"></i>
                        </a>

                        <a class="btn btn-sm btn-outline-primary"
                            href="{{ route('estudiantes.edit', $itemEstudiante->estudiante_id) }}">
                        ...
                        <div class="p-3 d-flex flex-wrap gap-3">
                            ...
                            <div class="flex-grow-1" style="min-width: 300px;">
                                <p><i class="icon-calendar mr-2"></i><strong>Fecha de Nacimiento:</strong>
                                    {{ $itemEstudiante->persona->fecha_nacimiento }}</p>
                                <p><i class="icon-information mr-2"></i><strong>Género:</strong>
                                    {{ $itemEstudiante->persona->genero_convertido }}
                                </p>
                                <p><i class="icon-envelope mr-2"></i><strong>Correo:</strong>
                                    <a
                                        href="mailto:{{ $itemEstudiante->persona->email }}">{{ $itemEstudiante->persona->email }}</a>
                                </p>
                                <p><i class="icon-location-pin mr-2"></i><strong>Dirección:</strong>
                                    <a href="https://www.google.com/maps/search/{{ urlencode($itemEstudiante->persona->direccion_completa) }}"
                                        target="_blank">
                                        {{ $itemEstudiante->persona->direccion_completa }}
                                    </a>
                                </p>
                                <p><i class="icon-calendar mr-2"></i><strong>Fecha de Registro:</strong>
                                    {{ $itemEstudiante->fecha_matricula }}
                                </p>
                            </div>
