...
<div id="tabla-representantes" class="table-responsive mt-2">
    <table id="add-row" class="table-hover table" style="border-radius: 8px; overflow: hidden;">
        ...
        <tbody style="font-family: 'Quicksand', sans-serif !important;">
            @forelse ($representante as $index => $itemRepresentante)
                <tr class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">

                    <td class="text-center">
                        {{ $itemRepresentante->persona->dni }}</td>
                    <td>
                        {{ $itemRepresentante->persona->nombre_completo }}
                    </td>

                    <td class="text-center">
                        <span class="badge badge-info"
                            style="background-color: #3091a4; font-weight:bold">{{ Str::substr($itemRepresentante->persona->telefono, 0, 3) . ' ' . Str::substr($itemRepresentante->persona->telefono, 3, 3) . ' ' . Str::substr($itemRepresentante->persona->telefono, 6, 3) }}</span>
                    </td>
