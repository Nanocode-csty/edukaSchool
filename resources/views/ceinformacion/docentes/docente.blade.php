        ...
        <tbody style="font-family: 'Quicksand', sans-serif !important;">
            @foreach ($docente as $index => $itemDocente)
                <tr class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                    ...
                    <td class="text-center">{{ $itemDocente->persona->dni }}</td>
                    <td>{{ $itemDocente->persona->apellidos . ', ' . $itemDocente->persona->nombres }}</td>
                    <td class=" text-center py-2">
                        <div data-bind="html:DescripcionEstado"><span class="badge badge-info"
                                ...
                                {{ $itemDocente->persona->especialidad }}</span>
                        </div>
                    </td>
                    <td class="text-center btn-action-group d-none d-md-block">
                        <!-- Botón -->
                        <button type="button" class="btn btn-link btn-sm" data-toggle="modal"
                            data-target="#modalEditar{{ $itemDocente->persona->profesor_id }}">
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="direccion" class="form-control"
                                                    value="{{ $itemDocente->persona->direccion }}" required>
                                            </div>
                                            <div class="form-group">
                                                <label>Teléfono</label>
                                                <input type="number" name="telefono" class="form-control"
                                                    value="{{ $itemDocente->persona->telefono }}" maxlength="9"
                                                    required>
                                            </div>
                                            <div class="form-group">
                                                <label>Email</label>
                                                <input type="email" name="email" class="form-control"
                                                    value="{{ $itemDocente->persona->email }}" required>
                                            </div>
                                            <div class="form-group">
                                                <label>Especialidad</label>
                                                <input type="text" name="especialidad" class="form-control"
                                                    value="{{ $itemDocente->persona->especialidad }}" required>
                                            </div>
                                            <div class="form-group">
                                                <label>Fecha de Contratación</label>
                                                <input type="date" name="fecha_contratacion" class="form-control"
                                                    value="{{ $itemDocente->persona->fecha_contratacion }}" required>
                                            </div>
                                            <div class="form-group">
                                                <label>Estado</label>
                                                <select name="estado" class="form-control" required>
                                                    <option
                                                        {{ $itemDocente->persona->estado == 'Activo' ? 'selected' : '' }}>
                                                        Activo</option>
                                                    <option
                                                        {{ $itemDocente->persona->estado == 'Inactivo' ? 'selected' : '' }}>
                                                        Inactivo</option>
                                                    <option
                                                        {{ $itemDocente->persona->estado == 'Licencia' ? 'selected' : '' }}>
                                                        Licencia</option>
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <form class="form-eliminar-docente"
                            data-nombre="{{ $itemDocente->persona->apellidos . ', ' . $itemDocente->persona->nombres }}"
                        ...
                        <div class="p-3 d-flex justify-content-between align-items-start" style="gap: 2rem;">
                            ...
                            <div class="flex-grow-1">
                                <p>
                                    ...
                                    <strong>Fecha de Nacimiento:</strong> {{ $itemDocente->persona->fecha_nacimiento }}
                                </p>
                                <p><i class="icon-information mr-4"></i><strong>Género:
                                    </strong>{{ $itemDocente->persona->genero == 'M' ? 'Masculino' : ($itemDocente->persona->genero == 'F' ? 'Femenino' : 'No especificado') }}
                                </p>
                                <p>
                                    <i class="icon-location-pin mr-4"></i>
                                    <strong>Dirección:</strong> {{ $itemDocente->persona->direccion }}
                                </p>
                                <p>
                                    <i class="icon-phone mr-4"></i>
                                    <strong>Teléfono:</strong> {{ $itemDocente->persona->telefono }}
                                </p>
                                <p>
                                    <i class="icon-envelope mr-4"></i>
                                    <strong>Correo:</strong> {{ $itemDocente->persona->email }}
                                </p>
                                <p>
                                    <i class="icon-calendar mr-4"></i>
                                    <strong>Fecha de Contratación:</strong>
                                    {{ $itemDocente->persona->fecha_contratacion }}
                                </p>
                            </div>
