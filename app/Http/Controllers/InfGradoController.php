...
class InfGradoController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($buscarpor) {
            $query->where(function ($q) use ($buscarpor) {
                $q->where('grados.nombre', 'like', '%' . $buscarpor . '%')
                    ...
                    ->orWhere('grados.nivel_id', 'like', '%' . $buscarpor . '%');
            });
        }
        ...
        // Obtener los niveles que tienen grados(tabla grados) registrados para el filtro
        $niveles = InfNivel::join('grados', 'niveleseducativos.nivel_id', '=', 'grados.nivel_id')
            ->select('niveleseducativos.*')
            ->distinct()
    ...
    {
        $query = InfGrado::select('grados.grado_id as id', 'grados.descripcion', 'grados.nivel_id')
        ...
        // Filter by nivel if provided (cascading)
        if ($request->filled('nivel_id')) {
            $query->where('grados.nivel_id', $request->nivel_id);
        }

        $grados = $query->orderBy('grados.descripcion', 'asc')->get();
