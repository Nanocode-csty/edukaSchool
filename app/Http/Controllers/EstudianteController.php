...
class EstudianteController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $asistenciasMes = AsistenciaAsignatura::where('matricula_id', $estudiante->matricula->id)
        ...
        // Asistencia de hoy
        $asistenciaHoy = AsistenciaAsignatura::where('matricula_id', $estudiante->matricula->id)
            ->whereDate('fecha', today())
            ->with('tipoAsistencia')
            ->first();

        // Últimas 5 asistencias
        $ultimasAsistencias = AsistenciaAsignatura::where('matricula_id', $estudiante->matricula->id)
            ->with(['tipoAsistencia', 'cursoAsignatura.asignatura'])
            ->orderBy('fecha', 'desc')
            ->limit(5)
            ->get();
    ...
    public function verAsistencia(Request $request)
    {
        ...
        $asistencias = AsistenciaAsignatura::where('matricula_id', $estudiante->matricula->id)
            ->whereMonth('fecha', $mes)
            ->whereYear('fecha', $anio)
            ->with(['cursoAsignatura.asignatura', 'tipoAsistencia'])
            ->orderBy('fecha', 'desc')
            ->paginate(20);
