...
class Matricula extends Model
{
    ...
    {
        return $this->hasMany(AsistenciaAsignatura::class, 'matricula_id', 'matricula_id');
    }
    ...
    public static function calcularSiguienteGrado($estudiante_id)
    {
        try {
            ...
            \Log::error('Error en calcularSiguienteGrado: ' . $e->getMessage());
            // Fallback: return first available grade
            return InfGrado::orderBy('grado_id')->first();
        }
    }
    ...
    public static function getSeccionesDisponibles($grado_id, $anio_academico = null)
    {
        try {
            ...
            return $todasLasSecciones->map(function ($seccion) use ($seccionesOcupadas) {
                ...
                $disponibles = isset($seccion->capacidad_maxima) ? $seccion->capacidad_maxima - ($ocupadas->total ?? 0) : 0;
            ...
            \Log::error('Error en getSeccionesDisponibles: ' . $e->getMessage());
