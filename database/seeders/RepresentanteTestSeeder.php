...
class RepresentanteTestSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $rolRepresentante = Rol::firstOrCreate(['nombre' => 'Representante']);

        // Asignar el rol a la persona (usando attach que maneja duplicados)
        try {
            $persona->roles()->attach($rolRepresentante->id_rol, ['fecha_asignacion' => now()]);
        } catch (\Exception $e) {
            // El rol ya está asignado, continuar
        }

        // Crear representante con ID específico (26)
        $representante = InfRepresentante::updateOrCreate(
            ['representante_id' => 26],
            [
                'persona_id' => $persona->id_persona,
                'parentesco' => 'Madre',
